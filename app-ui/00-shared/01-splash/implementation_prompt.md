# Implementation prompt — `00-shared/01-splash`
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `01-splash` |
| Chronology | `S-001` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/01-splash` |
...
| Localization prefix | `00_shared.01_splash` |
...
Folder: `app-ui/00-shared/01-splash/`
...
| Page | `frontend/lib/features/shared/presentation/pages/01_splash_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/01-splash`) |
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Get started → `/00-shared/03-login`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/01-splash` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.01_splash` only — no hard-coded user strings.
