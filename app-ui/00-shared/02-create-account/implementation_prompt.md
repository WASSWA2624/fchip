# Implementation prompt — `00-shared/02-create-account`
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `02-create-account` |
| Chronology | `S-002` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/02-create-account` |
...
| Localization prefix | `00_shared.02_create_account` |
...
Folder: `app-ui/00-shared/02-create-account/`
...
| Page | `frontend/lib/features/shared/presentation/pages/02_create_account_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/02-create-account`) |
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Create account → `/00-shared/05-consent-first-onboarding`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/02-create-account` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.02_create_account` only — no hard-coded user strings.
