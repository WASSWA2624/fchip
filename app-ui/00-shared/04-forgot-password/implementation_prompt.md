# Implementation prompt — `00-shared/04-forgot-password`
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `04-forgot-password` |
| Chronology | `S-004` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/04-forgot-password` |
...
| Localization prefix | `00_shared.04_forgot_password` |
...
Folder: `app-ui/00-shared/04-forgot-password/`
...
| Page | `frontend/lib/features/shared/presentation/pages/04_forgot_password_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/04-forgot-password`) |
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Save new password → `/00-shared/03-login`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/04-forgot-password` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.04_forgot_password` only — no hard-coded user strings.
