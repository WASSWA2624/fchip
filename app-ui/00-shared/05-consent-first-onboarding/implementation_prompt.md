# Implementation prompt — `00-shared/05-consent-first-onboarding`
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `05-consent-first-onboarding` |
| Chronology | `S-005` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/05-consent-first-onboarding` |
...
| Localization prefix | `00_shared.05_consent_first_onboarding` |
...
Folder: `app-ui/00-shared/05-consent-first-onboarding/`
...
| Page | `frontend/lib/features/shared/presentation/pages/05_consent_first_onboarding_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/05-consent-first-onboarding`) |
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: I understand · continue → `/00-shared/08-role-surface-picker`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/05-consent-first-onboarding` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.05_consent_first_onboarding` only — no hard-coded user strings.
