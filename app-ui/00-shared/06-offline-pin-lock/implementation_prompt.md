# Implementation prompt — `00-shared/06-offline-pin-lock`
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `06-offline-pin-lock` |
| Chronology | `S-006` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/06-offline-pin-lock` |
...
| Localization prefix | `00_shared.06_offline_pin_lock` |
...
Folder: `app-ui/00-shared/06-offline-pin-lock/`
...
| Page | `frontend/lib/features/shared/presentation/pages/06_offline_pin_lock_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/06-offline-pin-lock`) |
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Unlock → `/01-chw-vht-mobile/01-worklist-home`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/06-offline-pin-lock` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.06_offline_pin_lock` only — no hard-coded user strings.
