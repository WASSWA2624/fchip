# Implementation prompt — `00-shared/07-session-locked`
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `07-session-locked` |
| Chronology | `S-007` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/07-session-locked` |
...
| Localization prefix | `00_shared.07_session_locked` |
...
Folder: `app-ui/00-shared/07-session-locked/`
...
| Page | `frontend/lib/features/shared/presentation/pages/07_session_locked_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/07-session-locked`) |
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Sign in again → `/00-shared/08-role-surface-picker`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/07-session-locked` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.07_session_locked` only — no hard-coded user strings.
