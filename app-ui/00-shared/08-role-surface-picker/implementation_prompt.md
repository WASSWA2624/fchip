# Implementation prompt — `00-shared/08-role-surface-picker`
...
| Kind | Screen (`list`) |
| Module | `00-shared` |
| Screen slug | `08-role-surface-picker` |
| Chronology | `S-008` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
...
| Localization prefix | `00_shared.08_role_surface_picker` |
...
Folder: `app-ui/00-shared/08-role-surface-picker/`
...
| Page | `frontend/lib/features/shared/presentation/pages/08_role_surface_picker_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/08-role-surface-picker`) |
...
8. Register route `/00-shared/08-role-surface-picker` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.08_role_surface_picker` only — no hard-coded user strings.
