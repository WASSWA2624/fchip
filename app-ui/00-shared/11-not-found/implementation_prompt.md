# Implementation prompt — `00-shared/11-not-found`
...
| Kind | Screen (`empty`) |
| Module | `00-shared` |
| Screen slug | `11-not-found` |
| Chronology | `S-011` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/11-not-found` |
...
| Localization prefix | `00_shared.11_not_found` |
...
Folder: `app-ui/00-shared/11-not-found/`
...
| Page | `frontend/lib/features/shared/presentation/pages/11_not_found_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/11-not-found`) |
...
8. Register route `/00-shared/11-not-found` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.11_not_found` only — no hard-coded user strings.
