# Implementation prompt — `00-shared/12-preferences`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Language & appearance** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Choose readable settings for this device_
...
| Module | `00-shared` |
| Screen slug | `12-preferences` |
| Chronology | `S-012` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/12-preferences` |
| Parent route | `/00-shared/12-preferences` |
...
| Localization prefix | `00_shared.12_preferences` |
...
Folder: `app-ui/00-shared/12-preferences/`
...
| Page | `frontend/lib/features/shared/presentation/pages/12_preferences_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/12-preferences`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-012`:

### Phase 0 — Claim the screen

1. Confirm `S-012` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Workspaces → `/00-shared/08-role-surface-picker`; Notifications → `/00-shared/09-notifications-center`; Preferences → `/00-shared/12-preferences`; Sign out → `/00-shared/03-login`
   - Primary action: Save preferences → `/00-shared/12-preferences?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/12-preferences` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.12_preferences` only — no hard-coded user strings.
...
23. Mark `S-012` `status: done`, `backend: paired` (or `none` with static evidence only).
