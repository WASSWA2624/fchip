# Implementation prompt — `00-shared/access-denied`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Access denied** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: You do not have permission to open this workspace_
...
| Kind | Screen (`empty`) |
| Module | `00-shared` |
| Screen slug | `access-denied` |
| Chronology | `S-010` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/access-denied` |
| Parent route | `/00-shared/role-surface-picker` |
| Layout | `empty-state-shell` |
| Body layouts | `mobile` → `empty-state-shell`, `tablet` → `empty-state-shell`, `desktop` → `empty-state-shell` |
...
| Localization prefix | `00_shared.access_denied` |
...
Folder: `app-ui/00-shared/access-denied/`
...
- Shared layout specimens for `empty-state-shell` under `app-ui/00-shared/layouts/empty-state-shell/`
...
| Page | `frontend/lib/features/shared/presentation/pages/access_denied_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/access-denied`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `empty-state-shell`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-010`:

### Phase 0 — Claim the screen

1. Confirm `S-010` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `empty-state-shell` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   - Tabs: (none)
   - Primary action: Return to workspaces → `/00-shared/role-surface-picker`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/access-denied` with the access guard for roles `authenticated-user`.
9. Compose the page from shared shells + `empty-state-shell` + catalog components. Prefer:
...
11. Add ARB keys under `00_shared.access_denied` only — no hard-coded user strings.
...
23. Mark `S-010` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `empty-state-shell` is a shared layout implementation, not a one-off Scaffold tree
