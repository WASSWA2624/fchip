# Implementation prompt — shared component `01-brand/03-master-loop-badge`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Master loop badge** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: CAPTURE → FUSE → PREDICT → ALERT → ACT → LEARN_
...
| Category | `01-brand` |
| Slug | `03-master-loop-badge` |
| Suggested Dart name | `App03MasterLoopBadge` |
...
Folder: `app-ui/00-shared/components/01-brand/03-master-loop-badge/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/01-brand/app_03_master_loop_badge.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/01-brand/app_03_master_loop_badge_test.dart` |
| Goldens | `frontend/test/shared/components/01-brand/goldens/` |
...
4. Implement `App03MasterLoopBadge` with:
