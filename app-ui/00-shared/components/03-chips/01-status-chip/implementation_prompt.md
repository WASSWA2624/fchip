# Implementation prompt — shared component `03-chips/01-status-chip`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Status chip** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Compact status or count_
...
| Category | `03-chips` |
| Slug | `01-status-chip` |
| Suggested Dart name | `App01StatusChip` |
| Used by | `worklists`, `queues` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/components/03-chips/01-status-chip/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/03-chips/app_01_status_chip.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/03-chips/app_01_status_chip_test.dart` |
...
4. Implement `App01StatusChip` with:
