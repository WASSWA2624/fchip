# Implementation prompt — shared component `03-chips/02-filter-chip-row`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Filter chip row** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Horizontal chip group_
...
| Category | `03-chips` |
| Slug | `02-filter-chip-row` |
| Suggested Dart name | `App02FilterChipRow` |
| Used by | `maps`, `lists` |
...
Folder: `app-ui/00-shared/components/03-chips/02-filter-chip-row/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/03-chips/app_02_filter_chip_row.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/03-chips/app_02_filter_chip_row_test.dart` |
...
4. Implement `App02FilterChipRow` with:
