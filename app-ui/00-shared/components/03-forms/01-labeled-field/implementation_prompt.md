# Implementation prompt — shared component `03-forms/01-labeled-field`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Labeled field** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Label + value input shell_
...
| Category | `03-forms` |
| Slug | `01-labeled-field` |
| Suggested Dart name | `App01LabeledField` |
| Used by | `all forms` |
...
Folder: `app-ui/00-shared/components/03-forms/01-labeled-field/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/03-forms/app_01_labeled_field.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/03-forms/app_01_labeled_field_test.dart` |
...
4. Implement `App01LabeledField` with:
