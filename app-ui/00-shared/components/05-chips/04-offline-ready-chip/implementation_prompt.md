# Implementation prompt — shared component `05-chips/04-offline-ready-chip`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Offline-ready chip** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Field offline readiness_
...
| Category | `05-chips` |
| Slug | `04-offline-ready-chip` |
| Suggested Dart name | `App04OfflineReadyChip` |
| Used by | `CHW`, `feeders` |
...
Folder: `app-ui/00-shared/components/05-chips/04-offline-ready-chip/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/05-chips/app_04_offline_ready_chip.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/05-chips/app_04_offline_ready_chip_test.dart` |
| Goldens | `frontend/test/shared/components/05-chips/goldens/` |
...
4. Implement `App04OfflineReadyChip` with:
