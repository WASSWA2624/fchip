# Implementation prompt — shared component `06-navigation/01-top-app-bar`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Top app bar** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Brand + slogan / screen title_
...
| Slug | `01-top-app-bar` |
| Suggested Dart name | `App01TopAppBar` |
...
Folder: `app-ui/00-shared/components/06-navigation/01-top-app-bar/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/06-navigation/app_01_top_app_bar.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/06-navigation/app_01_top_app_bar_test.dart` |
...
4. Implement `App01TopAppBar` with:
