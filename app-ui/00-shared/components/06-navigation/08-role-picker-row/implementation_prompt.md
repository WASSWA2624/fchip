# Implementation prompt — shared component `06-navigation/08-role-picker-row`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Role picker row** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Workspace choice row_
...
| Slug | `08-role-picker-row` |
| Suggested Dart name | `App08RolePickerRow` |
| Used by | `role-surface-picker` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/components/06-navigation/08-role-picker-row/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/06-navigation/app_08_role_picker_row.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/06-navigation/app_08_role_picker_row_test.dart` |
...
4. Implement `App08RolePickerRow` with:
