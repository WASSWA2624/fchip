# Implementation prompt — shared component `07-feedback/02-loading-skeleton`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Loading skeleton** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Placeholder while data loads_
...
| Slug | `02-loading-skeleton` |
| Suggested Dart name | `App02LoadingSkeleton` |
| Used by | `dashboards` |
...
Folder: `app-ui/00-shared/components/07-feedback/02-loading-skeleton/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/07-feedback/app_02_loading_skeleton.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/07-feedback/app_02_loading_skeleton_test.dart` |
...
4. Implement `App02LoadingSkeleton` with:
