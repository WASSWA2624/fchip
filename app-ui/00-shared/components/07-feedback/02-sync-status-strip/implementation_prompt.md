# Implementation prompt — shared component `07-feedback/02-sync-status-strip`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Sync status strip** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Queue · last sync · errors_
...
| Slug | `02-sync-status-strip` |
| Suggested Dart name | `App02SyncStatusStrip` |
| Used by | `sync screens`, `feeder sync` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/components/07-feedback/02-sync-status-strip/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/07-feedback/app_02_sync_status_strip.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/07-feedback/app_02_sync_status_strip_test.dart` |
| Goldens | `frontend/test/shared/components/07-feedback/goldens/` |
...
4. Implement `App02SyncStatusStrip` with:
