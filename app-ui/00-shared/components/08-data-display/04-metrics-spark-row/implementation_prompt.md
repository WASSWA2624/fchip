# Implementation prompt — shared component `08-data-display/04-metrics-spark-row`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Metrics spark row** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Compact trend labels_
...
| Slug | `04-metrics-spark-row` |
| Suggested Dart name | `App04MetricsSparkRow` |
| Used by | `facility`, `NGO` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/components/08-data-display/04-metrics-spark-row/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/08-data-display/app_04_metrics_spark_row.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/08-data-display/app_04_metrics_spark_row_test.dart` |
...
4. Implement `App04MetricsSparkRow` with:
