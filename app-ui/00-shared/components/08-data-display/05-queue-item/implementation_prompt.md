# Implementation prompt — shared component `08-data-display/05-queue-item`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Queue item** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Desk / referral / lab queue row_
...
| Slug | `05-queue-item` |
| Suggested Dart name | `App05QueueItem` |
| Used by | `07`, `16` |
...
Folder: `app-ui/00-shared/components/08-data-display/05-queue-item/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/08-data-display/app_05_queue_item.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/08-data-display/app_05_queue_item_test.dart` |
...
4. Implement `App05QueueItem` with:
