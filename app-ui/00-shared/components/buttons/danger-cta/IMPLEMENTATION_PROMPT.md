# Implementation prompt — shared component `buttons/danger-cta`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Danger CTA** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Destructive or high-urgency action_
...
| Category | `buttons` |
| Slug | `danger-cta` |
| Suggested Dart name | `AppDangerCta` |
| Used by | `alerts`, `revoke access` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/components/buttons/danger-cta/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/buttons/app_danger_cta.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/buttons/app_danger_cta_test.dart` |
| Goldens | `frontend/test/shared/components/buttons/goldens/` |
