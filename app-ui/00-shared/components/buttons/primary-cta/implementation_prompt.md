# Implementation prompt — shared component `buttons/primary-cta`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Primary CTA** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Main action on a screen_
...
| Category | `buttons` |
| Slug | `primary-cta` |
| Suggested Dart name | `AppPrimaryCta` |
| Used by | `forms`, `lists`, `dashboards` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/components/buttons/primary-cta/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/buttons/app_primary_cta.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/buttons/app_primary_cta_test.dart` |
...
4. Implement `AppPrimaryCta` with:
