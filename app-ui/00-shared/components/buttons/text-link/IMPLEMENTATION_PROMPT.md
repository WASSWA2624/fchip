# Implementation prompt — shared component `buttons/text-link`

> Paste this whole file into an agent session when implementing this reusable component.

## Mission

Implement **Text link** as a single reusable Flutter widget in `frontend/lib/shared/components`, matching the six specimens, with no feature business logic, so every FCHIP screen can compose it safely.

_Purpose: Inline navigation without button chrome_
...
| Category | `buttons` |
| Slug | `text-link` |
| Suggested Dart name | `AppTextLink` |
| Used by | `lists`, `auth` |
...
Folder: `app-ui/00-shared/components/buttons/text-link/`
...
| Layer | Path |
| --- | --- |
| Widget | `frontend/lib/shared/components/buttons/app_text_link.dart` |
| Barrel / exports | `frontend/lib/shared/components/` |
| Widget tests | `frontend/test/shared/components/buttons/app_text_link_test.dart` |
