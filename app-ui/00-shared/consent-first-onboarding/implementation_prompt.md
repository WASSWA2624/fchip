# Implementation prompt — `00-shared/consent-first-onboarding`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Consent first** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Privacy before capture — least privilege_
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `consent-first-onboarding` |
| Chronology | `S-005` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/consent-first-onboarding` |
| Parent route | `/00-shared/role-surface-picker` |
| Layout | `auth-centered-card` |
| Body layouts | `mobile` → `auth-centered-card`, `tablet` → `auth-centered-card`, `desktop` → `auth-centered-card` |
| Shells | `mobile` → `auth-centered-card`, `tablet` → `auth-centered-card`, `desktop` → `auth-centered-card` |
| Roles | `authenticated-user` |
| Supported states | `default`, `loading`, `error` |
| Localization prefix | `00_shared.consent_first_onboarding` |
...
Folder: `app-ui/00-shared/consent-first-onboarding/`
...
- `app-ui/00-shared/README.md`
- `app-ui/00-shared/tokens.json`
- Shared layout specimens for `auth-centered-card` under `app-ui/00-shared/layouts/auth-centered-card/`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/shared/` |
| Page | `frontend/lib/features/shared/presentation/pages/consent_first_onboarding_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/shared/presentation/widgets/` |
| Presentation state | `frontend/lib/features/shared/presentation/state/` |
| Domain | `frontend/lib/features/shared/domain/` |
| Data / repository impl | `frontend/lib/features/shared/data/` |
| Repository contract | `frontend/lib/features/shared/domain/repositories/shared_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/consent-first-onboarding`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `auth-centered-card`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/shared/` |
| Backend module | `backend/src/modules/shared/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-005`:

### Phase 0 — Claim the screen

1. Confirm `S-005` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `auth-centered-card` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   - Tabs: (none)
   - Primary action: I understand · continue → `/00-shared/role-surface-picker`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/consent-first-onboarding` with the access guard for roles `authenticated-user`.
9. Compose the page from shared shells + `auth-centered-card` + catalog components. Prefer:
...
10. Implement **every** supported state: `default`, `loading`, `error`.
11. Add ARB keys under `00_shared.consent_first_onboarding` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/shared/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/shared/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-005` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `auth-centered-card` is a shared layout implementation, not a one-off Scaffold tree
