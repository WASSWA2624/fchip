# Implementation prompt — `00-shared/create-account`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Create account** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Phone number + password only — no other auth_
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `create-account` |
| Chronology | `S-002` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/create-account` |
...
| Localization prefix | `00_shared.create_account` |
...
Folder: `app-ui/00-shared/create-account/`
...
| Page | `frontend/lib/features/shared/presentation/pages/create_account_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/create-account`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-002`:

### Phase 0 — Claim the screen

1. Confirm `S-002` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Create account → `/00-shared/consent-first-onboarding`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/create-account` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.create_account` only — no hard-coded user strings.
...
23. Mark `S-002` `status: done`, `backend: paired` (or `none` with static evidence only).
