# Implementation prompt — shared layout `01-auth-centered-card`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Auth centered card** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `01-auth-centered-card` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: Create account / sign in — phone number + password only_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `01-auth-centered-card` |
| Suggested Dart name | `App01AuthCenteredCard` |
| Surfaces | `00-shared` |
...
Folder: `app-ui/00-shared/layouts/01-auth-centered-card/`
...
- `01-logo-lockup` → `app-ui/00-shared/components/**/01-logo-lockup/` + `frontend/lib/shared/components/`
- `02-slogan-line` → `app-ui/00-shared/components/**/02-slogan-line/` + `frontend/lib/shared/components/`
- `01-labeled-field` → `app-ui/00-shared/components/**/01-labeled-field/` + `frontend/lib/shared/components/`
- `01-primary-cta` → `app-ui/00-shared/components/**/01-primary-cta/` + `frontend/lib/shared/components/`
- `03-master-loop-badge` → `app-ui/00-shared/components/**/03-master-loop-badge/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_01_auth_centered_card.dart` |
...
| Tests | `frontend/test/shared/layout/app_01_auth_centered_card_test.dart` |
...
1. Confirm whether `01-auth-centered-card` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `01-auth-centered-card`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "01-auth-centered-card"` can mount without forking chrome
