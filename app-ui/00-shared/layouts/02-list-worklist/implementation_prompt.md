# Implementation prompt — shared layout `02-list-worklist`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **List / worklist** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `02-list-worklist` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `02-list-worklist` |
| Suggested Dart name | `App02ListWorklist` |
...
Folder: `app-ui/00-shared/layouts/02-list-worklist/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_02_list_worklist.dart` |
...
| Tests | `frontend/test/shared/layout/app_02_list_worklist_test.dart` |
...
1. Confirm whether `02-list-worklist` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `02-list-worklist`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "02-list-worklist"` can mount without forking chrome
