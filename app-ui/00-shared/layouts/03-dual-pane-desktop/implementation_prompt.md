# Implementation prompt — shared layout `03-dual-pane-desktop`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Dual-pane desktop** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `03-dual-pane-desktop` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `03-dual-pane-desktop` |
| Suggested Dart name | `App03DualPaneDesktop` |
...
Folder: `app-ui/00-shared/layouts/03-dual-pane-desktop/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_03_dual_pane_desktop.dart` |
...
| Tests | `frontend/test/shared/layout/app_03_dual_pane_desktop_test.dart` |
...
1. Confirm whether `03-dual-pane-desktop` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `03-dual-pane-desktop`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "03-dual-pane-desktop"` can mount without forking chrome
