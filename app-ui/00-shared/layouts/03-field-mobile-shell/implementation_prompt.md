# Implementation prompt — shared layout `03-field-mobile-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Field mobile shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `03-field-mobile-shell` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `03-field-mobile-shell` |
| Suggested Dart name | `App03FieldMobileShell` |
...
Folder: `app-ui/00-shared/layouts/03-field-mobile-shell/`
...
- `04-bottom-nav-field` → `app-ui/00-shared/components/**/04-bottom-nav-field/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_03_field_mobile_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_03_field_mobile_shell_test.dart` |
...
1. Confirm whether `03-field-mobile-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `03-field-mobile-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "03-field-mobile-shell"` can mount without forking chrome
