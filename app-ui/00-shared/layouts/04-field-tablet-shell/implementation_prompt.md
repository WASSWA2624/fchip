# Implementation prompt — shared layout `04-field-tablet-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Field tablet shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `04-field-tablet-shell` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `04-field-tablet-shell` |
| Suggested Dart name | `App04FieldTabletShell` |
...
Folder: `app-ui/00-shared/layouts/04-field-tablet-shell/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_04_field_tablet_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_04_field_tablet_shell_test.dart` |
...
1. Confirm whether `04-field-tablet-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `04-field-tablet-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "04-field-tablet-shell"` can mount without forking chrome
