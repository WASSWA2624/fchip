# Implementation prompt — shared layout `05-desktop-sidebar-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Desktop sidebar shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `05-desktop-sidebar-shell` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `05-desktop-sidebar-shell` |
| Suggested Dart name | `App05DesktopSidebarShell` |
...
Folder: `app-ui/00-shared/layouts/05-desktop-sidebar-shell/`
...
- `05-side-nav-desktop` → `app-ui/00-shared/components/**/05-side-nav-desktop/` + `frontend/lib/shared/components/`
- `03-top-app-bar` → `app-ui/00-shared/components/**/03-top-app-bar/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_05_desktop_sidebar_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_05_desktop_sidebar_shell_test.dart` |
...
1. Confirm whether `05-desktop-sidebar-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `05-desktop-sidebar-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "05-desktop-sidebar-shell"` can mount without forking chrome
