# Implementation prompt — shared layout `06-desktop-sidebar-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Desktop sidebar shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `06-desktop-sidebar-shell` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: Facility · district · admin · feeder desktop chrome_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `06-desktop-sidebar-shell` |
| Suggested Dart name | `App06DesktopSidebarShell` |
| Surfaces | `06–13`, `14–22 desktop` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/06-desktop-sidebar-shell/`
...
- `07-side-nav-desktop` → `app-ui/00-shared/components/**/07-side-nav-desktop/` + `frontend/lib/shared/components/`
- `01-top-app-bar` → `app-ui/00-shared/components/**/01-top-app-bar/` + `frontend/lib/shared/components/`
- `04-cascade-footer` → `app-ui/00-shared/components/**/04-cascade-footer/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_06_desktop_sidebar_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_06_desktop_sidebar_shell_test.dart` |
...
1. Confirm whether `06-desktop-sidebar-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `06-desktop-sidebar-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "06-desktop-sidebar-shell"` can mount without forking chrome
