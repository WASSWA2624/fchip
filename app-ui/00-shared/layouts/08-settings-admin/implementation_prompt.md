# Implementation prompt — shared layout `08-settings-admin`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Settings / admin** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `08-settings-admin` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `08-settings-admin` |
| Suggested Dart name | `App08SettingsAdmin` |
...
Folder: `app-ui/00-shared/layouts/08-settings-admin/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_08_settings_admin.dart` |
...
| Tests | `frontend/test/shared/layout/app_08_settings_admin_test.dart` |
...
1. Confirm whether `08-settings-admin` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `08-settings-admin`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "08-settings-admin"` can mount without forking chrome
