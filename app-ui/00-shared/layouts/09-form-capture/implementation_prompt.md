# Implementation prompt — shared layout `09-form-capture`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Form capture** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `09-form-capture` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `09-form-capture` |
| Suggested Dart name | `App09FormCapture` |
...
Folder: `app-ui/00-shared/layouts/09-form-capture/`
...
- `02-form-stack` → `app-ui/00-shared/components/**/02-form-stack/` + `frontend/lib/shared/components/`
- `03-consent-toggle` → `app-ui/00-shared/components/**/03-consent-toggle/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_09_form_capture.dart` |
...
| Tests | `frontend/test/shared/layout/app_09_form_capture_test.dart` |
...
1. Confirm whether `09-form-capture` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `09-form-capture`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "09-form-capture"` can mount without forking chrome
