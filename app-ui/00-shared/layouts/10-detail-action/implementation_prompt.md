# Implementation prompt — shared layout `10-detail-action`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Detail + action** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `10-detail-action` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: Explainable alert / referral detail with CTA_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `10-detail-action` |
| Suggested Dart name | `App10DetailAction` |
| Surfaces | `alert follow-up`, `referral detail`, `deploy action` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/10-detail-action/`
...
- `06-explainability-block` → `app-ui/00-shared/components/**/06-explainability-block/` + `frontend/lib/shared/components/`
...
- `03-danger-cta` → `app-ui/00-shared/components/**/03-danger-cta/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_10_detail_action.dart` |
...
| Tests | `frontend/test/shared/layout/app_10_detail_action_test.dart` |
...
1. Confirm whether `10-detail-action` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `10-detail-action`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "10-detail-action"` can mount without forking chrome
