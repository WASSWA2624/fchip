# Implementation prompt — shared layout `11-connector-status`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Connector status** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `11-connector-status` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: External system health (EMR, HMIS, climate)_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `11-connector-status` |
| Suggested Dart name | `App11ConnectorStatus` |
| Surfaces | `08`, `20`, `22`, `09 feeder board` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/11-connector-status/`
...
- `03-feeder-health-pill` → `app-ui/00-shared/components/**/03-feeder-health-pill/` + `frontend/lib/shared/components/`
- `06-warn-banner` → `app-ui/00-shared/components/**/06-warn-banner/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_11_connector_status.dart` |
...
| Tests | `frontend/test/shared/layout/app_11_connector_status_test.dart` |
...
1. Confirm whether `11-connector-status` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `11-connector-status`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "11-connector-status"` can mount without forking chrome
