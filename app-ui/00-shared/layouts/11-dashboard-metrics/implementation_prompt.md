# Implementation prompt — shared layout `11-dashboard-metrics`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Dashboard metrics** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `11-dashboard-metrics` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `11-dashboard-metrics` |
| Suggested Dart name | `App11DashboardMetrics` |
...
Folder: `app-ui/00-shared/layouts/11-dashboard-metrics/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_11_dashboard_metrics.dart` |
...
| Tests | `frontend/test/shared/layout/app_11_dashboard_metrics_test.dart` |
...
1. Confirm whether `11-dashboard-metrics` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `11-dashboard-metrics`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "11-dashboard-metrics"` can mount without forking chrome
