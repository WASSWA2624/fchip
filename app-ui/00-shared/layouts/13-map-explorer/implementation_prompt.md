# Implementation prompt — shared layout `13-map-explorer`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Map explorer** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `13-map-explorer` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: GIS hotspot + climate legend_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `13-map-explorer` |
| Suggested Dart name | `App13MapExplorer` |
| Surfaces | `facility map`, `district map`, `coverage`, `population` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/13-map-explorer/`
...
- `02-filter-chip-row` → `app-ui/00-shared/components/**/02-filter-chip-row/` + `frontend/lib/shared/components/`
- `01-hotspot-map` → `app-ui/00-shared/components/**/01-hotspot-map/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_13_map_explorer.dart` |
...
| Tests | `frontend/test/shared/layout/app_13_map_explorer_test.dart` |
...
1. Confirm whether `13-map-explorer` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `13-map-explorer`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "13-map-explorer"` can mount without forking chrome
