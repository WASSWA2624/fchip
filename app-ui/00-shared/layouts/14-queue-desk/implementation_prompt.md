# Implementation prompt — shared layout `14-queue-desk`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Queue desk** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `14-queue-desk` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Kind | Shared layout |
| Slug | `14-queue-desk` |
| Suggested Dart name | `App14QueueDesk` |
| Surfaces | `07`, `16` |
...
Folder: `app-ui/00-shared/layouts/14-queue-desk/`
...
- `02-queue-item` → `app-ui/00-shared/components/**/02-queue-item/` + `frontend/lib/shared/components/`
- `01-primary-cta` → `app-ui/00-shared/components/**/01-primary-cta/` + `frontend/lib/shared/components/`
- `02-sync-status-strip` → `app-ui/00-shared/components/**/02-sync-status-strip/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_14_queue_desk.dart` |
...
| Tests | `frontend/test/shared/layout/app_14_queue_desk_test.dart` |
...
1. Confirm whether `14-queue-desk` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `14-queue-desk`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "14-queue-desk"` can mount without forking chrome
