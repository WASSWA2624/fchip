# Implementation prompt — shared layout `15-feeder-home`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Feeder home** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `15-feeder-home` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Kind | Shared layout |
| Slug | `15-feeder-home` |
| Suggested Dart name | `App15FeederHome` |
| Surfaces | `14–22` |
...
Folder: `app-ui/00-shared/layouts/15-feeder-home/`
...
- `03-offline-ready-chip` → `app-ui/00-shared/components/**/03-offline-ready-chip/` + `frontend/lib/shared/components/`
- `01-primary-cta` → `app-ui/00-shared/components/**/01-primary-cta/` + `frontend/lib/shared/components/`
- `04-bottom-nav-feeder` → `app-ui/00-shared/components/**/04-bottom-nav-feeder/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_15_feeder_home.dart` |
...
| Tests | `frontend/test/shared/layout/app_15_feeder_home_test.dart` |
...
1. Confirm whether `15-feeder-home` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `15-feeder-home`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "15-feeder-home"` can mount without forking chrome
