# Implementation prompt — shared layout `15-upload-batch`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Upload / batch** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `15-upload-batch` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `15-upload-batch` |
| Suggested Dart name | `App15UploadBatch` |
...
Folder: `app-ui/00-shared/layouts/15-upload-batch/`
...
- `05-file-upload-field` → `app-ui/00-shared/components/**/05-file-upload-field/` + `frontend/lib/shared/components/`
- `05-error-inline` → `app-ui/00-shared/components/**/05-error-inline/` + `frontend/lib/shared/components/`
- `03-success-toast` → `app-ui/00-shared/components/**/03-success-toast/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_15_upload_batch.dart` |
...
| Tests | `frontend/test/shared/layout/app_15_upload_batch_test.dart` |
...
1. Confirm whether `15-upload-batch` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `15-upload-batch`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "15-upload-batch"` can mount without forking chrome
