# Implementation prompt — shared layout `17-insurance-prevention`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Insurance prevention** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `17-insurance-prevention` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: §7 prevention population insights — not claims / not CHIS identity_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `17-insurance-prevention` |
| Suggested Dart name | `App17InsurancePrevention` |
...
Folder: `app-ui/00-shared/layouts/17-insurance-prevention/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_17_insurance_prevention.dart` |
...
| Tests | `frontend/test/shared/layout/app_17_insurance_prevention_test.dart` |
...
1. Confirm whether `17-insurance-prevention` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `17-insurance-prevention`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "17-insurance-prevention"` can mount without forking chrome
