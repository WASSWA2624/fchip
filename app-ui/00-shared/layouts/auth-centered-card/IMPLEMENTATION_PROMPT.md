# Implementation prompt — shared layout `auth-centered-card`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Auth centered card** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `auth-centered-card` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `auth-centered-card` |
| Suggested Dart name | `AppAuthCenteredCard` |
| Surfaces | `00-shared` |
...
Folder: `app-ui/00-shared/layouts/auth-centered-card/`
...
- `logo-lockup` → `app-ui/00-shared/components/**/logo-lockup/` + `frontend/lib/shared/components/`
- `slogan-line` → `app-ui/00-shared/components/**/slogan-line/` + `frontend/lib/shared/components/`
- `labeled-field` → `app-ui/00-shared/components/**/labeled-field/` + `frontend/lib/shared/components/`
- `primary-cta` → `app-ui/00-shared/components/**/primary-cta/` + `frontend/lib/shared/components/`
- `master-loop-badge` → `app-ui/00-shared/components/**/master-loop-badge/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_auth_centered_card.dart` |
...
| Tests | `frontend/test/shared/layout/app_auth_centered_card_test.dart` |
...
1. Confirm whether `auth-centered-card` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `auth-centered-card`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "auth-centered-card"` can mount without forking chrome
