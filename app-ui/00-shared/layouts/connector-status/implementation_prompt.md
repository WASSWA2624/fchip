# Implementation prompt — shared layout `connector-status`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Connector status** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `connector-status` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Suggested Dart name | `AppConnectorStatus` |
...
Folder: `app-ui/00-shared/layouts/connector-status/`
...
- `feeder-health-pill` → `app-ui/00-shared/components/**/feeder-health-pill/` + `frontend/lib/shared/components/`
- `warn-banner` → `app-ui/00-shared/components/**/warn-banner/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_connector_status.dart` |
...
| Tests | `frontend/test/shared/layout/app_connector_status_test.dart` |
...
1. Confirm whether `connector-status` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `connector-status`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "connector-status"` can mount without forking chrome
