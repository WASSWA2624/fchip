# Implementation prompt — shared layout `desktop-sidebar-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Desktop sidebar shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `desktop-sidebar-shell` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `desktop-sidebar-shell` |
| Suggested Dart name | `AppDesktopSidebarShell` |
...
Folder: `app-ui/00-shared/layouts/desktop-sidebar-shell/`
...
- `side-nav-desktop` → `app-ui/00-shared/components/**/side-nav-desktop/` + `frontend/lib/shared/components/`
- `top-app-bar` → `app-ui/00-shared/components/**/top-app-bar/` + `frontend/lib/shared/components/`
- `cascade-footer` → `app-ui/00-shared/components/**/cascade-footer/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_desktop_sidebar_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_desktop_sidebar_shell_test.dart` |
...
1. Confirm whether `desktop-sidebar-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `desktop-sidebar-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "desktop-sidebar-shell"` can mount without forking chrome
