# Implementation prompt — shared layout `detail-action`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Detail + action** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `detail-action` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Kind | Shared layout |
| Slug | `detail-action` |
| Suggested Dart name | `AppDetailAction` |
...
Folder: `app-ui/00-shared/layouts/detail-action/`
...
- `explainability-block` → `app-ui/00-shared/components/**/explainability-block/` + `frontend/lib/shared/components/`
...
- `danger-cta` → `app-ui/00-shared/components/**/danger-cta/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_detail_action.dart` |
...
| Tests | `frontend/test/shared/layout/app_detail_action_test.dart` |
...
1. Confirm whether `detail-action` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `detail-action`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "detail-action"` can mount without forking chrome
