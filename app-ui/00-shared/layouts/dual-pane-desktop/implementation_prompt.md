# Implementation prompt — shared layout `dual-pane-desktop`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Dual-pane desktop** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `dual-pane-desktop` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| --- | --- |
| Kind | Shared layout |
| Slug | `dual-pane-desktop` |
| Suggested Dart name | `AppDualPaneDesktop` |
...
Folder: `app-ui/00-shared/layouts/dual-pane-desktop/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_dual_pane_desktop.dart` |
...
| Tests | `frontend/test/shared/layout/app_dual_pane_desktop_test.dart` |
...
1. Confirm whether `dual-pane-desktop` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `dual-pane-desktop`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "dual-pane-desktop"` can mount without forking chrome
