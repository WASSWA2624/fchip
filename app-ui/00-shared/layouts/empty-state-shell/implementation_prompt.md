# Implementation prompt — shared layout `empty-state-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Empty state shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `empty-state-shell` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: Happy-path layout with empty body (worklist / queue)_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `empty-state-shell` |
| Suggested Dart name | `AppEmptyStateShell` |
| Surfaces | `01 worklist-empty`, `07 referral-queue-empty` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/empty-state-shell/`
...
- `empty-state` → `app-ui/00-shared/components/**/empty-state/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_empty_state_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_empty_state_shell_test.dart` |
...
1. Confirm whether `empty-state-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `empty-state-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "empty-state-shell"` can mount without forking chrome
