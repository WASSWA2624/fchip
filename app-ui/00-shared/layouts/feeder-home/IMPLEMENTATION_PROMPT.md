# Implementation prompt — shared layout `feeder-home`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Feeder home** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `feeder-home` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Kind | Shared layout |
| Slug | `feeder-home` |
| Suggested Dart name | `AppFeederHome` |
| Surfaces | `14–22` |
...
Folder: `app-ui/00-shared/layouts/feeder-home/`
...
- `offline-ready-chip` → `app-ui/00-shared/components/**/offline-ready-chip/` + `frontend/lib/shared/components/`
- `primary-cta` → `app-ui/00-shared/components/**/primary-cta/` + `frontend/lib/shared/components/`
- `bottom-nav-feeder` → `app-ui/00-shared/components/**/bottom-nav-feeder/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_feeder_home.dart` |
...
| Tests | `frontend/test/shared/layout/app_feeder_home_test.dart` |
...
1. Confirm whether `feeder-home` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `feeder-home`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "feeder-home"` can mount without forking chrome
