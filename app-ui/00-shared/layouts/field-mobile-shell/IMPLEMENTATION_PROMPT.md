# Implementation prompt — shared layout `field-mobile-shell`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Field mobile shell** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `field-mobile-shell` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: CHW / caregiver / feeder capture on phone_
...
| --- | --- |
| Kind | Shared layout |
| Slug | `field-mobile-shell` |
| Suggested Dart name | `AppFieldMobileShell` |
| Surfaces | `01`, `02`, `14–19`, `21` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/field-mobile-shell/`
...
- `top-app-bar` → `app-ui/00-shared/components/**/top-app-bar/` + `frontend/lib/shared/components/`
- `section-header` → `app-ui/00-shared/components/**/section-header/` + `frontend/lib/shared/components/`
- `bottom-nav-field` → `app-ui/00-shared/components/**/bottom-nav-field/` + `frontend/lib/shared/components/`
- `status-chip` → `app-ui/00-shared/components/**/status-chip/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_field_mobile_shell.dart` |
...
| Tests | `frontend/test/shared/layout/app_field_mobile_shell_test.dart` |
...
1. Confirm whether `field-mobile-shell` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `field-mobile-shell`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "field-mobile-shell"` can mount without forking chrome
