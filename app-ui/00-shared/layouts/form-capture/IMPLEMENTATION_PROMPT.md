# Implementation prompt — shared layout `form-capture`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Form capture** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `form-capture` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: Structured offline / online data entry_
...
| Kind | Shared layout |
| Slug | `form-capture` |
| Suggested Dart name | `AppFormCapture` |
| Surfaces | `visits`, `referrals`, `all feeder entry` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/form-capture/`
...
- `form-stack` → `app-ui/00-shared/components/**/form-stack/` + `frontend/lib/shared/components/`
- `consent-toggle` → `app-ui/00-shared/components/**/consent-toggle/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_form_capture.dart` |
...
| Tests | `frontend/test/shared/layout/app_form_capture_test.dart` |
...
1. Confirm whether `form-capture` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `form-capture`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "form-capture"` can mount without forking chrome
