# Implementation prompt — shared layout `insurance-prevention`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Insurance prevention** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `insurance-prevention` in `screen.json` and plug content slots instead of rebuilding chrome.
...
Folder: `app-ui/00-shared/layouts/insurance-prevention/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_insurance_prevention.dart` |
...
| Tests | `frontend/test/shared/layout/app_insurance_prevention_test.dart` |
...
1. Confirm whether `insurance-prevention` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `insurance-prevention`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "insurance-prevention"` can mount without forking chrome
