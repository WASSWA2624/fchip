# Implementation prompt — shared layout `map-explorer`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Map explorer** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `map-explorer` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Kind | Shared layout |
| Slug | `map-explorer` |
| Suggested Dart name | `AppMapExplorer` |
...
Folder: `app-ui/00-shared/layouts/map-explorer/`
...
- `hotspot-map` → `app-ui/00-shared/components/**/hotspot-map/` + `frontend/lib/shared/components/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_map_explorer.dart` |
...
| Tests | `frontend/test/shared/layout/app_map_explorer_test.dart` |
...
1. Confirm whether `map-explorer` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `map-explorer`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "map-explorer"` can mount without forking chrome
