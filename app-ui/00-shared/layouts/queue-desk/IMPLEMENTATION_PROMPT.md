# Implementation prompt — shared layout `queue-desk`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Queue desk** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `queue-desk` in `screen.json` and plug content slots instead of rebuilding chrome.
...
| Kind | Shared layout |
| Slug | `queue-desk` |
| Suggested Dart name | `AppQueueDesk` |
| Surfaces | `07`, `16` |
...
Folder: `app-ui/00-shared/layouts/queue-desk/`
...
- `queue-item` → `app-ui/00-shared/components/**/queue-item/` + `frontend/lib/shared/components/`
- `primary-cta` → `app-ui/00-shared/components/**/primary-cta/` + `frontend/lib/shared/components/`
- `sync-status-strip` → `app-ui/00-shared/components/**/sync-status-strip/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_queue_desk.dart` |
...
| Tests | `frontend/test/shared/layout/app_queue_desk_test.dart` |
...
1. Confirm whether `queue-desk` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `queue-desk`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "queue-desk"` can mount without forking chrome
