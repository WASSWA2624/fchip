# Implementation prompt — shared layout `settings-admin`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Settings / admin** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `settings-admin` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: Org · roles · consent · API scopes · feeder registry_
...
| Kind | Shared layout |
| Slug | `settings-admin` |
| Suggested Dart name | `AppSettingsAdmin` |
| Surfaces | `13`, `08 scopes`, `20 mapping`, `22 config` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/settings-admin/`
...
| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_settings_admin.dart` |
...
| Tests | `frontend/test/shared/layout/app_settings_admin_test.dart` |
...
1. Confirm whether `settings-admin` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `settings-admin`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "settings-admin"` can mount without forking chrome
