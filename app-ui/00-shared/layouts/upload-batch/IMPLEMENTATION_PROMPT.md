# Implementation prompt — shared layout `upload-batch`

> Paste this whole file into an agent session when implementing this reusable page layout.

## Mission

Implement **Upload / batch** as a reusable responsive layout shell in `frontend/lib/shared/layout`, composed from shared components, so product screens declare `upload-batch` in `screen.json` and plug content slots instead of rebuilding chrome.

_Purpose: File upload + validation + push to ingest_
...
| Kind | Shared layout |
| Slug | `upload-batch` |
| Suggested Dart name | `AppUploadBatch` |
| Surfaces | `NGO upload`, `lab batch`, `outreach batch`, `HMIS exchange` |
| Breakpoints | `mobile`, `tablet`, `desktop` |
| Themes | `light`, `dark`, `system` |

## Visual sources

Folder: `app-ui/00-shared/layouts/upload-batch/`
...
- `file-upload-field` → `app-ui/00-shared/components/**/file-upload-field/` + `frontend/lib/shared/components/`
- `error-inline` → `app-ui/00-shared/components/**/error-inline/` + `frontend/lib/shared/components/`
- `success-toast` → `app-ui/00-shared/components/**/success-toast/` + `frontend/lib/shared/components/`
- `primary-cta` → `app-ui/00-shared/components/**/primary-cta/` + `frontend/lib/shared/components/`
- `note-banner` → `app-ui/00-shared/components/**/note-banner/` + `frontend/lib/shared/components/`

## Target code locations

| Layer | Path |
| --- | --- |
| Layout widget | `frontend/lib/shared/layout/app_upload_batch.dart` |
...
| Tests | `frontend/test/shared/layout/app_upload_batch_test.dart` |
...
1. Confirm whether `upload-batch` already maps to an existing shared layout (`ResponsiveAppShell`, workspace layouts, auth shell). Prefer adapting the existing shell over a parallel tree.
...
Layouts are structural. Backend pairing belongs to each **screen** that uses `upload-batch`. Ensure the layout supports the states those screens declare (`loading`, `empty`, `error`, `forbidden`, `offline`, `conflict`) via slots or `AsyncStateScaffold`, so backend-driven states render consistently.
...
- Goldens/tests pass
- `app-ui` screens that declare `layout: "upload-batch"` can mount without forking chrome
