# Implementation prompt — `00-shared/login`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Sign in** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Phone number + password only_
...
| Localization prefix | `00_shared.login` |
...
| Page | `frontend/lib/features/shared/presentation/pages/login_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/login`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-003`:

### Phase 0 — Claim the screen

1. Confirm `S-003` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Sign in → `/00-shared/role-surface-picker`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/login` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.login` only — no hard-coded user strings.
...
23. Mark `S-003` `status: done`, `backend: paired` (or `none` with static evidence only).
