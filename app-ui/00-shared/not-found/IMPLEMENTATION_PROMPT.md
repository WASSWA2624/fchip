# Implementation prompt — `00-shared/not-found`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Page not found** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: This link is unavailable or has moved_
...
| Kind | Screen (`empty`) |
| Module | `00-shared` |
| Screen slug | `not-found` |
| Chronology | `S-011` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/not-found` |
...
| Localization prefix | `00_shared.not_found` |
...
Folder: `app-ui/00-shared/not-found/`
...
| Page | `frontend/lib/features/shared/presentation/pages/not_found_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/not-found`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-011`:

### Phase 0 — Claim the screen

1. Confirm `S-011` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/00-shared/not-found` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.not_found` only — no hard-coded user strings.
...
23. Mark `S-011` `status: done`, `backend: paired` (or `none` with static evidence only).
