# Implementation prompt — `00-shared/preferences`
...
| Module | `00-shared` |
| Screen slug | `preferences` |
| Chronology | `S-012` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/preferences` |
| Parent route | `/00-shared/preferences` |
...
| Localization prefix | `00_shared.preferences` |
...
Folder: `app-ui/00-shared/preferences/`
...
| Page | `frontend/lib/features/shared/presentation/pages/preferences_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/preferences`) |
...
7. Map navigation:
   - Tabs: Workspaces → `/00-shared/role-surface-picker`; Notifications → `/00-shared/notifications-center`; Preferences → `/00-shared/preferences`; Sign out → `/00-shared/login`
   - Primary action: Save preferences → `/00-shared/preferences?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/preferences` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.preferences` only — no hard-coded user strings.
