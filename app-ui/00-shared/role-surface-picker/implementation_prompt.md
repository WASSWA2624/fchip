# Implementation prompt — `00-shared/role-surface-picker`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Choose your workspace** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Consumers · data feeders · admin_
...
| Kind | Screen (`list`) |
| Module | `00-shared` |
| Screen slug | `role-surface-picker` |
| Chronology | `S-008` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/role-surface-picker` |
...
| Localization prefix | `00_shared.role_surface_picker` |
...
Folder: `app-ui/00-shared/role-surface-picker/`
...
| Page | `frontend/lib/features/shared/presentation/pages/role_surface_picker_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/role-surface-picker`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-008`:

### Phase 0 — Claim the screen

1. Confirm `S-008` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/00-shared/role-surface-picker` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.role_surface_picker` only — no hard-coded user strings.
...
23. Mark `S-008` `status: done`, `backend: paired` (or `none` with static evidence only).
