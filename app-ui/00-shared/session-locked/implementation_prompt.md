# Implementation prompt — `00-shared/session-locked`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Session locked** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Idle timeout · sign in with phone + password_
...
| Kind | Screen (`auth`) |
| Module | `00-shared` |
| Screen slug | `session-locked` |
| Chronology | `S-007` |
| Slice | `VS-00` |
| Phase | `shared` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/00-shared/session-locked` |
...
| Localization prefix | `00_shared.session_locked` |
...
Folder: `app-ui/00-shared/session-locked/`
...
| Page | `frontend/lib/features/shared/presentation/pages/session_locked_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/00-shared/session-locked`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-007`:

### Phase 0 — Claim the screen

1. Confirm `S-007` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: (none)
   - Primary action: Sign in again → `/00-shared/role-surface-picker`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/00-shared/session-locked` with the access guard for roles `authenticated-user`.
...
11. Add ARB keys under `00_shared.session_locked` only — no hard-coded user strings.
...
23. Mark `S-007` `status: done`, `backend: paired` (or `none` with static evidence only).
