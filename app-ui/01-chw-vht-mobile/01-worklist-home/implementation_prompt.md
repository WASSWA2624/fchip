# Implementation prompt — `01-chw-vht-mobile/01-worklist-home`
...
| Screen slug | `01-worklist-home` |
| Chronology | `S-013` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/01-worklist-home` |
...
| Localization prefix | `01_chw_vht_mobile.01_worklist_home` |
...
Folder: `app-ui/01-chw-vht-mobile/01-worklist-home/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/01_worklist_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/01-worklist-home`) |
...
7. Map navigation:
   ...
   - Primary action: Start next visit → `/01-chw-vht-mobile/04-household-visit-form`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/01-worklist-home` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.01_worklist_home` only — no hard-coded user strings.
