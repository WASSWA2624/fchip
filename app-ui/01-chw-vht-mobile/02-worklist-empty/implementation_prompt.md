# Implementation prompt — `01-chw-vht-mobile/02-worklist-empty`
...
| Screen slug | `02-worklist-empty` |
| Chronology | `S-014` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/02-worklist-empty` |
...
| Localization prefix | `01_chw_vht_mobile.02_worklist_empty` |
...
Folder: `app-ui/01-chw-vht-mobile/02-worklist-empty/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/02_worklist_empty_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/02-worklist-empty`) |
...
7. Map navigation:
   ...
   - Primary action: Pull latest when online → `/01-chw-vht-mobile/01-worklist-home`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/02-worklist-empty` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.02_worklist_empty` only — no hard-coded user strings.
