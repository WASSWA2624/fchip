# Implementation prompt — `01-chw-vht-mobile/05-symptoms-vitals`
...
| Screen slug | `05-symptoms-vitals` |
| Chronology | `S-017` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/05-symptoms-vitals` |
...
| Localization prefix | `01_chw_vht_mobile.05_symptoms_vitals` |
...
Folder: `app-ui/01-chw-vht-mobile/05-symptoms-vitals/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/05_symptoms_vitals_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/05-symptoms-vitals`) |
...
7. Map navigation:
   ...
   - Primary action: Continue to MCH → `/01-chw-vht-mobile/06-maternal-child-indicators`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/05-symptoms-vitals` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.05_symptoms_vitals` only — no hard-coded user strings.
