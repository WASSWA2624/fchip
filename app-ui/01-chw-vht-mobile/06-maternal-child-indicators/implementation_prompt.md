# Implementation prompt — `01-chw-vht-mobile/06-maternal-child-indicators`
...
| Screen slug | `06-maternal-child-indicators` |
| Chronology | `S-018` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/06-maternal-child-indicators` |
...
| Localization prefix | `01_chw_vht_mobile.06_maternal_child_indicators` |
...
Folder: `app-ui/01-chw-vht-mobile/06-maternal-child-indicators/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/06_maternal_child_indicators_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/06-maternal-child-indicators`) |
...
7. Map navigation:
   ...
   - Primary action: Save indicators → `/01-chw-vht-mobile/07-visit-saved`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/06-maternal-child-indicators` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.06_maternal_child_indicators` only — no hard-coded user strings.
