# Implementation prompt — `01-chw-vht-mobile/07-visit-saved`
...
| Screen slug | `07-visit-saved` |
| Chronology | `S-019` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/07-visit-saved` |
...
| Localization prefix | `01_chw_vht_mobile.07_visit_saved` |
...
Folder: `app-ui/01-chw-vht-mobile/07-visit-saved/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/07_visit_saved_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/07-visit-saved`) |
...
7. Map navigation:
   ...
   - Primary action: Create referral → `/01-chw-vht-mobile/08-create-referral`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/07-visit-saved` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.07_visit_saved` only — no hard-coded user strings.
