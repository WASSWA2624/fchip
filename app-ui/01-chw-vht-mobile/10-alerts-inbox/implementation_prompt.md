# Implementation prompt — `01-chw-vht-mobile/10-alerts-inbox`
...
| Screen slug | `10-alerts-inbox` |
| Chronology | `S-022` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/10-alerts-inbox` |
| Parent route | `/01-chw-vht-mobile/10-alerts-inbox` |
...
| Localization prefix | `01_chw_vht_mobile.10_alerts_inbox` |
...
Folder: `app-ui/01-chw-vht-mobile/10-alerts-inbox/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/10_alerts_inbox_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/10-alerts-inbox`) |
...
7. Map navigation:
   ...
   - Primary action: Open top alert → `/01-chw-vht-mobile/11-alert-follow-up`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/10-alerts-inbox` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.10_alerts_inbox` only — no hard-coded user strings.
