# Implementation prompt — `01-chw-vht-mobile/11-alert-follow-up`
...
| Screen slug | `11-alert-follow-up` |
| Chronology | `S-023` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/11-alert-follow-up` |
...
| Localization prefix | `01_chw_vht_mobile.11_alert_follow_up` |
...
Folder: `app-ui/01-chw-vht-mobile/11-alert-follow-up/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/11_alert_follow_up_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/11-alert-follow-up`) |
...
7. Map navigation:
   ...
   - Primary action: Mark follow-up done → `/01-chw-vht-mobile/01-worklist-home`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/11-alert-follow-up` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.11_alert_follow_up` only — no hard-coded user strings.
