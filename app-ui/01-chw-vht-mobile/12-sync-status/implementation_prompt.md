# Implementation prompt — `01-chw-vht-mobile/12-sync-status`
...
| Screen slug | `12-sync-status` |
| Chronology | `S-024` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/12-sync-status` |
| Parent route | `/01-chw-vht-mobile/12-sync-status` |
...
| Localization prefix | `01_chw_vht_mobile.12_sync_status` |
...
Folder: `app-ui/01-chw-vht-mobile/12-sync-status/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/12_sync_status_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/12-sync-status`) |
...
7. Map navigation:
   ...
   - Primary action: Sync now → `/01-chw-vht-mobile/12-sync-status`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/12-sync-status` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.12_sync_status` only — no hard-coded user strings.
