# Implementation prompt — `01-chw-vht-mobile/13-sync-failed`
...
| Screen slug | `13-sync-failed` |
| Chronology | `S-025` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/13-sync-failed` |
...
| Localization prefix | `01_chw_vht_mobile.13_sync_failed` |
...
Folder: `app-ui/01-chw-vht-mobile/13-sync-failed/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/13_sync_failed_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/13-sync-failed`) |
...
7. Map navigation:
   ...
   - Primary action: Retry now → `/01-chw-vht-mobile/12-sync-status`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/13-sync-failed` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.13_sync_failed` only — no hard-coded user strings.
