# Implementation prompt — `01-chw-vht-mobile/14-sync-conflict`
...
| Screen slug | `14-sync-conflict` |
| Chronology | `S-026` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/14-sync-conflict` |
...
| Localization prefix | `01_chw_vht_mobile.14_sync_conflict` |
...
Folder: `app-ui/01-chw-vht-mobile/14-sync-conflict/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/14_sync_conflict_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/14-sync-conflict`) |
...
7. Map navigation:
   ...
   - Primary action: Review versions → `/01-chw-vht-mobile/01-worklist-home`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/14-sync-conflict` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.14_sync_conflict` only — no hard-coded user strings.
