# Implementation prompt — `01-chw-vht-mobile/alerts-inbox`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Alerts inbox** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Risk flags from intelligence core_
...
| Screen slug | `alerts-inbox` |
| Chronology | `S-022` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/alerts-inbox` |
| Parent route | `/01-chw-vht-mobile/alerts-inbox` |
...
| Localization prefix | `01_chw_vht_mobile.alerts_inbox` |
...
Folder: `app-ui/01-chw-vht-mobile/alerts-inbox/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/alerts_inbox_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/alerts-inbox`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-022`:

### Phase 0 — Claim the screen

1. Confirm `S-022` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Open top alert → `/01-chw-vht-mobile/alert-follow-up`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/alerts-inbox` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.alerts_inbox` only — no hard-coded user strings.
...
23. Mark `S-022` `status: done`, `backend: paired` (or `none` with static evidence only).
