# Implementation prompt — `01-chw-vht-mobile/create-referral`
...
| Screen slug | `create-referral` |
| Chronology | `S-020` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/create-referral` |
...
| Localization prefix | `01_chw_vht_mobile.create_referral` |
...
Folder: `app-ui/01-chw-vht-mobile/create-referral/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/create_referral_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/create-referral`) |
...
7. Map navigation:
   ...
   - Primary action: Submit referral → `/01-chw-vht-mobile/referral-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/create-referral` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.create_referral` only — no hard-coded user strings.
