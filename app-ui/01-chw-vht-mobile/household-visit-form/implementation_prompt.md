# Implementation prompt — `01-chw-vht-mobile/household-visit-form`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Household visit** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Offline structured capture_
...
| Screen slug | `household-visit-form` |
| Chronology | `S-016` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/household-visit-form` |
...
| Localization prefix | `01_chw_vht_mobile.household_visit_form` |
...
Folder: `app-ui/01-chw-vht-mobile/household-visit-form/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/household_visit_form_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/household-visit-form`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-016`:

### Phase 0 — Claim the screen

1. Confirm `S-016` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save locally → `/01-chw-vht-mobile/symptoms-vitals`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/household-visit-form` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.household_visit_form` only — no hard-coded user strings.
...
23. Mark `S-016` `status: done`, `backend: paired` (or `none` with static evidence only).
