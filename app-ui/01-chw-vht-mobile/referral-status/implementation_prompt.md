# Implementation prompt — `01-chw-vht-mobile/referral-status`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Referral sent** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Track the household through facility care_
...
| Screen slug | `referral-status` |
| Chronology | `S-021` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/referral-status` |
| Parent route | `/01-chw-vht-mobile/worklist-home` |
...
| Localization prefix | `01_chw_vht_mobile.referral_status` |
...
Folder: `app-ui/01-chw-vht-mobile/referral-status/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/referral_status_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/referral-status`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-021`:

### Phase 0 — Claim the screen

1. Confirm `S-021` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Worklist → `/01-chw-vht-mobile/worklist-home`; Alerts → `/01-chw-vht-mobile/alerts-inbox`; Sync → `/01-chw-vht-mobile/sync-status`; More → `/00-shared/notifications-center`
   - Primary action: Return to worklist → `/01-chw-vht-mobile/worklist-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/referral-status` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.referral_status` only — no hard-coded user strings.
...
23. Mark `S-021` `status: done`, `backend: paired` (or `none` with static evidence only).
