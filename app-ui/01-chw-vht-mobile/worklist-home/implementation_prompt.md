# Implementation prompt — `01-chw-vht-mobile/worklist-home`
...
_Subtitle: Visits · follow-ups · alert tasks_
...
| Screen slug | `worklist-home` |
| Chronology | `S-013` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/worklist-home` |
...
| Localization prefix | `01_chw_vht_mobile.worklist_home` |
...
Folder: `app-ui/01-chw-vht-mobile/worklist-home/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/worklist_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/worklist-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-013`:

### Phase 0 — Claim the screen

1. Confirm `S-013` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Start next visit → `/01-chw-vht-mobile/household-visit-form`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/01-chw-vht-mobile/worklist-home` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.worklist_home` only — no hard-coded user strings.
...
23. Mark `S-013` `status: done`, `backend: paired` (or `none` with static evidence only).
