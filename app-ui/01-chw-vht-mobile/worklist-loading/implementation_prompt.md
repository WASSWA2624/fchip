# Implementation prompt — `01-chw-vht-mobile/worklist-loading`
...
_Subtitle: Loading visits and alert tasks_
...
| Screen slug | `worklist-loading` |
| Chronology | `S-015` |
| Slice | `VS-01` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/01-chw-vht-mobile/worklist-loading` |
...
| Localization prefix | `01_chw_vht_mobile.worklist_loading` |
...
Folder: `app-ui/01-chw-vht-mobile/worklist-loading/`
...
| Page | `frontend/lib/features/chw_vht_mobile/presentation/pages/worklist_loading_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/01-chw-vht-mobile/worklist-loading`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-015`:

### Phase 0 — Claim the screen

1. Confirm `S-015` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/01-chw-vht-mobile/worklist-loading` with the access guard for roles `chw`, `vht`.
...
11. Add ARB keys under `01_chw_vht_mobile.worklist_loading` only — no hard-coded user strings.
...
23. Mark `S-015` `status: done`, `backend: paired` (or `none` with static evidence only).
