# Implementation prompt — `02-community-caregiver/guidance-hints`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Guidance** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Care tips · appointment hints — not booking/EMR_
...
| Screen slug | `guidance-hints` |
| Chronology | `S-063` |
| Slice | `VS-02` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/02-community-caregiver/guidance-hints` |
| Parent route | `/02-community-caregiver/guidance-hints` |
...
| Localization prefix | `02_community_caregiver.guidance_hints` |
...
Folder: `app-ui/02-community-caregiver/guidance-hints/`
...
| Page | `frontend/lib/features/community_caregiver/presentation/pages/guidance_hints_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-community-caregiver/guidance-hints`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-063`:

### Phase 0 — Claim the screen

1. Confirm `S-063` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/02-community-caregiver/guidance-hints` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `02_community_caregiver.guidance_hints` only — no hard-coded user strings.
...
23. Mark `S-063` `status: done`, `backend: paired` (or `none` with static evidence only).
