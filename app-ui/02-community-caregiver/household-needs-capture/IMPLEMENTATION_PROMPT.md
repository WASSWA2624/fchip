# Implementation prompt — `02-community-caregiver/household-needs-capture`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Household needs** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Community members feed priorities into cascade_
...
| --- | --- |
| Kind | Screen (`form`) |
| Module | `02-community-caregiver` |
| Screen slug | `household-needs-capture` |
| Chronology | `S-064` |
| Slice | `VS-02` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/02-community-caregiver/household-needs-capture` |
| Parent route | `/02-community-caregiver/household-needs-capture` |
| Layout | `form-capture` |
...
| Roles | `caregiver`, `community-member` |
| Supported states | `default`, `loading`, `error`, `success`, `offline`, `forbidden` |
| Localization prefix | `02_community_caregiver.household_needs_capture` |
...
Folder: `app-ui/02-community-caregiver/household-needs-capture/`
...
- `app-ui/02-community-caregiver/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/community_caregiver/` |
| Page | `frontend/lib/features/community_caregiver/presentation/pages/household_needs_capture_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/community_caregiver/presentation/widgets/` |
| Presentation state | `frontend/lib/features/community_caregiver/presentation/state/` |
| Domain | `frontend/lib/features/community_caregiver/domain/` |
| Data / repository impl | `frontend/lib/features/community_caregiver/data/` |
| Repository contract | `frontend/lib/features/community_caregiver/domain/repositories/community_caregiver_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/02-community-caregiver/household-needs-capture`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `form-capture`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/community_caregiver/` |
| Backend module | `backend/src/modules/community_caregiver/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-064`:

### Phase 0 — Claim the screen

1. Confirm `S-064` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/02-community-caregiver/my-household`; Report → `/02-community-caregiver/self-report`; Guidance → `/02-community-caregiver/guidance-hints`; More → `/02-community-caregiver/household-needs-capture`
   - Primary action: Submit needs signal → `/02-community-caregiver/household-needs-capture?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/02-community-caregiver/household-needs-capture` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `02_community_caregiver.household_needs_capture` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/community_caregiver/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/community_caregiver/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-064` `status: done`, `backend: paired` (or `none` with static evidence only).
