# Implementation prompt — `02-community-caregiver/self-report`
...
| Route | `/02-community-caregiver/self-report` |
| Parent route | `/02-community-caregiver/self-report` |
| Layout | `form-capture` |
...
| Localization prefix | `02_community_caregiver.self_report` |
...
Folder: `app-ui/02-community-caregiver/self-report/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-community-caregiver/self-report`) |
...
7. Map navigation:
   ...
   - Primary action: Send to CHW worklist → `/02-community-caregiver/my-household?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/02-community-caregiver/self-report` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `02_community_caregiver.self_report` only — no hard-coded user strings.
