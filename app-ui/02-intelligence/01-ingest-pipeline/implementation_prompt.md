# Implementation prompt — `02-intelligence/01-ingest-pipeline`
...
| Screen slug | `01-ingest-pipeline` |
| Chronology | `S-027` |
| Slice | `VS-02` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/02-intelligence/01-ingest-pipeline` |
...
| Localization prefix | `02_intelligence.01_ingest_pipeline` |
...
Folder: `app-ui/02-intelligence/01-ingest-pipeline/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/01_ingest_pipeline_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/01-ingest-pipeline`) |
...
8. Register route `/02-intelligence/01-ingest-pipeline` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.01_ingest_pipeline` only — no hard-coded user strings.
