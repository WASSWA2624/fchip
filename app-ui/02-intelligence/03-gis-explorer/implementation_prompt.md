# Implementation prompt — `02-intelligence/03-gis-explorer`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **GIS maps** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Disease distribution · hotspots · gaps_
...
| Screen slug | `03-gis-explorer` |
| Chronology | `S-052` |
| Slice | `VS-02` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/02-intelligence/03-gis-explorer` |
| Parent route | `/02-intelligence/03-gis-explorer` |
...
| Localization prefix | `02_intelligence.03_gis_explorer` |
...
Folder: `app-ui/02-intelligence/03-gis-explorer/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/03_gis_explorer_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/03-gis-explorer`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-052`:

### Phase 0 — Claim the screen

1. Confirm `S-052` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/02-intelligence/03-gis-explorer` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.03_gis_explorer` only — no hard-coded user strings.
...
23. Mark `S-052` `status: done`, `backend: paired` (or `none` with static evidence only).
