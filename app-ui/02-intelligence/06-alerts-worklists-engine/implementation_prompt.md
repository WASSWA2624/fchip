# Implementation prompt — `02-intelligence/06-alerts-worklists-engine`
...
| Screen slug | `06-alerts-worklists-engine` |
| Chronology | `S-055` |
| Slice | `VS-02` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/02-intelligence/06-alerts-worklists-engine` |
...
| Localization prefix | `02_intelligence.06_alerts_worklists_engine` |
...
Folder: `app-ui/02-intelligence/06-alerts-worklists-engine/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/06_alerts_worklists_engine_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/06-alerts-worklists-engine`) |
...
7. Map navigation:
   ...
   - Primary action: Save routing → `/02-intelligence/06-alerts-worklists-engine?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/02-intelligence/06-alerts-worklists-engine` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.06_alerts_worklists_engine` only — no hard-coded user strings.
