# Implementation prompt — `02-intelligence/alerts-worklists-engine`
...
| Route | `/02-intelligence/alerts-worklists-engine` |
| Parent route | `/02-intelligence/ingest-pipeline` |
...
| Localization prefix | `02_intelligence.alerts_worklists_engine` |
...
Folder: `app-ui/02-intelligence/alerts-worklists-engine/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/alerts-worklists-engine`) |
...
7. Map navigation:
   ...
   - Primary action: Save routing → `/02-intelligence/alerts-worklists-engine?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/02-intelligence/alerts-worklists-engine` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.alerts_worklists_engine` only — no hard-coded user strings.
