# Implementation prompt — `02-intelligence/clinical-support-guidance`
...
| Route | `/02-intelligence/clinical-support-guidance` |
| Parent route | `/02-intelligence/clinical-support-guidance` |
...
| Localization prefix | `02_intelligence.clinical_support_guidance` |
...
Folder: `app-ui/02-intelligence/clinical-support-guidance/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/clinical-support-guidance`) |
...
7. Map navigation:
   - Tabs: Ingest → `/02-intelligence/ingest-pipeline`; AI → `/02-intelligence/ai-risk-scores`; GIS → `/02-intelligence/gis-explorer`; Guidance → `/02-intelligence/clinical-support-guidance`
   - Primary action: Push to CHW worklist → `/02-intelligence/clinical-support-guidance`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/02-intelligence/clinical-support-guidance` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.clinical_support_guidance` only — no hard-coded user strings.
