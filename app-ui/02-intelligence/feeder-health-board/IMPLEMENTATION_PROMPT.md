# Implementation prompt — `02-intelligence/feeder-health-board`
...
| Route | `/02-intelligence/feeder-health-board` |
...
| Localization prefix | `02_intelligence.feeder_health_board` |
...
Folder: `app-ui/02-intelligence/feeder-health-board/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/feeder-health-board`) |
...
7. Map navigation:
   ...
   - Primary action: Open silent feeders → `/02-intelligence/ingest-pipeline`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/02-intelligence/feeder-health-board` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.feeder_health_board` only — no hard-coded user strings.
