# Implementation prompt — `02-intelligence/ingest-pipeline`
...
| Route | `/02-intelligence/ingest-pipeline` |
...
| Localization prefix | `02_intelligence.ingest_pipeline` |
...
Folder: `app-ui/02-intelligence/ingest-pipeline/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/02-intelligence/ingest-pipeline`) |
...
8. Register route `/02-intelligence/ingest-pipeline` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `02_intelligence.ingest_pipeline` only — no hard-coded user strings.
