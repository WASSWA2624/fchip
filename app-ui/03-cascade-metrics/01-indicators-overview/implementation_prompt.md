# Implementation prompt — `03-cascade-metrics/01-indicators-overview`
...
| Screen slug | `01-indicators-overview` |
| Chronology | `S-029` |
| Slice | `VS-03` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/03-cascade-metrics/01-indicators-overview` |
...
| Localization prefix | `03_cascade_metrics.01_indicators_overview` |
...
Folder: `app-ui/03-cascade-metrics/01-indicators-overview/`
...
| Page | `frontend/lib/features/cascade_metrics/presentation/pages/01_indicators_overview_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-cascade-metrics/01-indicators-overview`) |
...
8. Register route `/03-cascade-metrics/01-indicators-overview` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_cascade_metrics.01_indicators_overview` only — no hard-coded user strings.
