# Implementation prompt — `03-cascade-metrics/02-gap-detection`
...
Folder: `app-ui/03-cascade-metrics/02-gap-detection/`
...
| Page | `frontend/lib/features/cascade_metrics/presentation/pages/02_gap_detection_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-cascade-metrics/02-gap-detection`) |
...
7. Map navigation:
   ...
   - Primary action: Open action plan → `/03-cascade-metrics/01-indicators-overview`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-cascade-metrics/02-gap-detection` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_cascade_metrics.02_gap_detection` only — no hard-coded user strings.
