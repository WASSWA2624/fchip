# Implementation prompt — `03-cascade-metrics/03-partner-reports`
...
| Screen slug | `03-partner-reports` |
| Chronology | `S-031` |
| Slice | `VS-03` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/03-cascade-metrics/03-partner-reports` |
...
| Localization prefix | `03_cascade_metrics.03_partner_reports` |
...
Folder: `app-ui/03-cascade-metrics/03-partner-reports/`
...
| Page | `frontend/lib/features/cascade_metrics/presentation/pages/03_partner_reports_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-cascade-metrics/03-partner-reports`) |
...
7. Map navigation:
   ...
   - Primary action: Generate report → `/14-ngo-partner/01-programme-monitoring`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-cascade-metrics/03-partner-reports` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `03_cascade_metrics.03_partner_reports` only — no hard-coded user strings.
