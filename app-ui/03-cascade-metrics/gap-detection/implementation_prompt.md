# Implementation prompt — `03-cascade-metrics/gap-detection`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Gap detection** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Learn · improve · serve again_
...
| Screen slug | `gap-detection` |
| Chronology | `S-030` |
| Slice | `VS-03` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/03-cascade-metrics/gap-detection` |
...
| Localization prefix | `03_cascade_metrics.gap_detection` |
...
Folder: `app-ui/03-cascade-metrics/gap-detection/`
...
| Page | `frontend/lib/features/cascade_metrics/presentation/pages/gap_detection_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-cascade-metrics/gap-detection`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-030`:

### Phase 0 — Claim the screen

1. Confirm `S-030` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Open action plan → `/03-cascade-metrics/indicators-overview`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-cascade-metrics/gap-detection` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_cascade_metrics.gap_detection` only — no hard-coded user strings.
...
23. Mark `S-030` `status: done`, `backend: paired` (or `none` with static evidence only).
