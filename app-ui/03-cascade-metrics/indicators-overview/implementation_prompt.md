# Implementation prompt — `03-cascade-metrics/indicators-overview`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Cascade metrics** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: CHW · outreach · referrals · MCH · education_
...
| Module | `03-cascade-metrics` |
| Screen slug | `indicators-overview` |
| Chronology | `S-029` |
| Slice | `VS-03` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/03-cascade-metrics/indicators-overview` |
| Parent route | `/03-cascade-metrics/indicators-overview` |
...
| Localization prefix | `03_cascade_metrics.indicators_overview` |
...
Folder: `app-ui/03-cascade-metrics/indicators-overview/`
...
- `app-ui/03-cascade-metrics/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/cascade_metrics/` |
| Page | `frontend/lib/features/cascade_metrics/presentation/pages/indicators_overview_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/cascade_metrics/presentation/widgets/` |
| Presentation state | `frontend/lib/features/cascade_metrics/presentation/state/` |
| Domain | `frontend/lib/features/cascade_metrics/domain/` |
| Data / repository impl | `frontend/lib/features/cascade_metrics/data/` |
| Repository contract | `frontend/lib/features/cascade_metrics/domain/repositories/cascade_metrics_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/03-cascade-metrics/indicators-overview`) |
...
| Tests | `frontend/test/features/cascade_metrics/` |
| Backend module | `backend/src/modules/cascade_metrics/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-029`:

### Phase 0 — Claim the screen

1. Confirm `S-029` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/03-cascade-metrics/indicators-overview` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_cascade_metrics.indicators_overview` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/cascade_metrics/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/cascade_metrics/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-029` `status: done`, `backend: paired` (or `none` with static evidence only).
