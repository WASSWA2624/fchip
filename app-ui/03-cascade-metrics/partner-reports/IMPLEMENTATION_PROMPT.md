# Implementation prompt — `03-cascade-metrics/partner-reports`
...
| Route | `/03-cascade-metrics/partner-reports` |
...
| Localization prefix | `03_cascade_metrics.partner_reports` |
...
Folder: `app-ui/03-cascade-metrics/partner-reports/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-cascade-metrics/partner-reports`) |
...
7. Map navigation:
   ...
   - Primary action: Generate report → `/14-ngo-partner/programme-monitoring`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-cascade-metrics/partner-reports` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `03_cascade_metrics.partner_reports` only — no hard-coded user strings.
