# Implementation prompt — `03-outreach-school-health/campaign-planner`
...
| Module | `03-outreach-school-health` |
...
| Route | `/03-outreach-school-health/campaign-planner` |
| Parent route | `/10-district-moh/population-map` |
...
| Localization prefix | `03_outreach_school_health.campaign_planner` |
...
Folder: `app-ui/03-outreach-school-health/campaign-planner/`
...
- `app-ui/03-outreach-school-health/README.md`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-outreach-school-health/campaign-planner`) |
...
7. Map navigation:
   - Tabs: Map → `/10-district-moh/population-map`; Warnings → `/10-district-moh/early-warnings`; Metrics → `/04-cascade-metrics/indicators-overview`; Plan → `/10-district-moh/cascade-planning`
   - Primary action: Create campaign → `/10-district-moh/population-map`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-outreach-school-health/campaign-planner` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_outreach_school_health.campaign_planner` only — no hard-coded user strings.
