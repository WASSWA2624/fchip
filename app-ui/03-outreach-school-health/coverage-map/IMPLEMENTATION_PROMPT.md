# Implementation prompt — `03-outreach-school-health/coverage-map`
...
| Screen slug | `coverage-map` |
| Chronology | `S-067` |
| Slice | `VS-03` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/03-outreach-school-health/coverage-map` |
...
| Localization prefix | `03_outreach_school_health.coverage_map` |
...
Folder: `app-ui/03-outreach-school-health/coverage-map/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/coverage_map_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-outreach-school-health/coverage-map`) |
...
8. Register route `/03-outreach-school-health/coverage-map` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_outreach_school_health.coverage_map` only — no hard-coded user strings.
