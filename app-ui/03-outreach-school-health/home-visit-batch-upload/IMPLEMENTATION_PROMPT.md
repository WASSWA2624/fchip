# Implementation prompt — `03-outreach-school-health/home-visit-batch-upload`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Home-visit batch** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Supervisors upload structured visit packs_
...
| Screen slug | `home-visit-batch-upload` |
| Chronology | `S-069` |
| Slice | `VS-03` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/03-outreach-school-health/home-visit-batch-upload` |
...
| Localization prefix | `03_outreach_school_health.home_visit_batch_upload` |
...
Folder: `app-ui/03-outreach-school-health/home-visit-batch-upload/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/home_visit_batch_upload_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-outreach-school-health/home-visit-batch-upload`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-069`:

### Phase 0 — Claim the screen

1. Confirm `S-069` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Upload batch → `/03-outreach-school-health/home-visit-batch-upload?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-outreach-school-health/home-visit-batch-upload` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_outreach_school_health.home_visit_batch_upload` only — no hard-coded user strings.
...
23. Mark `S-069` `status: done`, `backend: paired` (or `none` with static evidence only).
