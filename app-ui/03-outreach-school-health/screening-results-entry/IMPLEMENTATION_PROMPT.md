# Implementation prompt — `03-outreach-school-health/screening-results-entry`
...
| Route | `/03-outreach-school-health/screening-results-entry` |
...
| Localization prefix | `03_outreach_school_health.screening_results_entry` |
...
Folder: `app-ui/03-outreach-school-health/screening-results-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/03-outreach-school-health/screening-results-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Queue to ingest → `/03-outreach-school-health/screening-results-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/03-outreach-school-health/screening-results-entry` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `03_outreach_school_health.screening_results_entry` only — no hard-coded user strings.
