# Implementation prompt — `04-cascade-metrics/gap-detection`
...
| Route | `/04-cascade-metrics/gap-detection` |
...
| Localization prefix | `04_cascade_metrics.gap_detection` |
...
Folder: `app-ui/04-cascade-metrics/gap-detection/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-cascade-metrics/gap-detection`) |
...
7. Map navigation:
   ...
   - Primary action: Open action plan → `/04-cascade-metrics/indicators-overview`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-cascade-metrics/gap-detection` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `04_cascade_metrics.gap_detection` only — no hard-coded user strings.
