# Implementation prompt — `04-cascade-metrics/partner-reports`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Partner reports** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: M&E exports from metrics spine_
...
| Screen slug | `partner-reports` |
| Chronology | `S-031` |
| Slice | `VS-04` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/04-cascade-metrics/partner-reports` |
...
| Localization prefix | `04_cascade_metrics.partner_reports` |
...
Folder: `app-ui/04-cascade-metrics/partner-reports/`
...
| Page | `frontend/lib/features/cascade_metrics/presentation/pages/partner_reports_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-cascade-metrics/partner-reports`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-031`:

### Phase 0 — Claim the screen

1. Confirm `S-031` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Generate report → `/11-ngo-partner/programme-monitoring`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-cascade-metrics/partner-reports` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `04_cascade_metrics.partner_reports` only — no hard-coded user strings.
...
23. Mark `S-031` `status: done`, `backend: paired` (or `none` with static evidence only).
