# Implementation prompt — `04-facility-dashboard/02-catchment-map`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Catchment map** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Village hotspots + climate overlay_
...
| Screen slug | `02-catchment-map` |
| Chronology | `S-033` |
| Slice | `VS-04` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/04-facility-dashboard/02-catchment-map` |
| Parent route | `/04-facility-dashboard/02-catchment-map` |
...
| Localization prefix | `04_facility_dashboard.02_catchment_map` |
...
Folder: `app-ui/04-facility-dashboard/02-catchment-map/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/02_catchment_map_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/02-catchment-map`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-033`:

### Phase 0 — Claim the screen

1. Confirm `S-033` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/04-facility-dashboard/02-catchment-map` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.02_catchment_map` only — no hard-coded user strings.
...
23. Mark `S-033` `status: done`, `backend: paired` (or `none` with static evidence only).
