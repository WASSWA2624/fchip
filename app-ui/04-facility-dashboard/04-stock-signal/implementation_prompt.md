# Implementation prompt — `04-facility-dashboard/04-stock-signal`
...
| Screen slug | `04-stock-signal` |
| Chronology | `S-035` |
| Slice | `VS-04` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/04-facility-dashboard/04-stock-signal` |
| Parent route | `/04-facility-dashboard/04-stock-signal` |
...
| Localization prefix | `04_facility_dashboard.04_stock_signal` |
...
Folder: `app-ui/04-facility-dashboard/04-stock-signal/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/04_stock_signal_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/04-stock-signal`) |
...
8. Register route `/04-facility-dashboard/04-stock-signal` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.04_stock_signal` only — no hard-coded user strings.
