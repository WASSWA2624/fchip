# Implementation prompt — `04-facility-dashboard/06-clinical-share-confirm`
...
| Screen slug | `06-clinical-share-confirm` |
| Chronology | `S-037` |
| Slice | `VS-04` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/04-facility-dashboard/06-clinical-share-confirm` |
...
| Localization prefix | `04_facility_dashboard.06_clinical_share_confirm` |
...
Folder: `app-ui/04-facility-dashboard/06-clinical-share-confirm/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/06_clinical_share_confirm_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/06-clinical-share-confirm`) |
...
7. Map navigation:
   ...
   - Primary action: Push to FCHIP → `/04-facility-dashboard/06-clinical-share-confirm?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-facility-dashboard/06-clinical-share-confirm` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.06_clinical_share_confirm` only — no hard-coded user strings.
