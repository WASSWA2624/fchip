# Implementation prompt — `04-facility-dashboard/08-medicine-demand-forecast`
...
| Screen slug | `08-medicine-demand-forecast` |
| Chronology | `S-122` |
| Slice | `VS-04` |
| Phase | `phase-3` |
| Deferred | `true` |
| Chronology status | `not-started` |
| Route | `/04-facility-dashboard/08-medicine-demand-forecast` |
...
| Localization prefix | `04_facility_dashboard.08_medicine_demand_forecast` |
...
Folder: `app-ui/04-facility-dashboard/08-medicine-demand-forecast/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/08_medicine_demand_forecast_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/08-medicine-demand-forecast`) |
...
7. Map navigation:
   ...
   - Primary action: Share with district / pharmacy → `/04-facility-dashboard/01-overview`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-facility-dashboard/08-medicine-demand-forecast` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.08_medicine_demand_forecast` only — no hard-coded user strings.
