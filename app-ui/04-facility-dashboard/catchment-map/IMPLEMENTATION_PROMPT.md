# Implementation prompt — `04-facility-dashboard/catchment-map`
...
| Route | `/04-facility-dashboard/catchment-map` |
| Parent route | `/04-facility-dashboard/catchment-map` |
| Layout | `map-explorer` |
...
| Localization prefix | `04_facility_dashboard.catchment_map` |
...
Folder: `app-ui/04-facility-dashboard/catchment-map/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/catchment-map`) |
...
8. Register route `/04-facility-dashboard/catchment-map` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.catchment_map` only — no hard-coded user strings.
