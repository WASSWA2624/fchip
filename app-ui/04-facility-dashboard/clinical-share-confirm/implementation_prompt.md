# Implementation prompt — `04-facility-dashboard/clinical-share-confirm`
...
| Route | `/04-facility-dashboard/clinical-share-confirm` |
...
| Localization prefix | `04_facility_dashboard.clinical_share_confirm` |
...
Folder: `app-ui/04-facility-dashboard/clinical-share-confirm/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/clinical-share-confirm`) |
...
7. Map navigation:
   ...
   - Primary action: Push to FCHIP → `/04-facility-dashboard/clinical-share-confirm?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-facility-dashboard/clinical-share-confirm` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.clinical_share_confirm` only — no hard-coded user strings.
