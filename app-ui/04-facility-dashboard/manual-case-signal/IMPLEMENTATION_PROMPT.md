# Implementation prompt — `04-facility-dashboard/manual-case-signal`
...
| Route | `/04-facility-dashboard/manual-case-signal` |
...
| Localization prefix | `04_facility_dashboard.manual_case_signal` |
...
Folder: `app-ui/04-facility-dashboard/manual-case-signal/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/manual-case-signal`) |
...
7. Map navigation:
   ...
   - Primary action: Send signal → `/04-facility-dashboard/manual-case-signal?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-facility-dashboard/manual-case-signal` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.manual_case_signal` only — no hard-coded user strings.
