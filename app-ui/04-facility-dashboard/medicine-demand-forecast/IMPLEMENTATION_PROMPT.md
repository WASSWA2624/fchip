# Implementation prompt — `04-facility-dashboard/medicine-demand-forecast`
...
| Route | `/04-facility-dashboard/medicine-demand-forecast` |
| Parent route | `/04-facility-dashboard/overview` |
...
| Localization prefix | `04_facility_dashboard.medicine_demand_forecast` |
...
Folder: `app-ui/04-facility-dashboard/medicine-demand-forecast/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/medicine-demand-forecast`) |
...
7. Map navigation:
   ...
   - Primary action: Share with district / pharmacy → `/04-facility-dashboard/overview`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-facility-dashboard/medicine-demand-forecast` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.medicine_demand_forecast` only — no hard-coded user strings.
