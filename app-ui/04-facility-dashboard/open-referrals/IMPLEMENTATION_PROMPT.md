# Implementation prompt — `04-facility-dashboard/open-referrals`
...
| Route | `/04-facility-dashboard/open-referrals` |
...
| Localization prefix | `04_facility_dashboard.open_referrals` |
...
Folder: `app-ui/04-facility-dashboard/open-referrals/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/open-referrals`) |
...
7. Map navigation:
   ...
   - Primary action: Open referrals desk → `/05-referrals-desk/referral-queue`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/04-facility-dashboard/open-referrals` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.open_referrals` only — no hard-coded user strings.
