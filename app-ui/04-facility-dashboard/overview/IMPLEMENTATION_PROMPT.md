# Implementation prompt — `04-facility-dashboard/overview`
...
| Route | `/04-facility-dashboard/overview` |
...
| Localization prefix | `04_facility_dashboard.overview` |
...
Folder: `app-ui/04-facility-dashboard/overview/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/overview`) |
...
8. Register route `/04-facility-dashboard/overview` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.overview` only — no hard-coded user strings.
