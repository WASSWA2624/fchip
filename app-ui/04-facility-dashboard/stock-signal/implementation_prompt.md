# Implementation prompt — `04-facility-dashboard/stock-signal`
...
| Route | `/04-facility-dashboard/stock-signal` |
| Parent route | `/04-facility-dashboard/stock-signal` |
...
| Localization prefix | `04_facility_dashboard.stock_signal` |
...
Folder: `app-ui/04-facility-dashboard/stock-signal/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/04-facility-dashboard/stock-signal`) |
...
8. Register route `/04-facility-dashboard/stock-signal` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `04_facility_dashboard.stock_signal` only — no hard-coded user strings.
