# Implementation prompt — `05-chis-livelihoods/contributions`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Contributions** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Where partners capture payments_
...
| Screen slug | `contributions` |
| Chronology | `S-079` |
| Slice | `VS-05` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/05-chis-livelihoods/contributions` |
...
| Localization prefix | `05_chis_livelihoods.contributions` |
...
Folder: `app-ui/05-chis-livelihoods/contributions/`
...
| Page | `frontend/lib/features/chis_livelihoods/presentation/pages/contributions_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-chis-livelihoods/contributions`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-079`:

### Phase 0 — Claim the screen

1. Confirm `S-079` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/05-chis-livelihoods/contributions` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `05_chis_livelihoods.contributions` only — no hard-coded user strings.
...
23. Mark `S-079` `status: done`, `backend: paired` (or `none` with static evidence only).
