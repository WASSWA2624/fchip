# Implementation prompt — `05-chis-livelihoods/enrolment`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **CHIS enrolment** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Optional financial-protection data_
...
| Screen slug | `enrolment` |
| Chronology | `S-078` |
| Slice | `VS-05` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/05-chis-livelihoods/enrolment` |
...
| Localization prefix | `05_chis_livelihoods.enrolment` |
...
Folder: `app-ui/05-chis-livelihoods/enrolment/`
...
| Page | `frontend/lib/features/chis_livelihoods/presentation/pages/enrolment_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-chis-livelihoods/enrolment`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-078`:

### Phase 0 — Claim the screen

1. Confirm `S-078` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save enrolment → `/05-chis-livelihoods/enrolment?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-chis-livelihoods/enrolment` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `05_chis_livelihoods.enrolment` only — no hard-coded user strings.
...
23. Mark `S-078` `status: done`, `backend: paired` (or `none` with static evidence only).
