# Implementation prompt — `05-chis-livelihoods/iga-participation-entry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **IGA participation entry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Livelihood groups feed participation where tracked_
...
| Screen slug | `iga-participation-entry` |
| Chronology | `S-081` |
| Slice | `VS-05` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/05-chis-livelihoods/iga-participation-entry` |
...
| Localization prefix | `05_chis_livelihoods.iga_participation_entry` |
...
Folder: `app-ui/05-chis-livelihoods/iga-participation-entry/`
...
| Page | `frontend/lib/features/chis_livelihoods/presentation/pages/iga_participation_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-chis-livelihoods/iga-participation-entry`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-081`:

### Phase 0 — Claim the screen

1. Confirm `S-081` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Submit IGA signal → `/05-chis-livelihoods/iga-participation-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-chis-livelihoods/iga-participation-entry` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `05_chis_livelihoods.iga_participation_entry` only — no hard-coded user strings.
...
23. Mark `S-081` `status: done`, `backend: paired` (or `none` with static evidence only).
