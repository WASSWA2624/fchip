# Implementation prompt — `05-referrals-desk/02-referral-queue-empty`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Referrals desk** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: No open inbound referrals_
...
| Screen slug | `02-referral-queue-empty` |
| Chronology | `S-040` |
| Slice | `VS-05` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/05-referrals-desk/02-referral-queue-empty` |
| Parent route | `/05-referrals-desk/01-referral-queue` |
| Layout | `13-queue-desk` |
| Body layouts | `mobile` → `13-queue-desk`, `tablet` → `13-queue-desk`, `desktop` → `13-queue-desk` |
...
| Localization prefix | `05_referrals_desk.02_referral_queue_empty` |
...
Folder: `app-ui/05-referrals-desk/02-referral-queue-empty/`
...
- Shared layout specimens for `13-queue-desk` under `app-ui/00-shared/layouts/13-queue-desk/`
...
| Page | `frontend/lib/features/referrals_desk/presentation/pages/02_referral_queue_empty_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/02-referral-queue-empty`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `13-queue-desk`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-040`:

### Phase 0 — Claim the screen

1. Confirm `S-040` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `13-queue-desk` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   ...
   - Primary action: Refresh queue → `/05-referrals-desk/01-referral-queue`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/02-referral-queue-empty` with the access guard for roles `facility-clinician`, `facility-manager`.
9. Compose the page from shared shells + `13-queue-desk` + catalog components. Prefer:
...
11. Add ARB keys under `05_referrals_desk.02_referral_queue_empty` only — no hard-coded user strings.
...
23. Mark `S-040` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `13-queue-desk` is a shared layout implementation, not a one-off Scaffold tree
