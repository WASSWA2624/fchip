# Implementation prompt — `05-referrals-desk/03-referral-detail`
...
| Screen slug | `03-referral-detail` |
| Chronology | `S-041` |
| Slice | `VS-05` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/05-referrals-desk/03-referral-detail` |
...
| Localization prefix | `05_referrals_desk.03_referral_detail` |
...
Folder: `app-ui/05-referrals-desk/03-referral-detail/`
...
| Page | `frontend/lib/features/referrals_desk/presentation/pages/03_referral_detail_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/03-referral-detail`) |
...
7. Map navigation:
   ...
   - Primary action: Mark completed → `/05-referrals-desk/04-outcome-feedback`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/03-referral-detail` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `05_referrals_desk.03_referral_detail` only — no hard-coded user strings.
