# Implementation prompt — `05-referrals-desk/04-outcome-feedback`
...
| Screen slug | `04-outcome-feedback` |
| Chronology | `S-042` |
| Slice | `VS-05` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/05-referrals-desk/04-outcome-feedback` |
...
| Localization prefix | `05_referrals_desk.04_outcome_feedback` |
...
Folder: `app-ui/05-referrals-desk/04-outcome-feedback/`
...
| Page | `frontend/lib/features/referrals_desk/presentation/pages/04_outcome_feedback_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/04-outcome-feedback`) |
...
7. Map navigation:
   ...
   - Primary action: Sync outcome → `/05-referrals-desk/01-referral-queue?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/04-outcome-feedback` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `05_referrals_desk.04_outcome_feedback` only — no hard-coded user strings.
