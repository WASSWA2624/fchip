# Implementation prompt — `05-referrals-desk/outcome-feedback`
...
| Route | `/05-referrals-desk/outcome-feedback` |
...
| Localization prefix | `05_referrals_desk.outcome_feedback` |
...
Folder: `app-ui/05-referrals-desk/outcome-feedback/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/outcome-feedback`) |
...
7. Map navigation:
   ...
   - Primary action: Sync outcome → `/05-referrals-desk/referral-queue?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/outcome-feedback` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `05_referrals_desk.outcome_feedback` only — no hard-coded user strings.
