# Implementation prompt — `05-referrals-desk/referral-detail`
...
| Route | `/05-referrals-desk/referral-detail` |
| Parent route | `/05-referrals-desk/referral-queue` |
...
| Localization prefix | `05_referrals_desk.referral_detail` |
...
Folder: `app-ui/05-referrals-desk/referral-detail/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/referral-detail`) |
...
7. Map navigation:
   - Tabs: Overview → `/04-facility-dashboard/overview`; Map → `/04-facility-dashboard/catchment-map`; Referrals → `/05-referrals-desk/referral-queue`; Stock → `/04-facility-dashboard/stock-signal`
   - Primary action: Mark completed → `/05-referrals-desk/outcome-feedback`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/referral-detail` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `05_referrals_desk.referral_detail` only — no hard-coded user strings.
