# Implementation prompt — `05-referrals-desk/referral-queue-empty`
...
| Route | `/05-referrals-desk/referral-queue-empty` |
...
| Localization prefix | `05_referrals_desk.referral_queue_empty` |
...
Folder: `app-ui/05-referrals-desk/referral-queue-empty/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/referral-queue-empty`) |
...
7. Map navigation:
   ...
   - Primary action: Refresh queue → `/05-referrals-desk/referral-queue`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/referral-queue-empty` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `05_referrals_desk.referral_queue_empty` only — no hard-coded user strings.
