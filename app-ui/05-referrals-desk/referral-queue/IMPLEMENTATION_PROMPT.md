# Implementation prompt — `05-referrals-desk/referral-queue`
...
| Route | `/05-referrals-desk/referral-queue` |
...
| Localization prefix | `05_referrals_desk.referral_queue` |
...
Folder: `app-ui/05-referrals-desk/referral-queue/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/05-referrals-desk/referral-queue`) |
...
7. Map navigation:
   ...
   - Primary action: Claim next → `/05-referrals-desk/referral-detail`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/05-referrals-desk/referral-queue` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `05_referrals_desk.referral_queue` only — no hard-coded user strings.
