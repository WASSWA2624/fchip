# Implementation prompt — `06-emr-connector/01-connector-status`
...
| Screen slug | `01-connector-status` |
| Chronology | `S-043` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-emr-connector/01-connector-status` |
...
| Localization prefix | `06_emr_connector.01_connector_status` |
...
Folder: `app-ui/06-emr-connector/01-connector-status/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/01_connector_status_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/01-connector-status`) |
...
8. Register route `/06-emr-connector/01-connector-status` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.01_connector_status` only — no hard-coded user strings.
