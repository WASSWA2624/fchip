# Implementation prompt — `06-emr-connector/02-connector-degraded`
...
| Screen slug | `02-connector-degraded` |
| Chronology | `S-044` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-emr-connector/02-connector-degraded` |
...
| Localization prefix | `06_emr_connector.02_connector_degraded` |
...
Folder: `app-ui/06-emr-connector/02-connector-degraded/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/02_connector_degraded_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/02-connector-degraded`) |
...
7. Map navigation:
   ...
   - Primary action: Retry failed pushes → `/06-emr-connector/01-connector-status`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/02-connector-degraded` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.02_connector_degraded` only — no hard-coded user strings.
