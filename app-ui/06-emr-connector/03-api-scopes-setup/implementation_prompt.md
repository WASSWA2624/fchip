# Implementation prompt — `06-emr-connector/03-api-scopes-setup`
...
| Screen slug | `03-api-scopes-setup` |
| Chronology | `S-045` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-emr-connector/03-api-scopes-setup` |
...
| Localization prefix | `06_emr_connector.03_api_scopes_setup` |
...
Folder: `app-ui/06-emr-connector/03-api-scopes-setup/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/03_api_scopes_setup_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/03-api-scopes-setup`) |
...
7. Map navigation:
   ...
   - Primary action: Save scopes → `/06-emr-connector/03-api-scopes-setup?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/03-api-scopes-setup` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.03_api_scopes_setup` only — no hard-coded user strings.
