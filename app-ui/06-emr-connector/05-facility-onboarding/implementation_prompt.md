# Implementation prompt — `06-emr-connector/05-facility-onboarding`
...
| Screen slug | `05-facility-onboarding` |
| Chronology | `S-047` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-emr-connector/05-facility-onboarding` |
...
| Localization prefix | `06_emr_connector.05_facility_onboarding` |
...
Folder: `app-ui/06-emr-connector/05-facility-onboarding/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/05_facility_onboarding_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/05-facility-onboarding`) |
...
7. Map navigation:
   ...
   - Primary action: Issue feeder credentials → `/06-emr-connector/05-facility-onboarding?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/05-facility-onboarding` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.05_facility_onboarding` only — no hard-coded user strings.
