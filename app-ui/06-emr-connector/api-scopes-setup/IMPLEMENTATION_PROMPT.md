# Implementation prompt — `06-emr-connector/api-scopes-setup`
...
| Route | `/06-emr-connector/api-scopes-setup` |
...
| Localization prefix | `06_emr_connector.api_scopes_setup` |
...
Folder: `app-ui/06-emr-connector/api-scopes-setup/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/api-scopes-setup`) |
...
7. Map navigation:
   ...
   - Primary action: Save scopes → `/06-emr-connector/api-scopes-setup?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/api-scopes-setup` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.api_scopes_setup` only — no hard-coded user strings.
