# Implementation prompt — `06-emr-connector/connector-degraded`
...
| Route | `/06-emr-connector/connector-degraded` |
| Parent route | `/06-emr-connector/connector-status` |
...
| Localization prefix | `06_emr_connector.connector_degraded` |
...
Folder: `app-ui/06-emr-connector/connector-degraded/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/connector-degraded`) |
...
7. Map navigation:
   ...
   - Primary action: Retry failed pushes → `/06-emr-connector/connector-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/connector-degraded` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.connector_degraded` only — no hard-coded user strings.
