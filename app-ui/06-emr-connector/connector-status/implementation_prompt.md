# Implementation prompt — `06-emr-connector/connector-status`
...
| Route | `/06-emr-connector/connector-status` |
...
| Localization prefix | `06_emr_connector.connector_status` |
...
Folder: `app-ui/06-emr-connector/connector-status/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/connector-status`) |
...
8. Register route `/06-emr-connector/connector-status` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.connector_status` only — no hard-coded user strings.
