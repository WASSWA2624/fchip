# Implementation prompt — `06-emr-connector/facility-onboarding`
...
| Route | `/06-emr-connector/facility-onboarding` |
...
| Localization prefix | `06_emr_connector.facility_onboarding` |
...
Folder: `app-ui/06-emr-connector/facility-onboarding/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/facility-onboarding`) |
...
7. Map navigation:
   ...
   - Primary action: Issue feeder credentials → `/06-emr-connector/facility-onboarding?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/facility-onboarding` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.facility_onboarding` only — no hard-coded user strings.
