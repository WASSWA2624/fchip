# Implementation prompt — `06-emr-connector/push-event-log`
...
| Route | `/06-emr-connector/push-event-log` |
...
| Localization prefix | `06_emr_connector.push_event_log` |
...
Folder: `app-ui/06-emr-connector/push-event-log/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-emr-connector/push-event-log`) |
...
7. Map navigation:
   ...
   - Primary action: Download audit CSV → `/06-emr-connector/connector-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-emr-connector/push-event-log` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `06_emr_connector.push_event_log` only — no hard-coded user strings.
