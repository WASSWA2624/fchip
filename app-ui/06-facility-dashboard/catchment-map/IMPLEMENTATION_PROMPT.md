# Implementation prompt — `06-facility-dashboard/catchment-map`
...
| Screen slug | `catchment-map` |
| Chronology | `S-033` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-facility-dashboard/catchment-map` |
| Parent route | `/06-facility-dashboard/catchment-map` |
| Layout | `map-explorer` |
...
| Localization prefix | `06_facility_dashboard.catchment_map` |
...
Folder: `app-ui/06-facility-dashboard/catchment-map/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/catchment_map_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-facility-dashboard/catchment-map`) |
...
8. Register route `/06-facility-dashboard/catchment-map` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `06_facility_dashboard.catchment_map` only — no hard-coded user strings.
