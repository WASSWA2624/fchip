# Implementation prompt — `06-facility-dashboard/clinical-share-confirm`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Clinical share confirm** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Facility staff approve what leaves EMR into FCHIP_
...
| Screen slug | `clinical-share-confirm` |
| Chronology | `S-037` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-facility-dashboard/clinical-share-confirm` |
...
| Localization prefix | `06_facility_dashboard.clinical_share_confirm` |
...
Folder: `app-ui/06-facility-dashboard/clinical-share-confirm/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/clinical_share_confirm_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-facility-dashboard/clinical-share-confirm`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-037`:

### Phase 0 — Claim the screen

1. Confirm `S-037` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Push to FCHIP → `/06-facility-dashboard/clinical-share-confirm?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-facility-dashboard/clinical-share-confirm` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `06_facility_dashboard.clinical_share_confirm` only — no hard-coded user strings.
...
23. Mark `S-037` `status: done`, `backend: paired` (or `none` with static evidence only).
