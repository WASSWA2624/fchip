# Implementation prompt — `06-facility-dashboard/medicine-demand-forecast`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Medicine demand forecast** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: §6.5 demand by facility / community (Phase 3 depth)_
...
| Module | `06-facility-dashboard` |
| Screen slug | `medicine-demand-forecast` |
| Chronology | `S-122` |
| Slice | `VS-06` |
| Phase | `phase-3` |
| Deferred | `true` |
| Chronology status | `not-started` |
| Route | `/06-facility-dashboard/medicine-demand-forecast` |
| Parent route | `/06-facility-dashboard/overview` |
...
| Localization prefix | `06_facility_dashboard.medicine_demand_forecast` |
...
Folder: `app-ui/06-facility-dashboard/medicine-demand-forecast/`
...
- `app-ui/06-facility-dashboard/README.md`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/medicine_demand_forecast_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-facility-dashboard/medicine-demand-forecast`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-122`:

### Phase 0 — Claim the screen

1. Confirm `S-122` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Share with district / pharmacy → `/06-facility-dashboard/overview`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-facility-dashboard/medicine-demand-forecast` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `06_facility_dashboard.medicine_demand_forecast` only — no hard-coded user strings.
...
23. Mark `S-122` `status: done`, `backend: paired` (or `none` with static evidence only).
