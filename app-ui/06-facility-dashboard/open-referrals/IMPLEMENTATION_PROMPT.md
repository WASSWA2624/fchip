# Implementation prompt — `06-facility-dashboard/open-referrals`
...
| Screen slug | `open-referrals` |
| Chronology | `S-034` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-facility-dashboard/open-referrals` |
...
| Localization prefix | `06_facility_dashboard.open_referrals` |
...
Folder: `app-ui/06-facility-dashboard/open-referrals/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/open_referrals_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-facility-dashboard/open-referrals`) |
...
7. Map navigation:
   ...
   - Primary action: Open referrals desk → `/07-referrals-desk/referral-queue`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/06-facility-dashboard/open-referrals` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `06_facility_dashboard.open_referrals` only — no hard-coded user strings.
