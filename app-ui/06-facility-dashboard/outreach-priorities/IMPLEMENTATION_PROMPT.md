# Implementation prompt — `06-facility-dashboard/outreach-priorities`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Outreach priorities** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Where facility should support field action_
...
| Screen slug | `outreach-priorities` |
| Chronology | `S-036` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-facility-dashboard/outreach-priorities` |
...
| Localization prefix | `06_facility_dashboard.outreach_priorities` |
...
Folder: `app-ui/06-facility-dashboard/outreach-priorities/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/outreach_priorities_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-facility-dashboard/outreach-priorities`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-036`:

### Phase 0 — Claim the screen

1. Confirm `S-036` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/06-facility-dashboard/outreach-priorities` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `06_facility_dashboard.outreach_priorities` only — no hard-coded user strings.
...
23. Mark `S-036` `status: done`, `backend: paired` (or `none` with static evidence only).
