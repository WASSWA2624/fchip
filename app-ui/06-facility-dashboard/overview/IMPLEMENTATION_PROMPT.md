# Implementation prompt — `06-facility-dashboard/overview`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Facility overview** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Alerts · caseload risk · cascade gaps_
...
| Screen slug | `overview` |
| Chronology | `S-032` |
| Slice | `VS-06` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/06-facility-dashboard/overview` |
...
| Localization prefix | `06_facility_dashboard.overview` |
...
Folder: `app-ui/06-facility-dashboard/overview/`
...
| Page | `frontend/lib/features/facility_dashboard/presentation/pages/overview_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/06-facility-dashboard/overview`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-032`:

### Phase 0 — Claim the screen

1. Confirm `S-032` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/06-facility-dashboard/overview` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `06_facility_dashboard.overview` only — no hard-coded user strings.
...
23. Mark `S-032` `status: done`, `backend: paired` (or `none` with static evidence only).
