# Implementation prompt — `07-climate-feeds/climate-home`
...
| Route | `/07-climate-feeds/climate-home` |
| Parent route | `/07-climate-feeds/climate-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `07_climate_feeds.climate_home` |
...
Folder: `app-ui/07-climate-feeds/climate-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/07-climate-feeds/climate-home`) |
...
7. Map navigation:
   ...
   - Primary action: Refresh feeds → `/07-climate-feeds/climate-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/07-climate-feeds/climate-home` with the access guard for roles `climate-data-operator`.
...
11. Add ARB keys under `07_climate_feeds.climate_home` only — no hard-coded user strings.
