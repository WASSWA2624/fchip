# Implementation prompt — `07-climate-feeds/feed-config-audit`
...
| Route | `/07-climate-feeds/feed-config-audit` |
| Parent route | `/07-climate-feeds/feed-config-audit` |
...
| Localization prefix | `07_climate_feeds.feed_config_audit` |
...
Folder: `app-ui/07-climate-feeds/feed-config-audit/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/07-climate-feeds/feed-config-audit`) |
...
7. Map navigation:
   - Tabs: Home → `/07-climate-feeds/climate-home`; Rain → `/07-climate-feeds/rainfall-temperature`; Extremes → `/07-climate-feeds/extremes-flood-heat`; Config → `/07-climate-feeds/feed-config-audit`
   - Primary action: Save feed config → `/07-climate-feeds/feed-config-audit?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/07-climate-feeds/feed-config-audit` with the access guard for roles `climate-data-operator`.
...
11. Add ARB keys under `07_climate_feeds.feed_config_audit` only — no hard-coded user strings.
