# Implementation prompt — `07-referrals-desk/outcome-feedback`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Outcome feedback** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Completed referral outcomes return into cascade metrics_
...
| Screen slug | `outcome-feedback` |
| Chronology | `S-042` |
| Slice | `VS-07` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/07-referrals-desk/outcome-feedback` |
...
| Localization prefix | `07_referrals_desk.outcome_feedback` |
...
Folder: `app-ui/07-referrals-desk/outcome-feedback/`
...
| Page | `frontend/lib/features/referrals_desk/presentation/pages/outcome_feedback_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/07-referrals-desk/outcome-feedback`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-042`:

### Phase 0 — Claim the screen

1. Confirm `S-042` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Sync outcome → `/07-referrals-desk/referral-queue?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/07-referrals-desk/outcome-feedback` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `07_referrals_desk.outcome_feedback` only — no hard-coded user strings.
...
23. Mark `S-042` `status: done`, `backend: paired` (or `none` with static evidence only).
