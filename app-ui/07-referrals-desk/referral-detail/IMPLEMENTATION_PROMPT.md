# Implementation prompt — `07-referrals-desk/referral-detail`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Referral detail** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Track completion · link EMR outcome_
...
| --- | --- |
| Kind | Screen (`detail`) |
| Module | `07-referrals-desk` |
| Screen slug | `referral-detail` |
| Chronology | `S-041` |
| Slice | `VS-07` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/07-referrals-desk/referral-detail` |
| Parent route | `/07-referrals-desk/referral-queue` |
...
| Localization prefix | `07_referrals_desk.referral_detail` |
...
Folder: `app-ui/07-referrals-desk/referral-detail/`
...
- `app-ui/07-referrals-desk/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/referrals_desk/` |
| Page | `frontend/lib/features/referrals_desk/presentation/pages/referral_detail_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/referrals_desk/presentation/widgets/` |
| Presentation state | `frontend/lib/features/referrals_desk/presentation/state/` |
| Domain | `frontend/lib/features/referrals_desk/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/07-referrals-desk/referral-detail`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-041`:

### Phase 0 — Claim the screen

1. Confirm `S-041` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Overview → `/06-facility-dashboard/overview`; Map → `/06-facility-dashboard/catchment-map`; Referrals → `/07-referrals-desk/referral-queue`; Stock → `/06-facility-dashboard/stock-signal`
   - Primary action: Mark completed → `/07-referrals-desk/outcome-feedback`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/07-referrals-desk/referral-detail` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `07_referrals_desk.referral_detail` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/referrals_desk/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/referrals_desk/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-041` `status: done`, `backend: paired` (or `none` with static evidence only).
