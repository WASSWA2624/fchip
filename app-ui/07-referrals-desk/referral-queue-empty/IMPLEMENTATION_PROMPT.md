# Implementation prompt — `07-referrals-desk/referral-queue-empty`
...
| Screen slug | `referral-queue-empty` |
| Chronology | `S-040` |
| Slice | `VS-07` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/07-referrals-desk/referral-queue-empty` |
| Parent route | `/07-referrals-desk/referral-queue` |
| Layout | `queue-desk` |
| Body layouts | `mobile` → `queue-desk`, `tablet` → `queue-desk`, `desktop` → `queue-desk` |
...
| Localization prefix | `07_referrals_desk.referral_queue_empty` |
...
Folder: `app-ui/07-referrals-desk/referral-queue-empty/`
...
| Page | `frontend/lib/features/referrals_desk/presentation/pages/referral_queue_empty_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/07-referrals-desk/referral-queue-empty`) |
...
7. Map navigation:
   ...
   - Primary action: Refresh queue → `/07-referrals-desk/referral-queue`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/07-referrals-desk/referral-queue-empty` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `07_referrals_desk.referral_queue_empty` only — no hard-coded user strings.
