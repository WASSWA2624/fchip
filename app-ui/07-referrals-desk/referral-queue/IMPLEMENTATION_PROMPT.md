# Implementation prompt — `07-referrals-desk/referral-queue`
...
_Subtitle: Facility referral queue (working desk)_
...
| Screen slug | `referral-queue` |
| Chronology | `S-039` |
| Slice | `VS-07` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/07-referrals-desk/referral-queue` |
...
| Localization prefix | `07_referrals_desk.referral_queue` |
...
Folder: `app-ui/07-referrals-desk/referral-queue/`
...
| Page | `frontend/lib/features/referrals_desk/presentation/pages/referral_queue_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/07-referrals-desk/referral-queue`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-039`:

### Phase 0 — Claim the screen

1. Confirm `S-039` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Claim next → `/07-referrals-desk/referral-detail`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/07-referrals-desk/referral-queue` with the access guard for roles `facility-clinician`, `facility-manager`.
...
11. Add ARB keys under `07_referrals_desk.referral_queue` only — no hard-coded user strings.
...
23. Mark `S-039` `status: done`, `backend: paired` (or `none` with static evidence only).
