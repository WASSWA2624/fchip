# Implementation prompt — `08-district-moh/02-early-warnings`
...
| Screen slug | `02-early-warnings` |
| Chronology | `S-058` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-district-moh/02-early-warnings` |
| Parent route | `/08-district-moh/02-early-warnings` |
...
| Localization prefix | `08_district_moh.02_early_warnings` |
...
Folder: `app-ui/08-district-moh/02-early-warnings/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/02_early_warnings_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/02-early-warnings`) |
...
7. Map navigation:
   ...
   - Primary action: Deploy response → `/08-district-moh/03-action-deploy`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-district-moh/02-early-warnings` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.02_early_warnings` only — no hard-coded user strings.
