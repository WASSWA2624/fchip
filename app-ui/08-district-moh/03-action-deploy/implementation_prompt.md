# Implementation prompt — `08-district-moh/03-action-deploy`
...
| Screen slug | `03-action-deploy` |
| Chronology | `S-059` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-district-moh/03-action-deploy` |
...
| Localization prefix | `08_district_moh.03_action_deploy` |
...
Folder: `app-ui/08-district-moh/03-action-deploy/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/03_action_deploy_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/03-action-deploy`) |
...
7. Map navigation:
   ...
   - Primary action: Launch deployment → `/08-district-moh/04-cascade-planning?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-district-moh/03-action-deploy` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.03_action_deploy` only — no hard-coded user strings.
