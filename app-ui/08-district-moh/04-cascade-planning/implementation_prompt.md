# Implementation prompt — `08-district-moh/04-cascade-planning`
...
| Screen slug | `04-cascade-planning` |
| Chronology | `S-060` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-district-moh/04-cascade-planning` |
| Parent route | `/08-district-moh/04-cascade-planning` |
...
| Localization prefix | `08_district_moh.04_cascade_planning` |
...
Folder: `app-ui/08-district-moh/04-cascade-planning/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/04_cascade_planning_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/04-cascade-planning`) |
...
8. Register route `/08-district-moh/04-cascade-planning` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.04_cascade_planning` only — no hard-coded user strings.
