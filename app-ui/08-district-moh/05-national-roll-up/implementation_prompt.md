# Implementation prompt — `08-district-moh/05-national-roll-up`
...
| Screen slug | `05-national-roll-up` |
| Chronology | `S-123` |
| Slice | `VS-08` |
| Phase | `phase-3` |
| Deferred | `true` |
| Chronology status | `not-started` |
| Route | `/08-district-moh/05-national-roll-up` |
...
| Localization prefix | `08_district_moh.05_national_roll_up` |
...
Folder: `app-ui/08-district-moh/05-national-roll-up/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/05_national_roll_up_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/05-national-roll-up`) |
...
7. Map navigation:
   ...
   - Primary action: Open district drill-down → `/08-district-moh/01-population-map`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-district-moh/05-national-roll-up` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.05_national_roll_up` only — no hard-coded user strings.
