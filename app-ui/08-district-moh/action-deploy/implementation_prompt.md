# Implementation prompt — `08-district-moh/action-deploy`
...
| Route | `/08-district-moh/action-deploy` |
...
| Localization prefix | `08_district_moh.action_deploy` |
...
Folder: `app-ui/08-district-moh/action-deploy/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/action-deploy`) |
...
7. Map navigation:
   ...
   - Primary action: Launch deployment → `/08-district-moh/cascade-planning?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-district-moh/action-deploy` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.action_deploy` only — no hard-coded user strings.
