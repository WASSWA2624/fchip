# Implementation prompt — `08-district-moh/early-warnings`
...
| Route | `/08-district-moh/early-warnings` |
| Parent route | `/08-district-moh/early-warnings` |
...
| Localization prefix | `08_district_moh.early_warnings` |
...
Folder: `app-ui/08-district-moh/early-warnings/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/early-warnings`) |
...
7. Map navigation:
   ...
   - Primary action: Deploy response → `/08-district-moh/action-deploy`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-district-moh/early-warnings` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.early_warnings` only — no hard-coded user strings.
