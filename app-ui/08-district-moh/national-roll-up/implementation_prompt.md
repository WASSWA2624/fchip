# Implementation prompt — `08-district-moh/national-roll-up`
...
| Route | `/08-district-moh/national-roll-up` |
...
| Localization prefix | `08_district_moh.national_roll_up` |
...
Folder: `app-ui/08-district-moh/national-roll-up/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/national-roll-up`) |
...
7. Map navigation:
   ...
   - Primary action: Open district drill-down → `/08-district-moh/population-map`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-district-moh/national-roll-up` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.national_roll_up` only — no hard-coded user strings.
