# Implementation prompt — `08-district-moh/population-map`
...
| Route | `/08-district-moh/population-map` |
...
| Localization prefix | `08_district_moh.population_map` |
...
Folder: `app-ui/08-district-moh/population-map/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-district-moh/population-map`) |
...
8. Register route `/08-district-moh/population-map` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `08_district_moh.population_map` only — no hard-coded user strings.
