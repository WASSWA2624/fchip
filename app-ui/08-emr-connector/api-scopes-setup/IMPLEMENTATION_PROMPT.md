# Implementation prompt — `08-emr-connector/api-scopes-setup`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **API scopes setup** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: What clinical data may flow in_
...
| Screen slug | `api-scopes-setup` |
| Chronology | `S-045` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-emr-connector/api-scopes-setup` |
...
| Localization prefix | `08_emr_connector.api_scopes_setup` |
...
Folder: `app-ui/08-emr-connector/api-scopes-setup/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/api_scopes_setup_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-emr-connector/api-scopes-setup`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-045`:

### Phase 0 — Claim the screen

1. Confirm `S-045` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save scopes → `/08-emr-connector/api-scopes-setup?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-emr-connector/api-scopes-setup` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `08_emr_connector.api_scopes_setup` only — no hard-coded user strings.
...
23. Mark `S-045` `status: done`, `backend: paired` (or `none` with static evidence only).
