# Implementation prompt — `08-emr-connector/connector-degraded`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **EMR / HMS connector** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Degraded feed — queue held · no PHI dump_
...
| Module | `08-emr-connector` |
| Screen slug | `connector-degraded` |
| Chronology | `S-044` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-emr-connector/connector-degraded` |
| Parent route | `/08-emr-connector/connector-status` |
...
| Localization prefix | `08_emr_connector.connector_degraded` |
...
Folder: `app-ui/08-emr-connector/connector-degraded/`
...
- `app-ui/08-emr-connector/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/emr_connector/` |
| Page | `frontend/lib/features/emr_connector/presentation/pages/connector_degraded_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/emr_connector/presentation/widgets/` |
| Presentation state | `frontend/lib/features/emr_connector/presentation/state/` |
| Domain | `frontend/lib/features/emr_connector/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-emr-connector/connector-degraded`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-044`:

### Phase 0 — Claim the screen

1. Confirm `S-044` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Retry failed pushes → `/08-emr-connector/connector-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-emr-connector/connector-degraded` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `08_emr_connector.connector_degraded` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/emr_connector/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/emr_connector/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-044` `status: done`, `backend: paired` (or `none` with static evidence only).
