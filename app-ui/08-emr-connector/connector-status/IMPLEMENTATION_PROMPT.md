# Implementation prompt — `08-emr-connector/connector-status`
...
_Subtitle: Secure ingest — does not replace EMR_
...
| Screen slug | `connector-status` |
| Chronology | `S-043` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-emr-connector/connector-status` |
...
| Localization prefix | `08_emr_connector.connector_status` |
...
Folder: `app-ui/08-emr-connector/connector-status/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/connector_status_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-emr-connector/connector-status`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-043`:

### Phase 0 — Claim the screen

1. Confirm `S-043` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/08-emr-connector/connector-status` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `08_emr_connector.connector_status` only — no hard-coded user strings.
...
23. Mark `S-043` `status: done`, `backend: paired` (or `none` with static evidence only).
