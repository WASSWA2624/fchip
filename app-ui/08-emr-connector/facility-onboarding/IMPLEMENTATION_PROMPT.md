# Implementation prompt — `08-emr-connector/facility-onboarding`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Facility onboarding** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Register an existing EMR/HMS as a data feeder_
...
| Screen slug | `facility-onboarding` |
| Chronology | `S-047` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-emr-connector/facility-onboarding` |
...
| Localization prefix | `08_emr_connector.facility_onboarding` |
...
Folder: `app-ui/08-emr-connector/facility-onboarding/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/facility_onboarding_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-emr-connector/facility-onboarding`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-047`:

### Phase 0 — Claim the screen

1. Confirm `S-047` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Issue feeder credentials → `/08-emr-connector/facility-onboarding?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-emr-connector/facility-onboarding` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `08_emr_connector.facility_onboarding` only — no hard-coded user strings.
...
23. Mark `S-047` `status: done`, `backend: paired` (or `none` with static evidence only).
