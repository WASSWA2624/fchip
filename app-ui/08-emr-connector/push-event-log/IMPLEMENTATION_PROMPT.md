# Implementation prompt — `08-emr-connector/push-event-log`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Push event log** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: EMR/HMS systems feeding FCHIP in real time_
...
| Screen slug | `push-event-log` |
| Chronology | `S-046` |
| Slice | `VS-08` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/08-emr-connector/push-event-log` |
...
| Localization prefix | `08_emr_connector.push_event_log` |
...
Folder: `app-ui/08-emr-connector/push-event-log/`
...
| Page | `frontend/lib/features/emr_connector/presentation/pages/push_event_log_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/08-emr-connector/push-event-log`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-046`:

### Phase 0 — Claim the screen

1. Confirm `S-046` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Download audit CSV → `/08-emr-connector/connector-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/08-emr-connector/push-event-log` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `08_emr_connector.push_event_log` only — no hard-coded user strings.
...
23. Mark `S-046` `status: done`, `backend: paired` (or `none` with static evidence only).
