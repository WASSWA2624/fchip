# Implementation prompt — `09-community-caregiver/02-self-report`
...
| Screen slug | `02-self-report` |
| Chronology | `S-062` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-community-caregiver/02-self-report` |
| Parent route | `/09-community-caregiver/02-self-report` |
| Layout | `form-capture` |
...
| Localization prefix | `09_community_caregiver.02_self_report` |
...
Folder: `app-ui/09-community-caregiver/02-self-report/`
...
| Page | `frontend/lib/features/community_caregiver/presentation/pages/02_self_report_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-community-caregiver/02-self-report`) |
...
7. Map navigation:
   ...
   - Primary action: Send to CHW worklist → `/09-community-caregiver/01-my-household?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/09-community-caregiver/02-self-report` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `09_community_caregiver.02_self_report` only — no hard-coded user strings.
