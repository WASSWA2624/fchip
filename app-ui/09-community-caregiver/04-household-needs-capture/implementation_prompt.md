# Implementation prompt — `09-community-caregiver/04-household-needs-capture`
...
| Screen slug | `04-household-needs-capture` |
| Chronology | `S-064` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-community-caregiver/04-household-needs-capture` |
| Parent route | `/09-community-caregiver/04-household-needs-capture` |
| Layout | `form-capture` |
...
| Localization prefix | `09_community_caregiver.04_household_needs_capture` |
...
Folder: `app-ui/09-community-caregiver/04-household-needs-capture/`
...
| Page | `frontend/lib/features/community_caregiver/presentation/pages/04_household_needs_capture_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-community-caregiver/04-household-needs-capture`) |
...
7. Map navigation:
   - Tabs: Home → `/09-community-caregiver/01-my-household`; Report → `/09-community-caregiver/02-self-report`; Guidance → `/09-community-caregiver/03-guidance-hints`; More → `/09-community-caregiver/04-household-needs-capture`
   - Primary action: Submit needs signal → `/09-community-caregiver/04-household-needs-capture?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/09-community-caregiver/04-household-needs-capture` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `09_community_caregiver.04_household_needs_capture` only — no hard-coded user strings.
