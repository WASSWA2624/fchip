# Implementation prompt — `09-community-caregiver/my-household`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **My household** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Optional caregiver self-view_
...
| Screen slug | `my-household` |
| Chronology | `S-061` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-community-caregiver/my-household` |
| Parent route | `/09-community-caregiver/my-household` |
| Layout | `feeder-home` |
...
| Localization prefix | `09_community_caregiver.my_household` |
...
Folder: `app-ui/09-community-caregiver/my-household/`
...
| Page | `frontend/lib/features/community_caregiver/presentation/pages/my_household_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-community-caregiver/my-household`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-061`:

### Phase 0 — Claim the screen

1. Confirm `S-061` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/09-community-caregiver/my-household` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `09_community_caregiver.my_household` only — no hard-coded user strings.
...
23. Mark `S-061` `status: done`, `backend: paired` (or `none` with static evidence only).
