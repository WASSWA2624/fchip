# Implementation prompt — `09-community-caregiver/self-report`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Self-report** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Symptoms · needs · concerns_
...
| Screen slug | `self-report` |
| Chronology | `S-062` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-community-caregiver/self-report` |
| Parent route | `/09-community-caregiver/self-report` |
| Layout | `form-capture` |
...
| Localization prefix | `09_community_caregiver.self_report` |
...
Folder: `app-ui/09-community-caregiver/self-report/`
...
| Page | `frontend/lib/features/community_caregiver/presentation/pages/self_report_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-community-caregiver/self-report`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-062`:

### Phase 0 — Claim the screen

1. Confirm `S-062` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Send to CHW worklist → `/09-community-caregiver/my-household?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/09-community-caregiver/self-report` with the access guard for roles `caregiver`, `community-member`.
...
11. Add ARB keys under `09_community_caregiver.self_report` only — no hard-coded user strings.
...
23. Mark `S-062` `status: done`, `backend: paired` (or `none` with static evidence only).
