# Implementation prompt — `09-intelligence/ai-risk-scores`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **AI / predictive** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Shared risk scores · explainable alerts_
...
| Screen slug | `ai-risk-scores` |
| Chronology | `S-054` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-intelligence/ai-risk-scores` |
| Parent route | `/09-intelligence/ai-risk-scores` |
...
| Localization prefix | `09_intelligence.ai_risk_scores` |
...
Folder: `app-ui/09-intelligence/ai-risk-scores/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/ai_risk_scores_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-intelligence/ai-risk-scores`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-054`:

### Phase 0 — Claim the screen

1. Confirm `S-054` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/09-intelligence/ai-risk-scores` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `09_intelligence.ai_risk_scores` only — no hard-coded user strings.
...
23. Mark `S-054` `status: done`, `backend: paired` (or `none` with static evidence only).
