# Implementation prompt — `09-intelligence/alerts-worklists-engine`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Alerts & worklists engine** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Routes intelligence to surfaces_
...
| Screen slug | `alerts-worklists-engine` |
| Chronology | `S-055` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-intelligence/alerts-worklists-engine` |
| Parent route | `/09-intelligence/ingest-pipeline` |
...
| Localization prefix | `09_intelligence.alerts_worklists_engine` |
...
Folder: `app-ui/09-intelligence/alerts-worklists-engine/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/alerts_worklists_engine_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-intelligence/alerts-worklists-engine`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-055`:

### Phase 0 — Claim the screen

1. Confirm `S-055` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save routing → `/09-intelligence/alerts-worklists-engine?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/09-intelligence/alerts-worklists-engine` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `09_intelligence.alerts_worklists_engine` only — no hard-coded user strings.
...
23. Mark `S-055` `status: done`, `backend: paired` (or `none` with static evidence only).
