# Implementation prompt — `09-intelligence/climate-fusion`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Climate fusion** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Rainfall · heat · floods × health_
...
| Screen slug | `climate-fusion` |
| Chronology | `S-053` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-intelligence/climate-fusion` |
...
| Localization prefix | `09_intelligence.climate_fusion` |
...
Folder: `app-ui/09-intelligence/climate-fusion/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/climate_fusion_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-intelligence/climate-fusion`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-053`:

### Phase 0 — Claim the screen

1. Confirm `S-053` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/09-intelligence/climate-fusion` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `09_intelligence.climate_fusion` only — no hard-coded user strings.
...
23. Mark `S-053` `status: done`, `backend: paired` (or `none` with static evidence only).
