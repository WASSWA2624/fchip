# Implementation prompt — `09-intelligence/clinical-support-guidance`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Clinical support guidance** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Explainable next steps for CHW · facility — not Phase-4 CDS/EMR_
...
| --- | --- |
| Kind | Screen (`detail`) |
| Module | `09-intelligence` |
| Screen slug | `clinical-support-guidance` |
| Chronology | `S-056` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-intelligence/clinical-support-guidance` |
| Parent route | `/09-intelligence/clinical-support-guidance` |
...
| Localization prefix | `09_intelligence.clinical_support_guidance` |
...
Folder: `app-ui/09-intelligence/clinical-support-guidance/`
...
- `app-ui/09-intelligence/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/intelligence/` |
| Page | `frontend/lib/features/intelligence/presentation/pages/clinical_support_guidance_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/intelligence/presentation/widgets/` |
| Presentation state | `frontend/lib/features/intelligence/presentation/state/` |
| Domain | `frontend/lib/features/intelligence/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-intelligence/clinical-support-guidance`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-056`:

### Phase 0 — Claim the screen

1. Confirm `S-056` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Ingest → `/09-intelligence/ingest-pipeline`; AI → `/09-intelligence/ai-risk-scores`; GIS → `/09-intelligence/gis-explorer`; Guidance → `/09-intelligence/clinical-support-guidance`
   - Primary action: Push to CHW worklist → `/09-intelligence/clinical-support-guidance`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/09-intelligence/clinical-support-guidance` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `09_intelligence.clinical_support_guidance` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/intelligence/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/intelligence/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-056` `status: done`, `backend: paired` (or `none` with static evidence only).
