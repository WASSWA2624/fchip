# Implementation prompt — `09-intelligence/feeder-health-board`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Feeder health board** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Which data parties are sending clean signals_
...
| Screen slug | `feeder-health-board` |
| Chronology | `S-028` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-intelligence/feeder-health-board` |
| Parent route | `/09-intelligence/ingest-pipeline` |
| Layout | `connector-status` |
| Body layouts | `mobile` → `connector-status`, `tablet` → `connector-status`, `desktop` → `dual-pane-desktop` |
...
| Localization prefix | `09_intelligence.feeder_health_board` |
...
Folder: `app-ui/09-intelligence/feeder-health-board/`
...
- Shared layout specimens for `connector-status` under `app-ui/00-shared/layouts/connector-status/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/feeder_health_board_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-intelligence/feeder-health-board`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `connector-status`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-028`:

### Phase 0 — Claim the screen

1. Confirm `S-028` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `connector-status` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   ...
   - Primary action: Open silent feeders → `/09-intelligence/ingest-pipeline`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/09-intelligence/feeder-health-board` with the access guard for roles `intelligence-analyst`, `platform-operator`.
9. Compose the page from shared shells + `connector-status` + catalog components. Prefer:
...
11. Add ARB keys under `09_intelligence.feeder_health_board` only — no hard-coded user strings.
...
23. Mark `S-028` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `connector-status` is a shared layout implementation, not a one-off Scaffold tree
