# Implementation prompt — `09-intelligence/ingest-pipeline`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Ingest & sync** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: All feeder parties → validate → store_
...
| Screen slug | `ingest-pipeline` |
| Chronology | `S-027` |
| Slice | `VS-09` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/09-intelligence/ingest-pipeline` |
...
| Localization prefix | `09_intelligence.ingest_pipeline` |
...
Folder: `app-ui/09-intelligence/ingest-pipeline/`
...
| Page | `frontend/lib/features/intelligence/presentation/pages/ingest_pipeline_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/09-intelligence/ingest-pipeline`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-027`:

### Phase 0 — Claim the screen

1. Confirm `S-027` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/09-intelligence/ingest-pipeline` with the access guard for roles `intelligence-analyst`, `platform-operator`.
...
11. Add ARB keys under `09_intelligence.ingest_pipeline` only — no hard-coded user strings.
...
23. Mark `S-027` `status: done`, `backend: paired` (or `none` with static evidence only).
