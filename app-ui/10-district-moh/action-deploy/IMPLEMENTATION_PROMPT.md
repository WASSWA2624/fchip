# Implementation prompt — `10-district-moh/action-deploy`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Deploy action** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Testing · stock · outreach_
...
| Screen slug | `action-deploy` |
| Chronology | `S-059` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-district-moh/action-deploy` |
...
| Localization prefix | `10_district_moh.action_deploy` |
...
Folder: `app-ui/10-district-moh/action-deploy/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/action_deploy_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-district-moh/action-deploy`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-059`:

### Phase 0 — Claim the screen

1. Confirm `S-059` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Launch deployment → `/10-district-moh/cascade-planning?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-district-moh/action-deploy` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_district_moh.action_deploy` only — no hard-coded user strings.
...
23. Mark `S-059` `status: done`, `backend: paired` (or `none` with static evidence only).
