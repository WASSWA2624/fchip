# Implementation prompt — `10-district-moh/early-warnings`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Early warnings** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Explainable outbreak & risk notices_
...
| Screen slug | `early-warnings` |
| Chronology | `S-058` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-district-moh/early-warnings` |
| Parent route | `/10-district-moh/early-warnings` |
...
| Localization prefix | `10_district_moh.early_warnings` |
...
Folder: `app-ui/10-district-moh/early-warnings/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/early_warnings_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-district-moh/early-warnings`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-058`:

### Phase 0 — Claim the screen

1. Confirm `S-058` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Deploy response → `/10-district-moh/action-deploy`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-district-moh/early-warnings` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_district_moh.early_warnings` only — no hard-coded user strings.
...
23. Mark `S-058` `status: done`, `backend: paired` (or `none` with static evidence only).
