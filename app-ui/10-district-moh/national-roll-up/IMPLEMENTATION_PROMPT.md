# Implementation prompt — `10-district-moh/national-roll-up`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **MoH national roll-up** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Phase 3 — multi-district planning · outbreak preparedness_
...
| Module | `10-district-moh` |
| Screen slug | `national-roll-up` |
| Chronology | `S-123` |
| Slice | `VS-10` |
| Phase | `phase-3` |
| Deferred | `true` |
| Chronology status | `not-started` |
| Route | `/10-district-moh/national-roll-up` |
...
| Localization prefix | `10_district_moh.national_roll_up` |
...
Folder: `app-ui/10-district-moh/national-roll-up/`
...
- `app-ui/10-district-moh/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/district_moh/` |
| Page | `frontend/lib/features/district_moh/presentation/pages/national_roll_up_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/district_moh/presentation/widgets/` |
| Presentation state | `frontend/lib/features/district_moh/presentation/state/` |
| Domain | `frontend/lib/features/district_moh/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-district-moh/national-roll-up`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-123`:

### Phase 0 — Claim the screen

1. Confirm `S-123` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Open district drill-down → `/10-district-moh/population-map`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-district-moh/national-roll-up` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_district_moh.national_roll_up` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/district_moh/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/district_moh/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-123` `status: done`, `backend: paired` (or `none` with static evidence only).
