# Implementation prompt — `10-district-moh/population-map`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Population map** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: District early-warning geography_
...
| Screen slug | `population-map` |
| Chronology | `S-057` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-district-moh/population-map` |
...
| Localization prefix | `10_district_moh.population_map` |
...
Folder: `app-ui/10-district-moh/population-map/`
...
| Page | `frontend/lib/features/district_moh/presentation/pages/population_map_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-district-moh/population-map`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-057`:

### Phase 0 — Claim the screen

1. Confirm `S-057` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/10-district-moh/population-map` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_district_moh.population_map` only — no hard-coded user strings.
...
23. Mark `S-057` `status: done`, `backend: paired` (or `none` with static evidence only).
