# Implementation prompt — `10-outreach-school-health/01-campaign-planner`
...
| Screen slug | `01-campaign-planner` |
| Chronology | `S-065` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-outreach-school-health/01-campaign-planner` |
...
| Localization prefix | `10_outreach_school_health.01_campaign_planner` |
...
Folder: `app-ui/10-outreach-school-health/01-campaign-planner/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/01_campaign_planner_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/01-campaign-planner`) |
...
7. Map navigation:
   ...
   - Primary action: Create campaign → `/08-district-moh/01-population-map`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-outreach-school-health/01-campaign-planner` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_outreach_school_health.01_campaign_planner` only — no hard-coded user strings.
