# Implementation prompt — `10-outreach-school-health/02-session-log`
...
| Screen slug | `02-session-log` |
| Chronology | `S-066` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-outreach-school-health/02-session-log` |
...
| Localization prefix | `10_outreach_school_health.02_session_log` |
...
Folder: `app-ui/10-outreach-school-health/02-session-log/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/02_session_log_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/02-session-log`) |
...
7. Map navigation:
   ...
   - Primary action: Save session → `/10-outreach-school-health/02-session-log?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-outreach-school-health/02-session-log` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_outreach_school_health.02_session_log` only — no hard-coded user strings.
