# Implementation prompt — `10-outreach-school-health/03-coverage-map`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Coverage map** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Where programmes ran_
...
| Screen slug | `03-coverage-map` |
| Chronology | `S-067` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-outreach-school-health/03-coverage-map` |
| Parent route | `/08-district-moh/01-population-map` |
| Layout | `12-map-explorer` |
| Body layouts | `mobile` → `12-map-explorer`, `tablet` → `12-map-explorer`, `desktop` → `12-map-explorer` |
...
| Localization prefix | `10_outreach_school_health.03_coverage_map` |
...
Folder: `app-ui/10-outreach-school-health/03-coverage-map/`
...
- Shared layout specimens for `12-map-explorer` under `app-ui/00-shared/layouts/12-map-explorer/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/03_coverage_map_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/03-coverage-map`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `12-map-explorer`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-067`:

### Phase 0 — Claim the screen

1. Confirm `S-067` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `12-map-explorer` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
...
8. Register route `/10-outreach-school-health/03-coverage-map` with the access guard for roles `district-health-officer`, `moh`.
9. Compose the page from shared shells + `12-map-explorer` + catalog components. Prefer:
...
11. Add ARB keys under `10_outreach_school_health.03_coverage_map` only — no hard-coded user strings.
...
23. Mark `S-067` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `12-map-explorer` is a shared layout implementation, not a one-off Scaffold tree
