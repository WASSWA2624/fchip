# Implementation prompt — `10-outreach-school-health/04-screening-results-entry`
...
| Screen slug | `04-screening-results-entry` |
| Chronology | `S-068` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-outreach-school-health/04-screening-results-entry` |
...
| Localization prefix | `10_outreach_school_health.04_screening_results_entry` |
...
Folder: `app-ui/10-outreach-school-health/04-screening-results-entry/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/04_screening_results_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/04-screening-results-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Queue to ingest → `/10-outreach-school-health/04-screening-results-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-outreach-school-health/04-screening-results-entry` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_outreach_school_health.04_screening_results_entry` only — no hard-coded user strings.
