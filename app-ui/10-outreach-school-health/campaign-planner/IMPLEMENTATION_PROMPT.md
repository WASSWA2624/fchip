# Implementation prompt — `10-outreach-school-health/campaign-planner`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Campaign planner** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Outreach · screening · school health_
...
| Module | `10-outreach-school-health` |
| Screen slug | `campaign-planner` |
| Chronology | `S-065` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-outreach-school-health/campaign-planner` |
| Parent route | `/08-district-moh/population-map` |
...
| Localization prefix | `10_outreach_school_health.campaign_planner` |
...
Folder: `app-ui/10-outreach-school-health/campaign-planner/`
...
- `app-ui/10-outreach-school-health/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/outreach_school_health/` |
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/campaign_planner_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/outreach_school_health/presentation/widgets/` |
| Presentation state | `frontend/lib/features/outreach_school_health/presentation/state/` |
| Domain | `frontend/lib/features/outreach_school_health/domain/` |
| Data / repository impl | `frontend/lib/features/outreach_school_health/data/` |
| Repository contract | `frontend/lib/features/outreach_school_health/domain/repositories/outreach_school_health_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/campaign-planner`) |
...
| Tests | `frontend/test/features/outreach_school_health/` |
| Backend module | `backend/src/modules/outreach_school_health/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-065`:

### Phase 0 — Claim the screen

1. Confirm `S-065` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Map → `/08-district-moh/population-map`; Warnings → `/08-district-moh/early-warnings`; Metrics → `/03-cascade-metrics/indicators-overview`; Plan → `/08-district-moh/cascade-planning`
   - Primary action: Create campaign → `/08-district-moh/population-map`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-outreach-school-health/campaign-planner` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_outreach_school_health.campaign_planner` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/outreach_school_health/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/outreach_school_health/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-065` `status: done`, `backend: paired` (or `none` with static evidence only).
