# Implementation prompt — `10-outreach-school-health/coverage-map`
...
| Route | `/10-outreach-school-health/coverage-map` |
...
| Localization prefix | `10_outreach_school_health.coverage_map` |
...
Folder: `app-ui/10-outreach-school-health/coverage-map/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/coverage-map`) |
...
8. Register route `/10-outreach-school-health/coverage-map` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_outreach_school_health.coverage_map` only — no hard-coded user strings.
