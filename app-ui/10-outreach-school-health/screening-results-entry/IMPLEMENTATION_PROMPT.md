# Implementation prompt — `10-outreach-school-health/screening-results-entry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Screening results entry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Outreach teams feed camp outcomes into ingest_
...
| Screen slug | `screening-results-entry` |
| Chronology | `S-068` |
| Slice | `VS-10` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/10-outreach-school-health/screening-results-entry` |
...
| Localization prefix | `10_outreach_school_health.screening_results_entry` |
...
Folder: `app-ui/10-outreach-school-health/screening-results-entry/`
...
| Page | `frontend/lib/features/outreach_school_health/presentation/pages/screening_results_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/10-outreach-school-health/screening-results-entry`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-068`:

### Phase 0 — Claim the screen

1. Confirm `S-068` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Queue to ingest → `/10-outreach-school-health/screening-results-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/10-outreach-school-health/screening-results-entry` with the access guard for roles `district-health-officer`, `moh`.
...
11. Add ARB keys under `10_outreach_school_health.screening_results_entry` only — no hard-coded user strings.
...
23. Mark `S-068` `status: done`, `backend: paired` (or `none` with static evidence only).
