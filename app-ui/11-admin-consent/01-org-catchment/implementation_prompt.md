# Implementation prompt — `11-admin-consent/01-org-catchment`
...
| Screen slug | `01-org-catchment` |
| Chronology | `S-070` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/01-org-catchment` |
...
| Localization prefix | `11_admin_consent.01_org_catchment` |
...
Folder: `app-ui/11-admin-consent/01-org-catchment/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/01_org_catchment_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/01-org-catchment`) |
...
7. Map navigation:
   ...
   - Primary action: Save org → `/11-admin-consent/01-org-catchment?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/01-org-catchment` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.01_org_catchment` only — no hard-coded user strings.
