# Implementation prompt — `11-admin-consent/02-users-roles`
...
| Screen slug | `02-users-roles` |
| Chronology | `S-071` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/02-users-roles` |
| Parent route | `/11-admin-consent/02-users-roles` |
...
| Localization prefix | `11_admin_consent.02_users_roles` |
...
Folder: `app-ui/11-admin-consent/02-users-roles/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/02_users_roles_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/02-users-roles`) |
...
7. Map navigation:
   ...
   - Primary action: Invite user → `/11-admin-consent/02-users-roles?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/02-users-roles` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.02_users_roles` only — no hard-coded user strings.
