# Implementation prompt — `11-admin-consent/03-consent-privacy`
...
| Screen slug | `03-consent-privacy` |
| Chronology | `S-072` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/03-consent-privacy` |
| Parent route | `/11-admin-consent/03-consent-privacy` |
...
| Localization prefix | `11_admin_consent.03_consent_privacy` |
...
Folder: `app-ui/11-admin-consent/03-consent-privacy/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/03_consent_privacy_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/03-consent-privacy`) |
...
7. Map navigation:
   ...
   - Primary action: Update policy → `/11-admin-consent/03-consent-privacy?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/03-consent-privacy` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.03_consent_privacy` only — no hard-coded user strings.
