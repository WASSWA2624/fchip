# Implementation prompt — `11-admin-consent/04-emr-api-access`
...
| Screen slug | `04-emr-api-access` |
| Chronology | `S-073` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/04-emr-api-access` |
...
| Localization prefix | `11_admin_consent.04_emr_api_access` |
...
Folder: `app-ui/11-admin-consent/04-emr-api-access/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/04_emr_api_access_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/04-emr-api-access`) |
...
7. Map navigation:
   ...
   - Primary action: Issue key → `/11-admin-consent/04-emr-api-access?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/04-emr-api-access` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.04_emr_api_access` only — no hard-coded user strings.
