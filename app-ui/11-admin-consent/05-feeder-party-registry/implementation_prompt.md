# Implementation prompt — `11-admin-consent/05-feeder-party-registry`
...
| Screen slug | `05-feeder-party-registry` |
| Chronology | `S-074` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/05-feeder-party-registry` |
| Parent route | `/11-admin-consent/01-org-catchment` |
| Layout | `07-settings-admin` |
| Body layouts | `mobile` → `07-settings-admin`, `tablet` → `07-settings-admin`, `desktop` → `07-settings-admin` |
...
| Localization prefix | `11_admin_consent.05_feeder_party_registry` |
...
Folder: `app-ui/11-admin-consent/05-feeder-party-registry/`
...
- Shared layout specimens for `07-settings-admin` under `app-ui/00-shared/layouts/07-settings-admin/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/05_feeder_party_registry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/05-feeder-party-registry`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `07-settings-admin`) |
...
6. Resolve `07-settings-admin` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   ...
   - Primary action: Add feeder party → `/11-admin-consent/05-feeder-party-registry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/05-feeder-party-registry` with the access guard for roles `organisation-admin`, `privacy-admin`.
9. Compose the page from shared shells + `07-settings-admin` + catalog components. Prefer:
...
11. Add ARB keys under `11_admin_consent.05_feeder_party_registry` only — no hard-coded user strings.
...
- [ ] Layout `07-settings-admin` is a shared layout implementation, not a one-off Scaffold tree
