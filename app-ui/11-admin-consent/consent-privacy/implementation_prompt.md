# Implementation prompt — `11-admin-consent/consent-privacy`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Consent & privacy** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Household consent · least privilege_
...
| Screen slug | `consent-privacy` |
| Chronology | `S-072` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/consent-privacy` |
| Parent route | `/11-admin-consent/consent-privacy` |
...
| Localization prefix | `11_admin_consent.consent_privacy` |
...
Folder: `app-ui/11-admin-consent/consent-privacy/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/consent_privacy_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/consent-privacy`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-072`:

### Phase 0 — Claim the screen

1. Confirm `S-072` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Update policy → `/11-admin-consent/consent-privacy?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/consent-privacy` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.consent_privacy` only — no hard-coded user strings.
...
23. Mark `S-072` `status: done`, `backend: paired` (or `none` with static evidence only).
