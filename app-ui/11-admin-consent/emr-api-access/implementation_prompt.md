# Implementation prompt — `11-admin-consent/emr-api-access`
...
| Route | `/11-admin-consent/emr-api-access` |
...
| Localization prefix | `11_admin_consent.emr_api_access` |
...
Folder: `app-ui/11-admin-consent/emr-api-access/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/emr-api-access`) |
...
7. Map navigation:
   ...
   - Primary action: Issue key → `/11-admin-consent/emr-api-access?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/emr-api-access` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.emr_api_access` only — no hard-coded user strings.
