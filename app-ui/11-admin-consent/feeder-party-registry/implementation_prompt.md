# Implementation prompt — `11-admin-consent/feeder-party-registry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Feeder party registry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Register every source that may push into ingest_

## Identity

| Field | Value |
| --- | --- |
| Kind | Screen (`settings`) |
| Module | `11-admin-consent` |
| Screen slug | `feeder-party-registry` |
| Chronology | `S-074` |
| Slice | `VS-11` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-admin-consent/feeder-party-registry` |
| Parent route | `/11-admin-consent/org-catchment` |
| Layout | `settings-admin` |
| Body layouts | `mobile` → `settings-admin`, `tablet` → `settings-admin`, `desktop` → `settings-admin` |
...
| Localization prefix | `11_admin_consent.feeder_party_registry` |
...
Folder: `app-ui/11-admin-consent/feeder-party-registry/`
...
- `app-ui/11-admin-consent/README.md`
- `app-ui/00-shared/tokens.json`
- Shared layout specimens for `settings-admin` under `app-ui/00-shared/layouts/settings-admin/`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/admin_consent/` |
| Page | `frontend/lib/features/admin_consent/presentation/pages/feeder_party_registry_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/admin_consent/presentation/widgets/` |
| Presentation state | `frontend/lib/features/admin_consent/presentation/state/` |
| Domain | `frontend/lib/features/admin_consent/domain/` |
| Data / repository impl | `frontend/lib/features/admin_consent/data/` |
| Repository contract | `frontend/lib/features/admin_consent/domain/repositories/admin_consent_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/feeder-party-registry`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `settings-admin`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/admin_consent/` |
| Backend module | `backend/src/modules/admin_consent/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-074`:

### Phase 0 — Claim the screen

1. Confirm `S-074` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `settings-admin` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   - Tabs: Org → `/11-admin-consent/org-catchment`; Users → `/11-admin-consent/users-roles`; Consent → `/11-admin-consent/consent-privacy`; APIs → `/06-emr-connector/connector-status`
   - Primary action: Add feeder party → `/11-admin-consent/feeder-party-registry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/feeder-party-registry` with the access guard for roles `organisation-admin`, `privacy-admin`.
9. Compose the page from shared shells + `settings-admin` + catalog components. Prefer:
...
11. Add ARB keys under `11_admin_consent.feeder_party_registry` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/admin_consent/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/admin_consent/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-074` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `settings-admin` is a shared layout implementation, not a one-off Scaffold tree
