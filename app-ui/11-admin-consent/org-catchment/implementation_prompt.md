# Implementation prompt — `11-admin-consent/org-catchment`
...
| Route | `/11-admin-consent/org-catchment` |
...
| Localization prefix | `11_admin_consent.org_catchment` |
...
Folder: `app-ui/11-admin-consent/org-catchment/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/org-catchment`) |
...
7. Map navigation:
   ...
   - Primary action: Save org → `/11-admin-consent/org-catchment?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/org-catchment` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.org_catchment` only — no hard-coded user strings.
