# Implementation prompt — `11-admin-consent/users-roles`
...
| Route | `/11-admin-consent/users-roles` |
| Parent route | `/11-admin-consent/users-roles` |
...
| Localization prefix | `11_admin_consent.users_roles` |
...
Folder: `app-ui/11-admin-consent/users-roles/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-admin-consent/users-roles`) |
...
7. Map navigation:
   ...
   - Primary action: Invite user → `/11-admin-consent/users-roles?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-admin-consent/users-roles` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `11_admin_consent.users_roles` only — no hard-coded user strings.
