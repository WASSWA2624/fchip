# Implementation prompt — `11-ngo-partner/field-dataset-upload`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Field dataset upload** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: NGO M&E teams feed anonymised programme data_
...
| Screen slug | `field-dataset-upload` |
| Chronology | `S-085` |
| Slice | `VS-11` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-ngo-partner/field-dataset-upload` |
...
| Localization prefix | `11_ngo_partner.field_dataset_upload` |
...
Folder: `app-ui/11-ngo-partner/field-dataset-upload/`
...
| Page | `frontend/lib/features/ngo_partner/presentation/pages/field_dataset_upload_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-ngo-partner/field-dataset-upload`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-085`:

### Phase 0 — Claim the screen

1. Confirm `S-085` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Upload to ingest → `/11-ngo-partner/field-dataset-upload?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-ngo-partner/field-dataset-upload` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `11_ngo_partner.field_dataset_upload` only — no hard-coded user strings.
...
23. Mark `S-085` `status: done`, `backend: paired` (or `none` with static evidence only).
