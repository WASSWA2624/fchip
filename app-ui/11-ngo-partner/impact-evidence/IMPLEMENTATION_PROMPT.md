# Implementation prompt — `11-ngo-partner/impact-evidence`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Impact evidence** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Optimisation · donor-ready proof_
...
| Screen slug | `impact-evidence` |
| Chronology | `S-083` |
| Slice | `VS-11` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-ngo-partner/impact-evidence` |
| Parent route | `/11-ngo-partner/impact-evidence` |
...
| Localization prefix | `11_ngo_partner.impact_evidence` |
...
Folder: `app-ui/11-ngo-partner/impact-evidence/`
...
| Page | `frontend/lib/features/ngo_partner/presentation/pages/impact_evidence_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-ngo-partner/impact-evidence`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-083`:

### Phase 0 — Claim the screen

1. Confirm `S-083` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Export brief → `/11-ngo-partner/impact-evidence`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-ngo-partner/impact-evidence` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `11_ngo_partner.impact_evidence` only — no hard-coded user strings.
...
23. Mark `S-083` `status: done`, `backend: paired` (or `none` with static evidence only).
