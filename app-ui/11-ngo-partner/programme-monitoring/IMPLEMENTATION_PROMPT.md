# Implementation prompt — `11-ngo-partner/programme-monitoring`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Programme monitoring** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Partner-scoped real-time M&E_
...
| Screen slug | `programme-monitoring` |
| Chronology | `S-082` |
| Slice | `VS-11` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-ngo-partner/programme-monitoring` |
...
| Localization prefix | `11_ngo_partner.programme_monitoring` |
...
Folder: `app-ui/11-ngo-partner/programme-monitoring/`
...
| Page | `frontend/lib/features/ngo_partner/presentation/pages/programme_monitoring_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-ngo-partner/programme-monitoring`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-082`:

### Phase 0 — Claim the screen

1. Confirm `S-082` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/11-ngo-partner/programme-monitoring` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `11_ngo_partner.programme_monitoring` only — no hard-coded user strings.
...
23. Mark `S-082` `status: done`, `backend: paired` (or `none` with static evidence only).
