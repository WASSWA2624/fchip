# Implementation prompt — `11-ngo-partner/training-skills-analytics`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Training · skills analytics** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: §2.4 research · partnerships · skills capacity_
...
| Module | `11-ngo-partner` |
| Screen slug | `training-skills-analytics` |
| Chronology | `S-084` |
| Slice | `VS-11` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/11-ngo-partner/training-skills-analytics` |
| Parent route | `/11-ngo-partner/training-skills-analytics` |
...
| Localization prefix | `11_ngo_partner.training_skills_analytics` |
...
Folder: `app-ui/11-ngo-partner/training-skills-analytics/`
...
- `app-ui/11-ngo-partner/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/ngo_partner/` |
| Page | `frontend/lib/features/ngo_partner/presentation/pages/training_skills_analytics_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/ngo_partner/presentation/widgets/` |
| Presentation state | `frontend/lib/features/ngo_partner/presentation/state/` |
| Domain | `frontend/lib/features/ngo_partner/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/11-ngo-partner/training-skills-analytics`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-084`:

### Phase 0 — Claim the screen

1. Confirm `S-084` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Export training brief → `/11-ngo-partner/training-skills-analytics`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/11-ngo-partner/training-skills-analytics` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `11_ngo_partner.training_skills_analytics` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/ngo_partner/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/ngo_partner/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-084` `status: done`, `backend: paired` (or `none` with static evidence only).
