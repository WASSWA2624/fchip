# Implementation prompt — `12-insurance-insights/01-prevention-overview`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Prevention overview** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Insurance partners — population prevention insights_

## Identity

| Field | Value |
| --- | --- |
| Kind | Screen (`dashboard`) |
| Module | `12-insurance-insights` |
| Screen slug | `01-prevention-overview` |
| Chronology | `S-075` |
| Slice | `VS-12` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/12-insurance-insights/01-prevention-overview` |
| Parent route | `/12-insurance-insights/01-prevention-overview` |
| Layout | `17-insurance-prevention` |
| Body layouts | `mobile` → `17-insurance-prevention`, `tablet` → `17-insurance-prevention`, `desktop` → `16-dual-pane-desktop` |
| Shells | `mobile` → `03-field-mobile-shell`, `tablet` → `04-field-tablet-shell`, `desktop` → `05-desktop-sidebar-shell` |
| Roles | `insurance-analyst` |
| Supported states | `default`, `loading`, `empty`, `error`, `forbidden` |
| Localization prefix | `12_insurance_insights.01_prevention_overview` |
...
Folder: `app-ui/12-insurance-insights/01-prevention-overview/`
...
- `app-ui/12-insurance-insights/README.md`
- `app-ui/00-shared/tokens.json`
- Shared layout specimens for `17-insurance-prevention` under `app-ui/00-shared/layouts/17-insurance-prevention/`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/insurance_insights/` |
| Page | `frontend/lib/features/insurance_insights/presentation/pages/01_prevention_overview_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/insurance_insights/presentation/widgets/` |
| Presentation state | `frontend/lib/features/insurance_insights/presentation/state/` |
| Domain | `frontend/lib/features/insurance_insights/domain/` |
| Data / repository impl | `frontend/lib/features/insurance_insights/data/` |
| Repository contract | `frontend/lib/features/insurance_insights/domain/repositories/insurance_insights_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/12-insurance-insights/01-prevention-overview`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `17-insurance-prevention`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/insurance_insights/` |
| Backend module | `backend/src/modules/insurance_insights/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-075`:

### Phase 0 — Claim the screen

1. Confirm `S-075` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `17-insurance-prevention` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   - Tabs: Prevent → `/12-insurance-insights/01-prevention-overview`; Cohorts → `/12-insurance-insights/02-risk-cohort-insights`; Trends → `/12-insurance-insights/03-anonymised-trends`; More → `/00-shared/09-notifications-center`
   - Primary action: Open risk cohorts → `/12-insurance-insights/01-prevention-overview`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/12-insurance-insights/01-prevention-overview` with the access guard for roles `insurance-analyst`.
9. Compose the page from shared shells + `17-insurance-prevention` + catalog components. Prefer:
...
11. Add ARB keys under `12_insurance_insights.01_prevention_overview` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/insurance_insights/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/insurance_insights/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-075` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `17-insurance-prevention` is a shared layout implementation, not a one-off Scaffold tree
