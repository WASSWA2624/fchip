# Implementation prompt — `12-insurance-insights/03-anonymised-trends`
...
| Screen slug | `03-anonymised-trends` |
| Chronology | `S-077` |
| Slice | `VS-12` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/12-insurance-insights/03-anonymised-trends` |
| Parent route | `/12-insurance-insights/03-anonymised-trends` |
...
| Localization prefix | `12_insurance_insights.03_anonymised_trends` |
...
Folder: `app-ui/12-insurance-insights/03-anonymised-trends/`
...
| Page | `frontend/lib/features/insurance_insights/presentation/pages/03_anonymised_trends_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/12-insurance-insights/03-anonymised-trends`) |
...
7. Map navigation:
   ...
   - Primary action: Schedule partner report → `/12-insurance-insights/03-anonymised-trends`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/12-insurance-insights/03-anonymised-trends` with the access guard for roles `insurance-analyst`.
...
11. Add ARB keys under `12_insurance_insights.03_anonymised_trends` only — no hard-coded user strings.
