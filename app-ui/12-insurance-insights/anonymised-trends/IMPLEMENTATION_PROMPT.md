# Implementation prompt — `12-insurance-insights/anonymised-trends`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Anonymised trends** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Prevention-focused population health trends_
...
| Screen slug | `anonymised-trends` |
| Chronology | `S-077` |
| Slice | `VS-12` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/12-insurance-insights/anonymised-trends` |
| Parent route | `/12-insurance-insights/anonymised-trends` |
...
| Localization prefix | `12_insurance_insights.anonymised_trends` |
...
Folder: `app-ui/12-insurance-insights/anonymised-trends/`
...
| Page | `frontend/lib/features/insurance_insights/presentation/pages/anonymised_trends_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/12-insurance-insights/anonymised-trends`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-077`:

### Phase 0 — Claim the screen

1. Confirm `S-077` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Schedule partner report → `/12-insurance-insights/anonymised-trends`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/12-insurance-insights/anonymised-trends` with the access guard for roles `insurance-analyst`.
...
11. Add ARB keys under `12_insurance_insights.anonymised_trends` only — no hard-coded user strings.
...
23. Mark `S-077` `status: done`, `backend: paired` (or `none` with static evidence only).
