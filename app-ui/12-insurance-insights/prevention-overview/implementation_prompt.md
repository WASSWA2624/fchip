# Implementation prompt — `12-insurance-insights/prevention-overview`
...
| Route | `/12-insurance-insights/prevention-overview` |
| Parent route | `/12-insurance-insights/prevention-overview` |
...
| Localization prefix | `12_insurance_insights.prevention_overview` |
...
Folder: `app-ui/12-insurance-insights/prevention-overview/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/12-insurance-insights/prevention-overview`) |
...
7. Map navigation:
   - Tabs: Prevent → `/12-insurance-insights/prevention-overview`; Cohorts → `/12-insurance-insights/risk-cohort-insights`; Trends → `/12-insurance-insights/anonymised-trends`; More → `/00-shared/notifications-center`
   - Primary action: Open risk cohorts → `/12-insurance-insights/prevention-overview`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/12-insurance-insights/prevention-overview` with the access guard for roles `insurance-analyst`.
...
11. Add ARB keys under `12_insurance_insights.prevention_overview` only — no hard-coded user strings.
