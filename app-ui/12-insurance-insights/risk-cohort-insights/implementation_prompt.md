# Implementation prompt — `12-insurance-insights/risk-cohort-insights`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Risk cohort insights** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Anonymised cohorts for prevention planning_
...
| Screen slug | `risk-cohort-insights` |
| Chronology | `S-076` |
| Slice | `VS-12` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/12-insurance-insights/risk-cohort-insights` |
| Parent route | `/12-insurance-insights/risk-cohort-insights` |
...
| Localization prefix | `12_insurance_insights.risk_cohort_insights` |
...
Folder: `app-ui/12-insurance-insights/risk-cohort-insights/`
...
| Page | `frontend/lib/features/insurance_insights/presentation/pages/risk_cohort_insights_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/12-insurance-insights/risk-cohort-insights`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-076`:

### Phase 0 — Claim the screen

1. Confirm `S-076` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Export anonymised brief → `/12-insurance-insights/risk-cohort-insights`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/12-insurance-insights/risk-cohort-insights` with the access guard for roles `insurance-analyst`.
...
11. Add ARB keys under `12_insurance_insights.risk_cohort_insights` only — no hard-coded user strings.
...
23. Mark `S-076` `status: done`, `backend: paired` (or `none` with static evidence only).
