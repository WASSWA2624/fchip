# Implementation prompt — `12-research-exports/evidence-catalog`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Evidence catalog** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Anonymised datasets · no raw PHI dumps_
...
| Screen slug | `evidence-catalog` |
| Chronology | `S-124` |
| Slice | `VS-12` |
| Phase | `phase-4` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/12-research-exports/evidence-catalog` |
...
| Localization prefix | `12_research_exports.evidence_catalog` |
...
Folder: `app-ui/12-research-exports/evidence-catalog/`
...
| Page | `frontend/lib/features/research_exports/presentation/pages/evidence_catalog_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/12-research-exports/evidence-catalog`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-124`:

### Phase 0 — Claim the screen

1. Confirm `S-124` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/12-research-exports/evidence-catalog` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `12_research_exports.evidence_catalog` only — no hard-coded user strings.
...
23. Mark `S-124` `status: done`, `backend: paired` (or `none` with static evidence only).
