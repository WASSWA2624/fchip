# Implementation prompt — `12-research-exports/export-request`
...
| Route | `/12-research-exports/export-request` |
...
| Localization prefix | `12_research_exports.export_request` |
...
Folder: `app-ui/12-research-exports/export-request/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/12-research-exports/export-request`) |
...
7. Map navigation:
   ...
   - Primary action: Submit request → `/12-research-exports/export-pending`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/12-research-exports/export-request` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `12_research_exports.export_request` only — no hard-coded user strings.
