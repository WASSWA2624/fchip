# Implementation prompt — `13-admin-consent/consent-privacy`
...
| Route | `/13-admin-consent/consent-privacy` |
| Parent route | `/13-admin-consent/consent-privacy` |
...
| Localization prefix | `13_admin_consent.consent_privacy` |
...
Folder: `app-ui/13-admin-consent/consent-privacy/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-admin-consent/consent-privacy`) |
...
7. Map navigation:
   ...
   - Primary action: Update policy → `/13-admin-consent/consent-privacy?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/13-admin-consent/consent-privacy` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `13_admin_consent.consent_privacy` only — no hard-coded user strings.
