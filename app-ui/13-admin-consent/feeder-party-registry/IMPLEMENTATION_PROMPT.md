# Implementation prompt — `13-admin-consent/feeder-party-registry`
...
| Route | `/13-admin-consent/feeder-party-registry` |
| Parent route | `/13-admin-consent/org-catchment` |
...
| Localization prefix | `13_admin_consent.feeder_party_registry` |
...
Folder: `app-ui/13-admin-consent/feeder-party-registry/`
...
- `app-ui/13-admin-consent/README.md`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-admin-consent/feeder-party-registry`) |
...
7. Map navigation:
   - Tabs: Org → `/13-admin-consent/org-catchment`; Users → `/13-admin-consent/users-roles`; Consent → `/13-admin-consent/consent-privacy`; APIs → `/08-emr-connector/connector-status`
   - Primary action: Add feeder party → `/13-admin-consent/feeder-party-registry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/13-admin-consent/feeder-party-registry` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `13_admin_consent.feeder_party_registry` only — no hard-coded user strings.
