# Implementation prompt — `13-admin-consent/org-catchment`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Org · catchment · facilities** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Where FCHIP is deployed_
...
| Screen slug | `org-catchment` |
| Chronology | `S-070` |
| Slice | `VS-13` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/13-admin-consent/org-catchment` |
...
| Localization prefix | `13_admin_consent.org_catchment` |
...
Folder: `app-ui/13-admin-consent/org-catchment/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/org_catchment_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-admin-consent/org-catchment`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-070`:

### Phase 0 — Claim the screen

1. Confirm `S-070` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save org → `/13-admin-consent/org-catchment?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/13-admin-consent/org-catchment` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `13_admin_consent.org_catchment` only — no hard-coded user strings.
...
23. Mark `S-070` `status: done`, `backend: paired` (or `none` with static evidence only).
