# Implementation prompt — `13-admin-consent/users-roles`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Users & roles** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Consumers and data-feeder roles_
...
| Screen slug | `users-roles` |
| Chronology | `S-071` |
| Slice | `VS-13` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/13-admin-consent/users-roles` |
| Parent route | `/13-admin-consent/users-roles` |
...
| Localization prefix | `13_admin_consent.users_roles` |
...
Folder: `app-ui/13-admin-consent/users-roles/`
...
| Page | `frontend/lib/features/admin_consent/presentation/pages/users_roles_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-admin-consent/users-roles`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-071`:

### Phase 0 — Claim the screen

1. Confirm `S-071` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Invite user → `/13-admin-consent/users-roles?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/13-admin-consent/users-roles` with the access guard for roles `organisation-admin`, `privacy-admin`.
...
11. Add ARB keys under `13_admin_consent.users_roles` only — no hard-coded user strings.
...
23. Mark `S-071` `status: done`, `backend: paired` (or `none` with static evidence only).
