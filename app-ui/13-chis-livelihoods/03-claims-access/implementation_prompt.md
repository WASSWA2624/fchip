# Implementation prompt — `13-chis-livelihoods/03-claims-access`
...
| Screen slug | `03-claims-access` |
| Chronology | `S-080` |
| Slice | `VS-13` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/13-chis-livelihoods/03-claims-access` |
...
| Localization prefix | `13_chis_livelihoods.03_claims_access` |
...
Folder: `app-ui/13-chis-livelihoods/03-claims-access/`
...
| Page | `frontend/lib/features/chis_livelihoods/presentation/pages/03_claims_access_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-chis-livelihoods/03-claims-access`) |
...
8. Register route `/13-chis-livelihoods/03-claims-access` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `13_chis_livelihoods.03_claims_access` only — no hard-coded user strings.
