# Implementation prompt — `13-chis-livelihoods/claims-access`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Claims / access** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Livelihood-linked care access_
...
| --- | --- |
| Kind | Screen (`detail`) |
| Module | `13-chis-livelihoods` |
| Screen slug | `claims-access` |
| Chronology | `S-080` |
| Slice | `VS-13` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/13-chis-livelihoods/claims-access` |
| Parent route | `/14-ngo-partner/programme-monitoring` |
...
| Localization prefix | `13_chis_livelihoods.claims_access` |
...
Folder: `app-ui/13-chis-livelihoods/claims-access/`
...
- `app-ui/13-chis-livelihoods/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/chis_livelihoods/` |
| Page | `frontend/lib/features/chis_livelihoods/presentation/pages/claims_access_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/chis_livelihoods/presentation/widgets/` |
| Presentation state | `frontend/lib/features/chis_livelihoods/presentation/state/` |
| Domain | `frontend/lib/features/chis_livelihoods/domain/` |
| Data / repository impl | `frontend/lib/features/chis_livelihoods/data/` |
| Repository contract | `frontend/lib/features/chis_livelihoods/domain/repositories/chis_livelihoods_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/13-chis-livelihoods/claims-access`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `detail-action`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/chis_livelihoods/` |
| Backend module | `backend/src/modules/chis_livelihoods/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-080`:

### Phase 0 — Claim the screen

1. Confirm `S-080` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/13-chis-livelihoods/claims-access` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `13_chis_livelihoods.claims_access` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/chis_livelihoods/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/chis_livelihoods/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-080` `status: done`, `backend: paired` (or `none` with static evidence only).
