# Implementation prompt — `13-chis-livelihoods/enrolment`
...
| Route | `/13-chis-livelihoods/enrolment` |
...
| Localization prefix | `13_chis_livelihoods.enrolment` |
...
Folder: `app-ui/13-chis-livelihoods/enrolment/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-chis-livelihoods/enrolment`) |
...
7. Map navigation:
   ...
   - Primary action: Save enrolment → `/13-chis-livelihoods/enrolment?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/13-chis-livelihoods/enrolment` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `13_chis_livelihoods.enrolment` only — no hard-coded user strings.
