# Implementation prompt — `13-chis-livelihoods/iga-participation-entry`
...
| Route | `/13-chis-livelihoods/iga-participation-entry` |
...
| Localization prefix | `13_chis_livelihoods.iga_participation_entry` |
...
Folder: `app-ui/13-chis-livelihoods/iga-participation-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/13-chis-livelihoods/iga-participation-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit IGA signal → `/13-chis-livelihoods/iga-participation-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/13-chis-livelihoods/iga-participation-entry` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `13_chis_livelihoods.iga_participation_entry` only — no hard-coded user strings.
