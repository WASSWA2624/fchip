# Implementation prompt — `14-ngo-partner/05-partner-indicator-entry`
...
| Screen slug | `05-partner-indicator-entry` |
| Chronology | `S-086` |
| Slice | `VS-14` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/14-ngo-partner/05-partner-indicator-entry` |
...
| Localization prefix | `14_ngo_partner.05_partner_indicator_entry` |
...
Folder: `app-ui/14-ngo-partner/05-partner-indicator-entry/`
...
| Page | `frontend/lib/features/ngo_partner/presentation/pages/05_partner_indicator_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-ngo-partner/05-partner-indicator-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit indicator → `/14-ngo-partner/05-partner-indicator-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-ngo-partner/05-partner-indicator-entry` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `14_ngo_partner.05_partner_indicator_entry` only — no hard-coded user strings.
