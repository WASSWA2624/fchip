# Implementation prompt — `14-ngo-partner/field-dataset-upload`
...
| Route | `/14-ngo-partner/field-dataset-upload` |
...
| Localization prefix | `14_ngo_partner.field_dataset_upload` |
...
Folder: `app-ui/14-ngo-partner/field-dataset-upload/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-ngo-partner/field-dataset-upload`) |
...
7. Map navigation:
   ...
   - Primary action: Upload to ingest → `/14-ngo-partner/field-dataset-upload?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-ngo-partner/field-dataset-upload` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `14_ngo_partner.field_dataset_upload` only — no hard-coded user strings.
