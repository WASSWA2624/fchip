# Implementation prompt — `14-ngo-partner/impact-evidence`
...
| Route | `/14-ngo-partner/impact-evidence` |
| Parent route | `/14-ngo-partner/impact-evidence` |
...
| Localization prefix | `14_ngo_partner.impact_evidence` |
...
Folder: `app-ui/14-ngo-partner/impact-evidence/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-ngo-partner/impact-evidence`) |
...
7. Map navigation:
   ...
   - Primary action: Export brief → `/14-ngo-partner/impact-evidence`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-ngo-partner/impact-evidence` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `14_ngo_partner.impact_evidence` only — no hard-coded user strings.
