# Implementation prompt — `14-ngo-partner/partner-indicator-entry`
...
| Route | `/14-ngo-partner/partner-indicator-entry` |
...
| Localization prefix | `14_ngo_partner.partner_indicator_entry` |
...
Folder: `app-ui/14-ngo-partner/partner-indicator-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-ngo-partner/partner-indicator-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit indicator → `/14-ngo-partner/partner-indicator-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-ngo-partner/partner-indicator-entry` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `14_ngo_partner.partner_indicator_entry` only — no hard-coded user strings.
