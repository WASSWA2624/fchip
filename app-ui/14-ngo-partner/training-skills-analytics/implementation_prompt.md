# Implementation prompt — `14-ngo-partner/training-skills-analytics`
...
| Route | `/14-ngo-partner/training-skills-analytics` |
| Parent route | `/14-ngo-partner/training-skills-analytics` |
...
| Localization prefix | `14_ngo_partner.training_skills_analytics` |
...
Folder: `app-ui/14-ngo-partner/training-skills-analytics/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-ngo-partner/training-skills-analytics`) |
...
7. Map navigation:
   ...
   - Primary action: Export training brief → `/14-ngo-partner/training-skills-analytics`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-ngo-partner/training-skills-analytics` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `14_ngo_partner.training_skills_analytics` only — no hard-coded user strings.
