# Implementation prompt — `14-schools-health/absenteeism-wellness`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Absenteeism & wellness** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Wellness signals from school rolls_
...
| Screen slug | `absenteeism-wellness` |
| Chronology | `S-090` |
| Slice | `VS-14` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/14-schools-health/absenteeism-wellness` |
| Parent route | `/14-schools-health/school-home` |
| Layout | `form-capture` |
...
| Localization prefix | `14_schools_health.absenteeism_wellness` |
...
Folder: `app-ui/14-schools-health/absenteeism-wellness/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/absenteeism_wellness_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-schools-health/absenteeism-wellness`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-090`:

### Phase 0 — Claim the screen

1. Confirm `S-090` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Send wellness signal → `/14-schools-health/absenteeism-wellness?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-schools-health/absenteeism-wellness` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `14_schools_health.absenteeism_wellness` only — no hard-coded user strings.
...
23. Mark `S-090` `status: done`, `backend: paired` (or `none` with static evidence only).
