# Implementation prompt — `14-schools-health/health-education-session`
...
| Route | `/14-schools-health/health-education-session` |
| Parent route | `/14-schools-health/health-education-session` |
| Layout | `form-capture` |
...
| Localization prefix | `14_schools_health.health_education_session` |
...
Folder: `app-ui/14-schools-health/health-education-session/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-schools-health/health-education-session`) |
...
7. Map navigation:
   ...
   - Primary action: Save to cascade metrics → `/14-schools-health/health-education-session?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-schools-health/health-education-session` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `14_schools_health.health_education_session` only — no hard-coded user strings.
