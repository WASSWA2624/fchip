# Implementation prompt — `14-schools-health/school-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **School health home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Schools feed sessions · screening · absenteeism_
...
| Screen slug | `school-home` |
| Chronology | `S-087` |
| Slice | `VS-14` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/14-schools-health/school-home` |
...
| Localization prefix | `14_schools_health.school_home` |
...
Folder: `app-ui/14-schools-health/school-home/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/school_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-schools-health/school-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-087`:

### Phase 0 — Claim the screen

1. Confirm `S-087` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Log today’s session → `/14-schools-health/school-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-schools-health/school-home` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `14_schools_health.school_home` only — no hard-coded user strings.
...
23. Mark `S-087` `status: done`, `backend: paired` (or `none` with static evidence only).
