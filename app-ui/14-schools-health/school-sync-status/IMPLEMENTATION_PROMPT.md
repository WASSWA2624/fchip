# Implementation prompt — `14-schools-health/school-sync-status`
...
| Route | `/14-schools-health/school-sync-status` |
| Parent route | `/14-schools-health/school-sync-status` |
...
| Localization prefix | `14_schools_health.school_sync_status` |
...
Folder: `app-ui/14-schools-health/school-sync-status/`
...
- `app-ui/14-schools-health/README.md`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/14-schools-health/school-sync-status`) |
...
7. Map navigation:
   - Tabs: Home → `/14-schools-health/school-home`; Session → `/14-schools-health/health-education-session`; Screen → `/14-schools-health/learner-screening-entry`; Sync → `/14-schools-health/school-sync-status`
   - Primary action: Sync now → `/14-schools-health/school-sync-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/14-schools-health/school-sync-status` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `14_schools_health.school_sync_status` only — no hard-coded user strings.
