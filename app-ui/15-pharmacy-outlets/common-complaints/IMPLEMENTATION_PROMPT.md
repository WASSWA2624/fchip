# Implementation prompt — `15-pharmacy-outlets/common-complaints`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Common complaints** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Community symptom patterns from outlets_
...
| Screen slug | `common-complaints` |
| Chronology | `S-095` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-pharmacy-outlets/common-complaints` |
| Parent route | `/15-pharmacy-outlets/common-complaints` |
...
| Localization prefix | `15_pharmacy_outlets.common_complaints` |
...
Folder: `app-ui/15-pharmacy-outlets/common-complaints/`
...
| Page | `frontend/lib/features/pharmacy_outlets/presentation/pages/common_complaints_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-pharmacy-outlets/common-complaints`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-095`:

### Phase 0 — Claim the screen

1. Confirm `S-095` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Submit weekly rollup → `/15-pharmacy-outlets/common-complaints`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-pharmacy-outlets/common-complaints` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `15_pharmacy_outlets.common_complaints` only — no hard-coded user strings.
...
23. Mark `S-095` `status: done`, `backend: paired` (or `none` with static evidence only).
