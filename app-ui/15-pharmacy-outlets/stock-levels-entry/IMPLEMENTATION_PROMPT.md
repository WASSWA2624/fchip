# Implementation prompt — `15-pharmacy-outlets/stock-levels-entry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Stock levels entry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Medicine availability signals for demand forecasts_
...
| --- | --- |
| Kind | Screen (`form`) |
| Module | `15-pharmacy-outlets` |
| Screen slug | `stock-levels-entry` |
| Chronology | `S-093` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-pharmacy-outlets/stock-levels-entry` |
| Parent route | `/15-pharmacy-outlets/stock-levels-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `15_pharmacy_outlets.stock_levels_entry` |
...
Folder: `app-ui/15-pharmacy-outlets/stock-levels-entry/`
...
- `app-ui/15-pharmacy-outlets/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/pharmacy_outlets/` |
| Page | `frontend/lib/features/pharmacy_outlets/presentation/pages/stock_levels_entry_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/pharmacy_outlets/presentation/widgets/` |
| Presentation state | `frontend/lib/features/pharmacy_outlets/presentation/state/` |
| Domain | `frontend/lib/features/pharmacy_outlets/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-pharmacy-outlets/stock-levels-entry`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-093`:

### Phase 0 — Claim the screen

1. Confirm `S-093` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Stock → `/15-pharmacy-outlets/stock-levels-entry`; Dispense → `/15-pharmacy-outlets/dispense-log`; Complaints → `/15-pharmacy-outlets/common-complaints`; Sync → `/15-pharmacy-outlets/prestock-ack`
   - Primary action: Push stock snapshot → `/15-pharmacy-outlets/stock-levels-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-pharmacy-outlets/stock-levels-entry` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `15_pharmacy_outlets.stock_levels_entry` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/pharmacy_outlets/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/pharmacy_outlets/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-093` `status: done`, `backend: paired` (or `none` with static evidence only).
