# Implementation prompt — `15-schools-health/01-school-home`
...
| Screen slug | `01-school-home` |
| Chronology | `S-087` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-schools-health/01-school-home` |
...
| Localization prefix | `15_schools_health.01_school_home` |
...
Folder: `app-ui/15-schools-health/01-school-home/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/01_school_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/01-school-home`) |
...
7. Map navigation:
   ...
   - Primary action: Log today’s session → `/15-schools-health/01-school-home`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/01-school-home` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.01_school_home` only — no hard-coded user strings.
