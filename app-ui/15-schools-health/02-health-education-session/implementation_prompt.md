# Implementation prompt — `15-schools-health/02-health-education-session`
...
| Screen slug | `02-health-education-session` |
| Chronology | `S-088` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-schools-health/02-health-education-session` |
| Parent route | `/15-schools-health/02-health-education-session` |
...
| Localization prefix | `15_schools_health.02_health_education_session` |
...
Folder: `app-ui/15-schools-health/02-health-education-session/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/02_health_education_session_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/02-health-education-session`) |
...
7. Map navigation:
   ...
   - Primary action: Save to cascade metrics → `/15-schools-health/02-health-education-session?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/02-health-education-session` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.02_health_education_session` only — no hard-coded user strings.
