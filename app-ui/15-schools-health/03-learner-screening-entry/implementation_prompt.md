# Implementation prompt — `15-schools-health/03-learner-screening-entry`
...
| Screen slug | `03-learner-screening-entry` |
| Chronology | `S-089` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-schools-health/03-learner-screening-entry` |
| Parent route | `/15-schools-health/03-learner-screening-entry` |
...
| Localization prefix | `15_schools_health.03_learner_screening_entry` |
...
Folder: `app-ui/15-schools-health/03-learner-screening-entry/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/03_learner_screening_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/03-learner-screening-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit screening pack → `/15-schools-health/03-learner-screening-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/03-learner-screening-entry` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.03_learner_screening_entry` only — no hard-coded user strings.
