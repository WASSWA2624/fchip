# Implementation prompt — `15-schools-health/04-absenteeism-wellness`
...
| Screen slug | `04-absenteeism-wellness` |
| Chronology | `S-090` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-schools-health/04-absenteeism-wellness` |
| Parent route | `/15-schools-health/01-school-home` |
...
| Localization prefix | `15_schools_health.04_absenteeism_wellness` |
...
Folder: `app-ui/15-schools-health/04-absenteeism-wellness/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/04_absenteeism_wellness_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/04-absenteeism-wellness`) |
...
7. Map navigation:
   ...
   - Primary action: Send wellness signal → `/15-schools-health/04-absenteeism-wellness?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/04-absenteeism-wellness` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.04_absenteeism_wellness` only — no hard-coded user strings.
