# Implementation prompt — `15-schools-health/absenteeism-wellness`
...
| Route | `/15-schools-health/absenteeism-wellness` |
| Parent route | `/15-schools-health/school-home` |
| Layout | `form-capture` |
...
| Localization prefix | `15_schools_health.absenteeism_wellness` |
...
Folder: `app-ui/15-schools-health/absenteeism-wellness/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/absenteeism-wellness`) |
...
7. Map navigation:
   ...
   - Primary action: Send wellness signal → `/15-schools-health/absenteeism-wellness?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/absenteeism-wellness` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.absenteeism_wellness` only — no hard-coded user strings.
