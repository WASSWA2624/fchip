# Implementation prompt — `15-schools-health/health-education-session`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Health education session** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: School / community education metrics_
...
| Screen slug | `health-education-session` |
| Chronology | `S-088` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-schools-health/health-education-session` |
| Parent route | `/15-schools-health/health-education-session` |
| Layout | `form-capture` |
...
| Localization prefix | `15_schools_health.health_education_session` |
...
Folder: `app-ui/15-schools-health/health-education-session/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/health_education_session_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/health-education-session`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-088`:

### Phase 0 — Claim the screen

1. Confirm `S-088` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save to cascade metrics → `/15-schools-health/health-education-session?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/health-education-session` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.health_education_session` only — no hard-coded user strings.
...
23. Mark `S-088` `status: done`, `backend: paired` (or `none` with static evidence only).
