# Implementation prompt — `15-schools-health/learner-screening-entry`
...
| Route | `/15-schools-health/learner-screening-entry` |
| Parent route | `/15-schools-health/learner-screening-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `15_schools_health.learner_screening_entry` |
...
Folder: `app-ui/15-schools-health/learner-screening-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/learner-screening-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit screening pack → `/15-schools-health/learner-screening-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/learner-screening-entry` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.learner_screening_entry` only — no hard-coded user strings.
