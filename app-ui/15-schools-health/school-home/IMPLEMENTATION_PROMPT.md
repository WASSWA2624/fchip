# Implementation prompt — `15-schools-health/school-home`
...
| Route | `/15-schools-health/school-home` |
...
| Localization prefix | `15_schools_health.school_home` |
...
Folder: `app-ui/15-schools-health/school-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/school-home`) |
...
7. Map navigation:
   ...
   - Primary action: Log today’s session → `/15-schools-health/school-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/school-home` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.school_home` only — no hard-coded user strings.
