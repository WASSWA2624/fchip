# Implementation prompt — `15-schools-health/school-sync-status`
...
| Screen slug | `school-sync-status` |
| Chronology | `S-091` |
| Slice | `VS-15` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/15-schools-health/school-sync-status` |
| Parent route | `/15-schools-health/school-sync-status` |
...
| Localization prefix | `15_schools_health.school_sync_status` |
...
Folder: `app-ui/15-schools-health/school-sync-status/`
...
| Page | `frontend/lib/features/schools_health/presentation/pages/school_sync_status_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/15-schools-health/school-sync-status`) |
...
7. Map navigation:
   - Tabs: Home → `/15-schools-health/school-home`; Session → `/15-schools-health/health-education-session`; Screen → `/15-schools-health/learner-screening-entry`; Sync → `/15-schools-health/school-sync-status`
   - Primary action: Sync now → `/15-schools-health/school-sync-status`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/15-schools-health/school-sync-status` with the access guard for roles `school-health-worker`.
...
11. Add ARB keys under `15_schools_health.school_sync_status` only — no hard-coded user strings.
