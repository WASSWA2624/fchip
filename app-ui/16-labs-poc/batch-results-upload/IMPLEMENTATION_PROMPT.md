# Implementation prompt — `16-labs-poc/batch-results-upload`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Batch results upload** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Facility lab batches into FCHIP_
...
| Kind | Screen (`form`) |
| Module | `16-labs-poc` |
| Screen slug | `batch-results-upload` |
| Chronology | `S-099` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/16-labs-poc/batch-results-upload` |
| Parent route | `/16-labs-poc/batch-results-upload` |
| Layout | `upload-batch` |
...
| Localization prefix | `16_labs_poc.batch_results_upload` |
...
Folder: `app-ui/16-labs-poc/batch-results-upload/`
...
| Page | `frontend/lib/features/labs_poc/presentation/pages/batch_results_upload_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-labs-poc/batch-results-upload`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-099`:

### Phase 0 — Claim the screen

1. Confirm `S-099` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/16-labs-poc/lab-home`; Results → `/16-labs-poc/result-entry`; Queue → `/16-labs-poc/result-queue`; Sync → `/16-labs-poc/batch-results-upload`
   - Primary action: Upload batch → `/16-labs-poc/batch-results-upload?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-labs-poc/batch-results-upload` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `16_labs_poc.batch_results_upload` only — no hard-coded user strings.
...
23. Mark `S-099` `status: done`, `backend: paired` (or `none` with static evidence only).
