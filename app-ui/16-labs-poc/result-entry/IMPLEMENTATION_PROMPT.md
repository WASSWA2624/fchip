# Implementation prompt — `16-labs-poc/result-entry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Result entry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Point-of-care / lab result → ingest_
...
| Kind | Screen (`form`) |
| Module | `16-labs-poc` |
| Screen slug | `result-entry` |
| Chronology | `S-098` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/16-labs-poc/result-entry` |
| Parent route | `/16-labs-poc/result-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `16_labs_poc.result_entry` |
...
Folder: `app-ui/16-labs-poc/result-entry/`
...
| Page | `frontend/lib/features/labs_poc/presentation/pages/result_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-labs-poc/result-entry`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-098`:

### Phase 0 — Claim the screen

1. Confirm `S-098` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Submit result → `/16-labs-poc/result-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-labs-poc/result-entry` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `16_labs_poc.result_entry` only — no hard-coded user strings.
...
23. Mark `S-098` `status: done`, `backend: paired` (or `none` with static evidence only).
