# Implementation prompt — `16-labs-poc/result-queue`
...
| Kind | Screen (`queue`) |
| Module | `16-labs-poc` |
| Screen slug | `result-queue` |
| Chronology | `S-100` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/16-labs-poc/result-queue` |
| Parent route | `/16-labs-poc/result-queue` |
| Layout | `queue-desk` |
...
| Localization prefix | `16_labs_poc.result_queue` |
...
Folder: `app-ui/16-labs-poc/result-queue/`
...
| Page | `frontend/lib/features/labs_poc/presentation/pages/result_queue_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-labs-poc/result-queue`) |
...
7. Map navigation:
   ...
   - Primary action: Sync queue → `/16-labs-poc/result-queue`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-labs-poc/result-queue` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `16_labs_poc.result_queue` only — no hard-coded user strings.
