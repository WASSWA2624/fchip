# Implementation prompt — `16-pharmacy-outlets/02-stock-levels-entry`
...
| Screen slug | `02-stock-levels-entry` |
| Chronology | `S-093` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/16-pharmacy-outlets/02-stock-levels-entry` |
...
| Localization prefix | `16_pharmacy_outlets.02_stock_levels_entry` |
...
Folder: `app-ui/16-pharmacy-outlets/02-stock-levels-entry/`
...
| Page | `frontend/lib/features/pharmacy_outlets/presentation/pages/02_stock_levels_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/02-stock-levels-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Push stock snapshot → `/16-pharmacy-outlets/02-stock-levels-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/02-stock-levels-entry` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.02_stock_levels_entry` only — no hard-coded user strings.
