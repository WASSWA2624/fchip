# Implementation prompt — `16-pharmacy-outlets/03-dispense-log`
...
| Screen slug | `03-dispense-log` |
| Chronology | `S-094` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/16-pharmacy-outlets/03-dispense-log` |
| Parent route | `/16-pharmacy-outlets/03-dispense-log` |
...
| Localization prefix | `16_pharmacy_outlets.03_dispense_log` |
...
Folder: `app-ui/16-pharmacy-outlets/03-dispense-log/`
...
| Page | `frontend/lib/features/pharmacy_outlets/presentation/pages/03_dispense_log_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/03-dispense-log`) |
...
7. Map navigation:
   ...
   - Primary action: Log dispense → `/16-pharmacy-outlets/03-dispense-log?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/03-dispense-log` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.03_dispense_log` only — no hard-coded user strings.
