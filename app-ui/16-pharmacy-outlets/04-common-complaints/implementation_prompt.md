# Implementation prompt — `16-pharmacy-outlets/04-common-complaints`
...
| Screen slug | `04-common-complaints` |
| Chronology | `S-095` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
...
Folder: `app-ui/16-pharmacy-outlets/04-common-complaints/`
...
| Page | `frontend/lib/features/pharmacy_outlets/presentation/pages/04_common_complaints_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/04-common-complaints`) |
...
7. Map navigation:
   ...
   - Primary action: Submit weekly rollup → `/16-pharmacy-outlets/04-common-complaints`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/04-common-complaints` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.04_common_complaints` only — no hard-coded user strings.
