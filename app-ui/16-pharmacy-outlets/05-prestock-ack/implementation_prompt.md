# Implementation prompt — `16-pharmacy-outlets/05-prestock-ack`
...
| Screen slug | `05-prestock-ack` |
| Chronology | `S-096` |
| Slice | `VS-16` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/16-pharmacy-outlets/05-prestock-ack` |
| Parent route | `/16-pharmacy-outlets/05-prestock-ack` |
| Layout | `form-capture` |
...
| Localization prefix | `16_pharmacy_outlets.05_prestock_ack` |
...
Folder: `app-ui/16-pharmacy-outlets/05-prestock-ack/`
...
| Page | `frontend/lib/features/pharmacy_outlets/presentation/pages/05_prestock_ack_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/05-prestock-ack`) |
...
7. Map navigation:
   ...
   - Primary action: Confirm to district → `/16-pharmacy-outlets/05-prestock-ack?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/05-prestock-ack` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.05_prestock_ack` only — no hard-coded user strings.
