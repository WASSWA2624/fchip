# Implementation prompt — `16-pharmacy-outlets/common-complaints`
...
| Route | `/16-pharmacy-outlets/common-complaints` |
| Parent route | `/16-pharmacy-outlets/common-complaints` |
...
| Localization prefix | `16_pharmacy_outlets.common_complaints` |
...
Folder: `app-ui/16-pharmacy-outlets/common-complaints/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/common-complaints`) |
...
7. Map navigation:
   ...
   - Primary action: Submit weekly rollup → `/16-pharmacy-outlets/common-complaints`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/common-complaints` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.common_complaints` only — no hard-coded user strings.
