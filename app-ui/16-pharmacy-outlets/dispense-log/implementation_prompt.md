# Implementation prompt — `16-pharmacy-outlets/dispense-log`
...
| Route | `/16-pharmacy-outlets/dispense-log` |
| Parent route | `/16-pharmacy-outlets/dispense-log` |
| Layout | `form-capture` |
...
| Localization prefix | `16_pharmacy_outlets.dispense_log` |
...
Folder: `app-ui/16-pharmacy-outlets/dispense-log/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/dispense-log`) |
...
7. Map navigation:
   ...
   - Primary action: Log dispense → `/16-pharmacy-outlets/dispense-log?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/dispense-log` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.dispense_log` only — no hard-coded user strings.
