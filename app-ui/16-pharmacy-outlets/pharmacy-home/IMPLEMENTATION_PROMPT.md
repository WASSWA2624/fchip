# Implementation prompt — `16-pharmacy-outlets/pharmacy-home`
...
| Route | `/16-pharmacy-outlets/pharmacy-home` |
...
| Localization prefix | `16_pharmacy_outlets.pharmacy_home` |
...
Folder: `app-ui/16-pharmacy-outlets/pharmacy-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/pharmacy-home`) |
...
7. Map navigation:
   ...
   - Primary action: Update stock → `/16-pharmacy-outlets/stock-levels-entry`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/pharmacy-home` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.pharmacy_home` only — no hard-coded user strings.
