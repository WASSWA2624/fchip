# Implementation prompt — `16-pharmacy-outlets/prestock-ack`
...
| Route | `/16-pharmacy-outlets/prestock-ack` |
| Parent route | `/16-pharmacy-outlets/prestock-ack` |
| Layout | `form-capture` |
...
| Localization prefix | `16_pharmacy_outlets.prestock_ack` |
...
Folder: `app-ui/16-pharmacy-outlets/prestock-ack/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/prestock-ack`) |
...
7. Map navigation:
   ...
   - Primary action: Confirm to district → `/16-pharmacy-outlets/prestock-ack?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/prestock-ack` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.prestock_ack` only — no hard-coded user strings.
