# Implementation prompt — `16-pharmacy-outlets/stock-levels-entry`
...
| Route | `/16-pharmacy-outlets/stock-levels-entry` |
| Parent route | `/16-pharmacy-outlets/stock-levels-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `16_pharmacy_outlets.stock_levels_entry` |
...
Folder: `app-ui/16-pharmacy-outlets/stock-levels-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/16-pharmacy-outlets/stock-levels-entry`) |
...
7. Map navigation:
   - Tabs: Stock → `/16-pharmacy-outlets/stock-levels-entry`; Dispense → `/16-pharmacy-outlets/dispense-log`; Complaints → `/16-pharmacy-outlets/common-complaints`; Sync → `/16-pharmacy-outlets/prestock-ack`
   - Primary action: Push stock snapshot → `/16-pharmacy-outlets/stock-levels-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/16-pharmacy-outlets/stock-levels-entry` with the access guard for roles `pharmacy-outlet-worker`.
...
11. Add ARB keys under `16_pharmacy_outlets.stock_levels_entry` only — no hard-coded user strings.
