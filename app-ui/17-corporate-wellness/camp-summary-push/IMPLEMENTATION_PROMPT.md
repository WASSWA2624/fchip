# Implementation prompt — `17-corporate-wellness/camp-summary-push`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Camp summary push** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Aggregate wellness signals to NCD intelligence_
...
| --- | --- |
| Kind | Screen (`form`) |
| Module | `17-corporate-wellness` |
| Screen slug | `camp-summary-push` |
| Chronology | `S-103` |
| Slice | `VS-17` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/17-corporate-wellness/camp-summary-push` |
| Parent route | `/17-corporate-wellness/camp-summary-push` |
| Layout | `form-capture` |
...
| Localization prefix | `17_corporate_wellness.camp_summary_push` |
...
Folder: `app-ui/17-corporate-wellness/camp-summary-push/`
...
- `app-ui/17-corporate-wellness/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/corporate_wellness/` |
| Page | `frontend/lib/features/corporate_wellness/presentation/pages/camp_summary_push_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/corporate_wellness/presentation/widgets/` |
| Presentation state | `frontend/lib/features/corporate_wellness/presentation/state/` |
| Domain | `frontend/lib/features/corporate_wellness/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-corporate-wellness/camp-summary-push`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-103`:

### Phase 0 — Claim the screen

1. Confirm `S-103` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/17-corporate-wellness/corporate-home`; Vitals → `/17-corporate-wellness/camp-vitals-entry`; Summary → `/17-corporate-wellness/camp-summary-push`; Flags → `/17-corporate-wellness/occupational-flags`
   - Primary action: Push summary to ingest → `/17-corporate-wellness/camp-summary-push?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-corporate-wellness/camp-summary-push` with the access guard for roles `wellness-programme-worker`.
...
11. Add ARB keys under `17_corporate_wellness.camp_summary_push` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/corporate_wellness/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/corporate_wellness/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-103` `status: done`, `backend: paired` (or `none` with static evidence only).
