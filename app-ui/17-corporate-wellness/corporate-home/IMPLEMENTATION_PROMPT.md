# Implementation prompt — `17-corporate-wellness/corporate-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Corporate wellness home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Workplace programmes feed BP · BMI · glucose_
...
| Screen slug | `corporate-home` |
| Chronology | `S-101` |
| Slice | `VS-17` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/17-corporate-wellness/corporate-home` |
| Parent route | `/17-corporate-wellness/corporate-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `17_corporate_wellness.corporate_home` |
...
Folder: `app-ui/17-corporate-wellness/corporate-home/`
...
| Page | `frontend/lib/features/corporate_wellness/presentation/pages/corporate_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-corporate-wellness/corporate-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-101`:

### Phase 0 — Claim the screen

1. Confirm `S-101` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Start camp entry → `/17-corporate-wellness/corporate-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-corporate-wellness/corporate-home` with the access guard for roles `wellness-programme-worker`.
...
11. Add ARB keys under `17_corporate_wellness.corporate_home` only — no hard-coded user strings.
...
23. Mark `S-101` `status: done`, `backend: paired` (or `none` with static evidence only).
