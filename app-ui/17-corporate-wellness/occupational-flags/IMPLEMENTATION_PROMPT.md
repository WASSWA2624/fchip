# Implementation prompt — `17-corporate-wellness/occupational-flags`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Occupational flags** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Workplace health risk patterns_
...
| Screen slug | `occupational-flags` |
| Chronology | `S-104` |
| Slice | `VS-17` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/17-corporate-wellness/occupational-flags` |
| Parent route | `/17-corporate-wellness/occupational-flags` |
...
| Localization prefix | `17_corporate_wellness.occupational_flags` |
...
Folder: `app-ui/17-corporate-wellness/occupational-flags/`
...
| Page | `frontend/lib/features/corporate_wellness/presentation/pages/occupational_flags_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-corporate-wellness/occupational-flags`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-104`:

### Phase 0 — Claim the screen

1. Confirm `S-104` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/17-corporate-wellness/occupational-flags` with the access guard for roles `wellness-programme-worker`.
...
11. Add ARB keys under `17_corporate_wellness.occupational_flags` only — no hard-coded user strings.
...
23. Mark `S-104` `status: done`, `backend: paired` (or `none` with static evidence only).
