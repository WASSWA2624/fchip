# Implementation prompt — `17-labs-poc/01-lab-home`
...
| Module | `17-labs-poc` |
| Screen slug | `01-lab-home` |
| Chronology | `S-097` |
| Slice | `VS-17` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/17-labs-poc/01-lab-home` |
| Parent route | `/17-labs-poc/01-lab-home` |
...
| Localization prefix | `17_labs_poc.01_lab_home` |
...
Folder: `app-ui/17-labs-poc/01-lab-home/`
...
| Page | `frontend/lib/features/labs_poc/presentation/pages/01_lab_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/01-lab-home`) |
...
7. Map navigation:
   ...
   - Primary action: Enter result → `/17-labs-poc/01-lab-home`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/01-lab-home` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.01_lab_home` only — no hard-coded user strings.
