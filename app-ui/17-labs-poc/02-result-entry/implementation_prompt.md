# Implementation prompt — `17-labs-poc/02-result-entry`
...
| Kind | Screen (`form`) |
| Module | `17-labs-poc` |
| Screen slug | `02-result-entry` |
| Chronology | `S-098` |
| Slice | `VS-17` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/17-labs-poc/02-result-entry` |
| Parent route | `/17-labs-poc/02-result-entry` |
...
| Localization prefix | `17_labs_poc.02_result_entry` |
...
Folder: `app-ui/17-labs-poc/02-result-entry/`
...
| Page | `frontend/lib/features/labs_poc/presentation/pages/02_result_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/02-result-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit result → `/17-labs-poc/02-result-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/02-result-entry` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.02_result_entry` only — no hard-coded user strings.
