# Implementation prompt — `17-labs-poc/04-result-queue`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Result queue** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Results waiting to sync or link to referrals_
...
| Kind | Screen (`queue`) |
| Module | `17-labs-poc` |
| Screen slug | `04-result-queue` |
| Chronology | `S-100` |
| Slice | `VS-17` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/17-labs-poc/04-result-queue` |
| Parent route | `/17-labs-poc/04-result-queue` |
| Layout | `13-queue-desk` |
| Body layouts | `mobile` → `13-queue-desk`, `tablet` → `13-queue-desk`, `desktop` → `16-dual-pane-desktop` |
...
| Localization prefix | `17_labs_poc.04_result_queue` |
...
Folder: `app-ui/17-labs-poc/04-result-queue/`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/labs_poc/` |
| Page | `frontend/lib/features/labs_poc/presentation/pages/04_result_queue_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/labs_poc/presentation/widgets/` |
| Presentation state | `frontend/lib/features/labs_poc/presentation/state/` |
| Domain | `frontend/lib/features/labs_poc/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/04-result-queue`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-100`:

### Phase 0 — Claim the screen

1. Confirm `S-100` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Sync queue → `/17-labs-poc/04-result-queue`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/04-result-queue` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.04_result_queue` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/labs_poc/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/labs_poc/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-100` `status: done`, `backend: paired` (or `none` with static evidence only).
