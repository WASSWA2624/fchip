# Implementation prompt — `17-labs-poc/batch-results-upload`
...
| Route | `/17-labs-poc/batch-results-upload` |
| Parent route | `/17-labs-poc/batch-results-upload` |
| Layout | `upload-batch` |
...
| Localization prefix | `17_labs_poc.batch_results_upload` |
...
Folder: `app-ui/17-labs-poc/batch-results-upload/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/batch-results-upload`) |
...
7. Map navigation:
   - Tabs: Home → `/17-labs-poc/lab-home`; Results → `/17-labs-poc/result-entry`; Queue → `/17-labs-poc/result-queue`; Sync → `/17-labs-poc/batch-results-upload`
   - Primary action: Upload batch → `/17-labs-poc/batch-results-upload?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/batch-results-upload` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.batch_results_upload` only — no hard-coded user strings.
