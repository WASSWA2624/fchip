# Implementation prompt — `17-labs-poc/lab-home`
...
| Route | `/17-labs-poc/lab-home` |
| Parent route | `/17-labs-poc/lab-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `17_labs_poc.lab_home` |
...
Folder: `app-ui/17-labs-poc/lab-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/lab-home`) |
...
7. Map navigation:
   ...
   - Primary action: Enter result → `/17-labs-poc/lab-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/lab-home` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.lab_home` only — no hard-coded user strings.
