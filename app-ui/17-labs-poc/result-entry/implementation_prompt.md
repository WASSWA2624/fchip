# Implementation prompt — `17-labs-poc/result-entry`
...
| Route | `/17-labs-poc/result-entry` |
| Parent route | `/17-labs-poc/result-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `17_labs_poc.result_entry` |
...
Folder: `app-ui/17-labs-poc/result-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/result-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Submit result → `/17-labs-poc/result-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/result-entry` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.result_entry` only — no hard-coded user strings.
