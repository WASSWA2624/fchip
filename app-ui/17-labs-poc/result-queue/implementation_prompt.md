# Implementation prompt — `17-labs-poc/result-queue`
...
| Route | `/17-labs-poc/result-queue` |
| Parent route | `/17-labs-poc/result-queue` |
| Layout | `queue-desk` |
...
| Localization prefix | `17_labs_poc.result_queue` |
...
Folder: `app-ui/17-labs-poc/result-queue/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/17-labs-poc/result-queue`) |
...
7. Map navigation:
   ...
   - Primary action: Sync queue → `/17-labs-poc/result-queue`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/17-labs-poc/result-queue` with the access guard for roles `lab-worker`.
...
11. Add ARB keys under `17_labs_poc.result_queue` only — no hard-coded user strings.
