# Implementation prompt — `18-corporate-wellness/03-camp-summary-push`
...
| Screen slug | `03-camp-summary-push` |
| Chronology | `S-103` |
| Slice | `VS-18` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/18-corporate-wellness/03-camp-summary-push` |
| Parent route | `/18-corporate-wellness/03-camp-summary-push` |
...
| Localization prefix | `18_corporate_wellness.03_camp_summary_push` |
...
Folder: `app-ui/18-corporate-wellness/03-camp-summary-push/`
...
| Page | `frontend/lib/features/corporate_wellness/presentation/pages/03_camp_summary_push_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-corporate-wellness/03-camp-summary-push`) |
...
7. Map navigation:
   ...
   - Primary action: Push summary to ingest → `/18-corporate-wellness/03-camp-summary-push?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-corporate-wellness/03-camp-summary-push` with the access guard for roles `wellness-programme-worker`.
...
11. Add ARB keys under `18_corporate_wellness.03_camp_summary_push` only — no hard-coded user strings.
