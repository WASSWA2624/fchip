# Implementation prompt — `18-corporate-wellness/camp-summary-push`
...
| Route | `/18-corporate-wellness/camp-summary-push` |
| Parent route | `/18-corporate-wellness/camp-summary-push` |
| Layout | `form-capture` |
...
| Localization prefix | `18_corporate_wellness.camp_summary_push` |
...
Folder: `app-ui/18-corporate-wellness/camp-summary-push/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-corporate-wellness/camp-summary-push`) |
...
7. Map navigation:
   - Tabs: Home → `/18-corporate-wellness/corporate-home`; Vitals → `/18-corporate-wellness/camp-vitals-entry`; Summary → `/18-corporate-wellness/camp-summary-push`; Flags → `/18-corporate-wellness/occupational-flags`
   - Primary action: Push summary to ingest → `/18-corporate-wellness/camp-summary-push?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-corporate-wellness/camp-summary-push` with the access guard for roles `wellness-programme-worker`.
...
11. Add ARB keys under `18_corporate_wellness.camp_summary_push` only — no hard-coded user strings.
