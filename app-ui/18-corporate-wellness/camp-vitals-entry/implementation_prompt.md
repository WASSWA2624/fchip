# Implementation prompt — `18-corporate-wellness/camp-vitals-entry`
...
| Route | `/18-corporate-wellness/camp-vitals-entry` |
| Parent route | `/18-corporate-wellness/camp-vitals-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `18_corporate_wellness.camp_vitals_entry` |
...
Folder: `app-ui/18-corporate-wellness/camp-vitals-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-corporate-wellness/camp-vitals-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Save vitals → `/18-corporate-wellness/camp-vitals-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-corporate-wellness/camp-vitals-entry` with the access guard for roles `wellness-programme-worker`.
...
11. Add ARB keys under `18_corporate_wellness.camp_vitals_entry` only — no hard-coded user strings.
