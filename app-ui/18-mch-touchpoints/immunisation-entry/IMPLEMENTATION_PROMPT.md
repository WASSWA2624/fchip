# Implementation prompt — `18-mch-touchpoints/immunisation-entry`
...
| Route | `/18-mch-touchpoints/immunisation-entry` |
| Parent route | `/18-mch-touchpoints/immunisation-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `18_mch_touchpoints.immunisation_entry` |
...
Folder: `app-ui/18-mch-touchpoints/immunisation-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-mch-touchpoints/immunisation-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Record dose → `/18-mch-touchpoints/immunisation-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-mch-touchpoints/immunisation-entry` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `18_mch_touchpoints.immunisation_entry` only — no hard-coded user strings.
