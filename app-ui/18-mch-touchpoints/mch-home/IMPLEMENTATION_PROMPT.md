# Implementation prompt — `18-mch-touchpoints/mch-home`
...
| Route | `/18-mch-touchpoints/mch-home` |
...
| Localization prefix | `18_mch_touchpoints.mch_home` |
...
Folder: `app-ui/18-mch-touchpoints/mch-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-mch-touchpoints/mch-home`) |
...
7. Map navigation:
   ...
   - Primary action: Open ANC register → `/18-mch-touchpoints/mch-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-mch-touchpoints/mch-home` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `18_mch_touchpoints.mch_home` only — no hard-coded user strings.
