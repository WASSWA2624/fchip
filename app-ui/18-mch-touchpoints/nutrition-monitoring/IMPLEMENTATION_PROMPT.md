# Implementation prompt — `18-mch-touchpoints/nutrition-monitoring`
...
| Route | `/18-mch-touchpoints/nutrition-monitoring` |
| Parent route | `/18-mch-touchpoints/nutrition-monitoring` |
| Layout | `form-capture` |
...
| Localization prefix | `18_mch_touchpoints.nutrition_monitoring` |
...
Folder: `app-ui/18-mch-touchpoints/nutrition-monitoring/`
...
- `app-ui/18-mch-touchpoints/README.md`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-mch-touchpoints/nutrition-monitoring`) |
...
7. Map navigation:
   - Tabs: Home → `/18-mch-touchpoints/mch-home`; ANC → `/18-mch-touchpoints/anc-visit-entry`; Immunise → `/18-mch-touchpoints/immunisation-entry`; Nutrition → `/18-mch-touchpoints/nutrition-monitoring`
   - Primary action: Submit nutrition flag → `/18-mch-touchpoints/nutrition-monitoring?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-mch-touchpoints/nutrition-monitoring` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `18_mch_touchpoints.nutrition_monitoring` only — no hard-coded user strings.
