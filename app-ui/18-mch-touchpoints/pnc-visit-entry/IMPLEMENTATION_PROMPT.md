# Implementation prompt — `18-mch-touchpoints/pnc-visit-entry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **PNC visit entry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Postnatal follow-up signals_
...
| Screen slug | `pnc-visit-entry` |
| Chronology | `S-107` |
| Slice | `VS-18` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/18-mch-touchpoints/pnc-visit-entry` |
| Parent route | `/18-mch-touchpoints/mch-home` |
| Layout | `form-capture` |
...
| Localization prefix | `18_mch_touchpoints.pnc_visit_entry` |
...
Folder: `app-ui/18-mch-touchpoints/pnc-visit-entry/`
...
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/pnc_visit_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/18-mch-touchpoints/pnc-visit-entry`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-107`:

### Phase 0 — Claim the screen

1. Confirm `S-107` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save PNC visit → `/18-mch-touchpoints/pnc-visit-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/18-mch-touchpoints/pnc-visit-entry` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `18_mch_touchpoints.pnc_visit_entry` only — no hard-coded user strings.
...
23. Mark `S-107` `status: done`, `backend: paired` (or `none` with static evidence only).
