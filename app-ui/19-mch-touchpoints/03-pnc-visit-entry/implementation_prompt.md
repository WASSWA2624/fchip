# Implementation prompt — `19-mch-touchpoints/03-pnc-visit-entry`
...
| Screen slug | `03-pnc-visit-entry` |
| Chronology | `S-107` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-mch-touchpoints/03-pnc-visit-entry` |
| Parent route | `/19-mch-touchpoints/01-mch-home` |
...
| Localization prefix | `19_mch_touchpoints.03_pnc_visit_entry` |
...
Folder: `app-ui/19-mch-touchpoints/03-pnc-visit-entry/`
...
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/03_pnc_visit_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/03-pnc-visit-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Save PNC visit → `/19-mch-touchpoints/03-pnc-visit-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/03-pnc-visit-entry` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.03_pnc_visit_entry` only — no hard-coded user strings.
