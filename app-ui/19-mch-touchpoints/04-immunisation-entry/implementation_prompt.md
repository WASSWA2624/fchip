# Implementation prompt — `19-mch-touchpoints/04-immunisation-entry`
...
| Screen slug | `04-immunisation-entry` |
| Chronology | `S-108` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-mch-touchpoints/04-immunisation-entry` |
| Parent route | `/19-mch-touchpoints/04-immunisation-entry` |
...
| Localization prefix | `19_mch_touchpoints.04_immunisation_entry` |
...
Folder: `app-ui/19-mch-touchpoints/04-immunisation-entry/`
...
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/04_immunisation_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/04-immunisation-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Record dose → `/19-mch-touchpoints/04-immunisation-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/04-immunisation-entry` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.04_immunisation_entry` only — no hard-coded user strings.
