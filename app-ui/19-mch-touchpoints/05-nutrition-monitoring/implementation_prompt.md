# Implementation prompt — `19-mch-touchpoints/05-nutrition-monitoring`
...
| Screen slug | `05-nutrition-monitoring` |
| Chronology | `S-109` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-mch-touchpoints/05-nutrition-monitoring` |
| Parent route | `/19-mch-touchpoints/05-nutrition-monitoring` |
...
| Localization prefix | `19_mch_touchpoints.05_nutrition_monitoring` |
...
Folder: `app-ui/19-mch-touchpoints/05-nutrition-monitoring/`
...
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/05_nutrition_monitoring_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/05-nutrition-monitoring`) |
...
7. Map navigation:
   - Tabs: Home → `/19-mch-touchpoints/01-mch-home`; ANC → `/19-mch-touchpoints/02-anc-visit-entry`; Immunise → `/19-mch-touchpoints/04-immunisation-entry`; Nutrition → `/19-mch-touchpoints/05-nutrition-monitoring`
   - Primary action: Submit nutrition flag → `/19-mch-touchpoints/05-nutrition-monitoring?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/05-nutrition-monitoring` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.05_nutrition_monitoring` only — no hard-coded user strings.
