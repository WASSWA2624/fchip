# Implementation prompt — `19-mch-touchpoints/immunisation-entry`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Immunisation entry** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Immunisation post doses → child health coverage_
...
| Screen slug | `immunisation-entry` |
| Chronology | `S-108` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-mch-touchpoints/immunisation-entry` |
| Parent route | `/19-mch-touchpoints/immunisation-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `19_mch_touchpoints.immunisation_entry` |
...
Folder: `app-ui/19-mch-touchpoints/immunisation-entry/`
...
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/immunisation_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/immunisation-entry`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-108`:

### Phase 0 — Claim the screen

1. Confirm `S-108` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Record dose → `/19-mch-touchpoints/immunisation-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/immunisation-entry` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.immunisation_entry` only — no hard-coded user strings.
...
23. Mark `S-108` `status: done`, `backend: paired` (or `none` with static evidence only).
