# Implementation prompt — `19-mch-touchpoints/mch-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **MCH touchpoints home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: ANC / PNC · immunisation · nutrition posts feed care signals_
...
| Screen slug | `mch-home` |
| Chronology | `S-105` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-mch-touchpoints/mch-home` |
...
| Localization prefix | `19_mch_touchpoints.mch_home` |
...
Folder: `app-ui/19-mch-touchpoints/mch-home/`
...
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/mch_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/mch-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-105`:

### Phase 0 — Claim the screen

1. Confirm `S-105` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Open ANC register → `/19-mch-touchpoints/mch-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/mch-home` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.mch_home` only — no hard-coded user strings.
...
23. Mark `S-105` `status: done`, `backend: paired` (or `none` with static evidence only).
