# Implementation prompt — `19-mch-touchpoints/nutrition-monitoring`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Nutrition monitoring** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Growth / nutrition programme feed_
...
| --- | --- |
| Kind | Screen (`form`) |
| Module | `19-mch-touchpoints` |
| Screen slug | `nutrition-monitoring` |
| Chronology | `S-109` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-mch-touchpoints/nutrition-monitoring` |
| Parent route | `/19-mch-touchpoints/nutrition-monitoring` |
| Layout | `form-capture` |
...
| Localization prefix | `19_mch_touchpoints.nutrition_monitoring` |
...
Folder: `app-ui/19-mch-touchpoints/nutrition-monitoring/`
...
- `app-ui/19-mch-touchpoints/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/mch_touchpoints/` |
| Page | `frontend/lib/features/mch_touchpoints/presentation/pages/nutrition_monitoring_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/mch_touchpoints/presentation/widgets/` |
| Presentation state | `frontend/lib/features/mch_touchpoints/presentation/state/` |
| Domain | `frontend/lib/features/mch_touchpoints/domain/` |
| Data / repository impl | `frontend/lib/features/mch_touchpoints/data/` |
| Repository contract | `frontend/lib/features/mch_touchpoints/domain/repositories/mch_touchpoints_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/nutrition-monitoring`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `form-capture`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/mch_touchpoints/` |
| Backend module | `backend/src/modules/mch_touchpoints/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-109`:

### Phase 0 — Claim the screen

1. Confirm `S-109` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/19-mch-touchpoints/mch-home`; ANC → `/19-mch-touchpoints/anc-visit-entry`; Immunise → `/19-mch-touchpoints/immunisation-entry`; Nutrition → `/19-mch-touchpoints/nutrition-monitoring`
   - Primary action: Submit nutrition flag → `/19-mch-touchpoints/nutrition-monitoring?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/nutrition-monitoring` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.nutrition_monitoring` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/mch_touchpoints/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/mch_touchpoints/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-109` `status: done`, `backend: paired` (or `none` with static evidence only).
