# Implementation prompt — `19-mch-touchpoints/pnc-visit-entry`
...
| Route | `/19-mch-touchpoints/pnc-visit-entry` |
| Parent route | `/19-mch-touchpoints/mch-home` |
| Layout | `form-capture` |
...
| Localization prefix | `19_mch_touchpoints.pnc_visit_entry` |
...
Folder: `app-ui/19-mch-touchpoints/pnc-visit-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-mch-touchpoints/pnc-visit-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Save PNC visit → `/19-mch-touchpoints/pnc-visit-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-mch-touchpoints/pnc-visit-entry` with the access guard for roles `mch-worker`.
...
11. Add ARB keys under `19_mch_touchpoints.pnc_visit_entry` only — no hard-coded user strings.
