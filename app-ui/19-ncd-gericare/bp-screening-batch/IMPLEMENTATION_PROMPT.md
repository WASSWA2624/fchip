# Implementation prompt — `19-ncd-gericare/bp-screening-batch`
...
| Route | `/19-ncd-gericare/bp-screening-batch` |
| Parent route | `/19-ncd-gericare/bp-screening-batch` |
| Layout | `form-capture` |
...
| Localization prefix | `19_ncd_gericare.bp_screening_batch` |
...
Folder: `app-ui/19-ncd-gericare/bp-screening-batch/`
...
- `app-ui/19-ncd-gericare/README.md`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-ncd-gericare/bp-screening-batch`) |
...
7. Map navigation:
   - Tabs: Home → `/19-ncd-gericare/cohort-home`; Visit → `/19-ncd-gericare/cohort-visit-entry`; BP → `/19-ncd-gericare/bp-screening-batch`; Flags → `/19-ncd-gericare/stroke-risk-flags`
   - Primary action: Push BP batch → `/19-ncd-gericare/bp-screening-batch?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-ncd-gericare/bp-screening-batch` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `19_ncd_gericare.bp_screening_batch` only — no hard-coded user strings.
