# Implementation prompt — `19-ncd-gericare/cohort-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **NCD / Gericare home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Chronic and elderly cohorts feed longitudinal signals_
...
| Screen slug | `cohort-home` |
| Chronology | `S-110` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-ncd-gericare/cohort-home` |
| Parent route | `/19-ncd-gericare/cohort-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `19_ncd_gericare.cohort_home` |
...
Folder: `app-ui/19-ncd-gericare/cohort-home/`
...
| Page | `frontend/lib/features/ncd_gericare/presentation/pages/cohort_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-ncd-gericare/cohort-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-110`:

### Phase 0 — Claim the screen

1. Confirm `S-110` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Start cohort visit → `/19-ncd-gericare/cohort-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/19-ncd-gericare/cohort-home` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `19_ncd_gericare.cohort_home` only — no hard-coded user strings.
...
23. Mark `S-110` `status: done`, `backend: paired` (or `none` with static evidence only).
