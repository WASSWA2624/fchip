# Implementation prompt — `19-ncd-gericare/stroke-risk-flags`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Stroke / NCD risk flags** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Signals that feed NCD predictions_
...
| Screen slug | `stroke-risk-flags` |
| Chronology | `S-113` |
| Slice | `VS-19` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/19-ncd-gericare/stroke-risk-flags` |
| Parent route | `/19-ncd-gericare/stroke-risk-flags` |
...
| Localization prefix | `19_ncd_gericare.stroke_risk_flags` |
...
Folder: `app-ui/19-ncd-gericare/stroke-risk-flags/`
...
| Page | `frontend/lib/features/ncd_gericare/presentation/pages/stroke_risk_flags_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/19-ncd-gericare/stroke-risk-flags`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-113`:

### Phase 0 — Claim the screen

1. Confirm `S-113` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/19-ncd-gericare/stroke-risk-flags` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `19_ncd_gericare.stroke_risk_flags` only — no hard-coded user strings.
...
23. Mark `S-113` `status: done`, `backend: paired` (or `none` with static evidence only).
