# Implementation prompt — `20-hmis-dhis2/aggregate-push-pull`
...
| Screen slug | `aggregate-push-pull` |
| Chronology | `S-116` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-hmis-dhis2/aggregate-push-pull` |
| Parent route | `/20-hmis-dhis2/aggregate-push-pull` |
| Layout | `form-capture` |
...
| Localization prefix | `20_hmis_dhis2.aggregate_push_pull` |
...
Folder: `app-ui/20-hmis-dhis2/aggregate-push-pull/`
...
| Page | `frontend/lib/features/hmis_dhis2/presentation/pages/aggregate_push_pull_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-hmis-dhis2/aggregate-push-pull`) |
...
7. Map navigation:
   ...
   - Primary action: Execute exchange → `/20-hmis-dhis2/aggregate-push-pull?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-hmis-dhis2/aggregate-push-pull` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `20_hmis_dhis2.aggregate_push_pull` only — no hard-coded user strings.
