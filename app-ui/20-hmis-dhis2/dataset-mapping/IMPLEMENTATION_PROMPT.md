# Implementation prompt — `20-hmis-dhis2/dataset-mapping`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Dataset mapping** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Map HMIS/DHIS2 elements to cascade indicators_
...
| Module | `20-hmis-dhis2` |
| Screen slug | `dataset-mapping` |
| Chronology | `S-115` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-hmis-dhis2/dataset-mapping` |
| Parent route | `/20-hmis-dhis2/dataset-mapping` |
...
| Localization prefix | `20_hmis_dhis2.dataset_mapping` |
...
Folder: `app-ui/20-hmis-dhis2/dataset-mapping/`
...
- `app-ui/20-hmis-dhis2/README.md`
...
| Page | `frontend/lib/features/hmis_dhis2/presentation/pages/dataset_mapping_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-hmis-dhis2/dataset-mapping`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-115`:

### Phase 0 — Claim the screen

1. Confirm `S-115` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/20-hmis-dhis2/hmis-home`; Map → `/20-hmis-dhis2/dataset-mapping`; Exchange → `/20-hmis-dhis2/aggregate-push-pull`; Audit → `/20-hmis-dhis2/hmis-audit`
   - Primary action: Save mapping → `/20-hmis-dhis2/dataset-mapping?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-hmis-dhis2/dataset-mapping` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `20_hmis_dhis2.dataset_mapping` only — no hard-coded user strings.
...
23. Mark `S-115` `status: done`, `backend: paired` (or `none` with static evidence only).
