# Implementation prompt — `20-hmis-dhis2/hmis-audit`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **HMIS audit** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: What public aggregates entered the pipeline_
...
| Screen slug | `hmis-audit` |
| Chronology | `S-117` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-hmis-dhis2/hmis-audit` |
| Parent route | `/20-hmis-dhis2/hmis-audit` |
...
| Localization prefix | `20_hmis_dhis2.hmis_audit` |
...
Folder: `app-ui/20-hmis-dhis2/hmis-audit/`
...
| Page | `frontend/lib/features/hmis_dhis2/presentation/pages/hmis_audit_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-hmis-dhis2/hmis-audit`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-117`:

### Phase 0 — Claim the screen

1. Confirm `S-117` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/20-hmis-dhis2/hmis-audit` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `20_hmis_dhis2.hmis_audit` only — no hard-coded user strings.
...
23. Mark `S-117` `status: done`, `backend: paired` (or `none` with static evidence only).
