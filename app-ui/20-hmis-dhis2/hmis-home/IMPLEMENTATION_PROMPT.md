# Implementation prompt — `20-hmis-dhis2/hmis-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **HMIS / DHIS2 home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Approved public-health aggregates into FCHIP_
...
| Screen slug | `hmis-home` |
| Chronology | `S-114` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-hmis-dhis2/hmis-home` |
| Parent route | `/20-hmis-dhis2/hmis-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `20_hmis_dhis2.hmis_home` |
...
Folder: `app-ui/20-hmis-dhis2/hmis-home/`
...
| Page | `frontend/lib/features/hmis_dhis2/presentation/pages/hmis_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-hmis-dhis2/hmis-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-114`:

### Phase 0 — Claim the screen

1. Confirm `S-114` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Run sync → `/20-hmis-dhis2/hmis-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-hmis-dhis2/hmis-home` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `20_hmis_dhis2.hmis_home` only — no hard-coded user strings.
...
23. Mark `S-114` `status: done`, `backend: paired` (or `none` with static evidence only).
