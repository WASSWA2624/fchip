# Implementation prompt — `20-ncd-gericare/02-cohort-visit-entry`
...
| Screen slug | `02-cohort-visit-entry` |
| Chronology | `S-111` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-ncd-gericare/02-cohort-visit-entry` |
| Parent route | `/20-ncd-gericare/02-cohort-visit-entry` |
...
| Localization prefix | `20_ncd_gericare.02_cohort_visit_entry` |
...
Folder: `app-ui/20-ncd-gericare/02-cohort-visit-entry/`
...
| Page | `frontend/lib/features/ncd_gericare/presentation/pages/02_cohort_visit_entry_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/02-cohort-visit-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Save cohort visit → `/20-ncd-gericare/02-cohort-visit-entry?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-ncd-gericare/02-cohort-visit-entry` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.02_cohort_visit_entry` only — no hard-coded user strings.
