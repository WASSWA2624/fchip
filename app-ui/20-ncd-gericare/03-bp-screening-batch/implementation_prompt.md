# Implementation prompt — `20-ncd-gericare/03-bp-screening-batch`
...
| Screen slug | `03-bp-screening-batch` |
| Chronology | `S-112` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-ncd-gericare/03-bp-screening-batch` |
| Parent route | `/20-ncd-gericare/03-bp-screening-batch` |
| Layout | `form-capture` |
...
| Localization prefix | `20_ncd_gericare.03_bp_screening_batch` |
...
Folder: `app-ui/20-ncd-gericare/03-bp-screening-batch/`
...
| Page | `frontend/lib/features/ncd_gericare/presentation/pages/03_bp_screening_batch_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/03-bp-screening-batch`) |
...
7. Map navigation:
   - Tabs: Home → `/20-ncd-gericare/01-cohort-home`; Visit → `/20-ncd-gericare/02-cohort-visit-entry`; BP → `/20-ncd-gericare/03-bp-screening-batch`; Flags → `/20-ncd-gericare/04-stroke-risk-flags`
   - Primary action: Push BP batch → `/20-ncd-gericare/03-bp-screening-batch?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-ncd-gericare/03-bp-screening-batch` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.03_bp_screening_batch` only — no hard-coded user strings.
