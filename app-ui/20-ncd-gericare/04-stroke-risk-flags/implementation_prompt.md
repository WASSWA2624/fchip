# Implementation prompt — `20-ncd-gericare/04-stroke-risk-flags`
...
| Screen slug | `04-stroke-risk-flags` |
| Chronology | `S-113` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-ncd-gericare/04-stroke-risk-flags` |
| Parent route | `/20-ncd-gericare/04-stroke-risk-flags` |
...
| Localization prefix | `20_ncd_gericare.04_stroke_risk_flags` |
...
Folder: `app-ui/20-ncd-gericare/04-stroke-risk-flags/`
...
| Page | `frontend/lib/features/ncd_gericare/presentation/pages/04_stroke_risk_flags_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/04-stroke-risk-flags`) |
...
8. Register route `/20-ncd-gericare/04-stroke-risk-flags` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.04_stroke_risk_flags` only — no hard-coded user strings.
