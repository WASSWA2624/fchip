# Implementation prompt — `20-ncd-gericare/bp-screening-batch`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **BP screening batch** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Community/clinic BP packs for hotspot detection_
...
| --- | --- |
| Kind | Screen (`form`) |
| Module | `20-ncd-gericare` |
| Screen slug | `bp-screening-batch` |
| Chronology | `S-112` |
| Slice | `VS-20` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/20-ncd-gericare/bp-screening-batch` |
| Parent route | `/20-ncd-gericare/bp-screening-batch` |
| Layout | `form-capture` |
...
| Roles | `ncd-care-worker` |
| Supported states | `default`, `loading`, `error`, `success`, `offline`, `forbidden` |
| Localization prefix | `20_ncd_gericare.bp_screening_batch` |
...
Folder: `app-ui/20-ncd-gericare/bp-screening-batch/`
...
- `app-ui/20-ncd-gericare/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/ncd_gericare/` |
| Page | `frontend/lib/features/ncd_gericare/presentation/pages/bp_screening_batch_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/ncd_gericare/presentation/widgets/` |
| Presentation state | `frontend/lib/features/ncd_gericare/presentation/state/` |
| Domain | `frontend/lib/features/ncd_gericare/domain/` |
| Data / repository impl | `frontend/lib/features/ncd_gericare/data/` |
| Repository contract | `frontend/lib/features/ncd_gericare/domain/repositories/ncd_gericare_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/bp-screening-batch`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `form-capture`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/ncd_gericare/` |
| Backend module | `backend/src/modules/ncd_gericare/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-112`:

### Phase 0 — Claim the screen

1. Confirm `S-112` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/20-ncd-gericare/cohort-home`; Visit → `/20-ncd-gericare/cohort-visit-entry`; BP → `/20-ncd-gericare/bp-screening-batch`; Flags → `/20-ncd-gericare/stroke-risk-flags`
   - Primary action: Push BP batch → `/20-ncd-gericare/bp-screening-batch?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-ncd-gericare/bp-screening-batch` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.bp_screening_batch` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/ncd_gericare/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/ncd_gericare/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-112` `status: done`, `backend: paired` (or `none` with static evidence only).
