# Implementation prompt — `20-ncd-gericare/cohort-home`
...
| Route | `/20-ncd-gericare/cohort-home` |
| Parent route | `/20-ncd-gericare/cohort-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `20_ncd_gericare.cohort_home` |
...
Folder: `app-ui/20-ncd-gericare/cohort-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/cohort-home`) |
...
7. Map navigation:
   ...
   - Primary action: Start cohort visit → `/20-ncd-gericare/cohort-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-ncd-gericare/cohort-home` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.cohort_home` only — no hard-coded user strings.
