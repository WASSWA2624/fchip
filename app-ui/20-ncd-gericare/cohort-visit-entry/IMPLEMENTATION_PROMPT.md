# Implementation prompt — `20-ncd-gericare/cohort-visit-entry`
...
| Route | `/20-ncd-gericare/cohort-visit-entry` |
| Parent route | `/20-ncd-gericare/cohort-visit-entry` |
| Layout | `form-capture` |
...
| Localization prefix | `20_ncd_gericare.cohort_visit_entry` |
...
Folder: `app-ui/20-ncd-gericare/cohort-visit-entry/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/cohort-visit-entry`) |
...
7. Map navigation:
   ...
   - Primary action: Save cohort visit → `/20-ncd-gericare/cohort-visit-entry?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/20-ncd-gericare/cohort-visit-entry` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.cohort_visit_entry` only — no hard-coded user strings.
