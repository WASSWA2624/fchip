# Implementation prompt — `20-ncd-gericare/stroke-risk-flags`
...
| Route | `/20-ncd-gericare/stroke-risk-flags` |
| Parent route | `/20-ncd-gericare/stroke-risk-flags` |
...
| Localization prefix | `20_ncd_gericare.stroke_risk_flags` |
...
Folder: `app-ui/20-ncd-gericare/stroke-risk-flags/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/20-ncd-gericare/stroke-risk-flags`) |
...
8. Register route `/20-ncd-gericare/stroke-risk-flags` with the access guard for roles `ncd-care-worker`.
...
11. Add ARB keys under `20_ncd_gericare.stroke_risk_flags` only — no hard-coded user strings.
