# Implementation prompt — `21-community-events/community-dialogue`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Community dialogue** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Needs & priorities from community conversations_
...
| Screen slug | `community-dialogue` |
| Chronology | `S-120` |
| Slice | `VS-21` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/21-community-events/community-dialogue` |
| Parent route | `/21-community-events/community-dialogue` |
| Layout | `form-capture` |
...
| Localization prefix | `21_community_events.community_dialogue` |
...
Folder: `app-ui/21-community-events/community-dialogue/`
...
| Page | `frontend/lib/features/community_events/presentation/pages/community_dialogue_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-community-events/community-dialogue`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-120`:

### Phase 0 — Claim the screen

1. Confirm `S-120` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Submit dialogue notes → `/21-community-events/community-dialogue?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-community-events/community-dialogue` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `21_community_events.community_dialogue` only — no hard-coded user strings.
...
23. Mark `S-120` `status: done`, `backend: paired` (or `none` with static evidence only).
