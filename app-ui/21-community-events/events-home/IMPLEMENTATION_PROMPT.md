# Implementation prompt — `21-community-events/events-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Community events home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Churches · mosques · dialogues feed community signals_
...
| Screen slug | `events-home` |
| Chronology | `S-118` |
| Slice | `VS-21` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/21-community-events/events-home` |
| Parent route | `/21-community-events/events-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `21_community_events.events_home` |
...
Folder: `app-ui/21-community-events/events-home/`
...
| Page | `frontend/lib/features/community_events/presentation/pages/events_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-community-events/events-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-118`:

### Phase 0 — Claim the screen

1. Confirm `S-118` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Log event → `/21-community-events/events-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-community-events/events-home` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `21_community_events.events_home` only — no hard-coded user strings.
...
23. Mark `S-118` `status: done`, `backend: paired` (or `none` with static evidence only).
