# Implementation prompt — `21-community-events/outreach-event-log`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Outreach event log** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Community outreach events → ingest_
...
| Screen slug | `outreach-event-log` |
| Chronology | `S-119` |
| Slice | `VS-21` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/21-community-events/outreach-event-log` |
| Parent route | `/21-community-events/outreach-event-log` |
| Layout | `form-capture` |
...
| Localization prefix | `21_community_events.outreach_event_log` |
...
Folder: `app-ui/21-community-events/outreach-event-log/`
...
| Page | `frontend/lib/features/community_events/presentation/pages/outreach_event_log_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-community-events/outreach-event-log`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-119`:

### Phase 0 — Claim the screen

1. Confirm `S-119` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Save event → `/21-community-events/outreach-event-log?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-community-events/outreach-event-log` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `21_community_events.outreach_event_log` only — no hard-coded user strings.
...
23. Mark `S-119` `status: done`, `backend: paired` (or `none` with static evidence only).
