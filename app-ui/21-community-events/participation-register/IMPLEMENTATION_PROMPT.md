# Implementation prompt — `21-community-events/participation-register`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Participation register** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Who engaged — cascade participation signal_
...
| --- | --- |
| Kind | Screen (`list`) |
| Module | `21-community-events` |
| Screen slug | `participation-register` |
| Chronology | `S-121` |
| Slice | `VS-21` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/21-community-events/participation-register` |
| Parent route | `/21-community-events/participation-register` |
...
| Localization prefix | `21_community_events.participation_register` |
...
Folder: `app-ui/21-community-events/participation-register/`
...
- `app-ui/21-community-events/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/community_events/` |
| Page | `frontend/lib/features/community_events/presentation/pages/participation_register_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/community_events/presentation/widgets/` |
| Presentation state | `frontend/lib/features/community_events/presentation/state/` |
| Domain | `frontend/lib/features/community_events/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-community-events/participation-register`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-121`:

### Phase 0 — Claim the screen

1. Confirm `S-121` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/21-community-events/events-home`; Events → `/21-community-events/outreach-event-log`; Dialogue → `/21-community-events/community-dialogue`; Register → `/21-community-events/participation-register`
   - Primary action: Export register → `/21-community-events/participation-register`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-community-events/participation-register` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `21_community_events.participation_register` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/community_events/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/community_events/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-121` `status: done`, `backend: paired` (or `none` with static evidence only).
