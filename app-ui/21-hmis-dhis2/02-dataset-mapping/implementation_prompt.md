# Implementation prompt — `21-hmis-dhis2/02-dataset-mapping`
...
| Screen slug | `02-dataset-mapping` |
| Chronology | `S-115` |
| Slice | `VS-21` |
| Phase | `phase-2` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/21-hmis-dhis2/02-dataset-mapping` |
| Parent route | `/21-hmis-dhis2/02-dataset-mapping` |
...
| Localization prefix | `21_hmis_dhis2.02_dataset_mapping` |
...
Folder: `app-ui/21-hmis-dhis2/02-dataset-mapping/`
...
| Page | `frontend/lib/features/hmis_dhis2/presentation/pages/02_dataset_mapping_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-hmis-dhis2/02-dataset-mapping`) |
...
7. Map navigation:
   ...
   - Primary action: Save mapping → `/21-hmis-dhis2/02-dataset-mapping?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-hmis-dhis2/02-dataset-mapping` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `21_hmis_dhis2.02_dataset_mapping` only — no hard-coded user strings.
