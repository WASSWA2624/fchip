# Implementation prompt — `21-hmis-dhis2/aggregate-push-pull`
...
| Route | `/21-hmis-dhis2/aggregate-push-pull` |
| Parent route | `/21-hmis-dhis2/aggregate-push-pull` |
| Layout | `form-capture` |
...
| Localization prefix | `21_hmis_dhis2.aggregate_push_pull` |
...
Folder: `app-ui/21-hmis-dhis2/aggregate-push-pull/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-hmis-dhis2/aggregate-push-pull`) |
...
7. Map navigation:
   ...
   - Primary action: Execute exchange → `/21-hmis-dhis2/aggregate-push-pull?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-hmis-dhis2/aggregate-push-pull` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `21_hmis_dhis2.aggregate_push_pull` only — no hard-coded user strings.
