# Implementation prompt — `21-hmis-dhis2/dataset-mapping`
...
| Route | `/21-hmis-dhis2/dataset-mapping` |
| Parent route | `/21-hmis-dhis2/dataset-mapping` |
...
| Localization prefix | `21_hmis_dhis2.dataset_mapping` |
...
Folder: `app-ui/21-hmis-dhis2/dataset-mapping/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-hmis-dhis2/dataset-mapping`) |
...
7. Map navigation:
   - Tabs: Home → `/21-hmis-dhis2/hmis-home`; Map → `/21-hmis-dhis2/dataset-mapping`; Exchange → `/21-hmis-dhis2/aggregate-push-pull`; Audit → `/21-hmis-dhis2/hmis-audit`
   - Primary action: Save mapping → `/21-hmis-dhis2/dataset-mapping?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-hmis-dhis2/dataset-mapping` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `21_hmis_dhis2.dataset_mapping` only — no hard-coded user strings.
