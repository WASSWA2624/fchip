# Implementation prompt — `21-hmis-dhis2/hmis-home`
...
| Route | `/21-hmis-dhis2/hmis-home` |
| Parent route | `/21-hmis-dhis2/hmis-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `21_hmis_dhis2.hmis_home` |
...
Folder: `app-ui/21-hmis-dhis2/hmis-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/21-hmis-dhis2/hmis-home`) |
...
7. Map navigation:
   ...
   - Primary action: Run sync → `/21-hmis-dhis2/hmis-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/21-hmis-dhis2/hmis-home` with the access guard for roles `hmis-operator`.
...
11. Add ARB keys under `21_hmis_dhis2.hmis_home` only — no hard-coded user strings.
