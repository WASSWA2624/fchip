# Implementation prompt — `22-climate-feeds/climate-home`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Climate feeds home** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Climate API as a first-class data feeder_
...
| Screen slug | `climate-home` |
| Chronology | `S-048` |
| Slice | `VS-22` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/22-climate-feeds/climate-home` |
| Parent route | `/22-climate-feeds/climate-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `22_climate_feeds.climate_home` |
...
Folder: `app-ui/22-climate-feeds/climate-home/`
...
| Page | `frontend/lib/features/climate_feeds/presentation/pages/climate_home_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-climate-feeds/climate-home`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-048`:

### Phase 0 — Claim the screen

1. Confirm `S-048` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Refresh feeds → `/22-climate-feeds/climate-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-climate-feeds/climate-home` with the access guard for roles `climate-data-operator`.
...
11. Add ARB keys under `22_climate_feeds.climate_home` only — no hard-coded user strings.
...
23. Mark `S-048` `status: done`, `backend: paired` (or `none` with static evidence only).
