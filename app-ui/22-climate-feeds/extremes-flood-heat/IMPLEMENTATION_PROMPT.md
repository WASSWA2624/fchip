# Implementation prompt — `22-climate-feeds/extremes-flood-heat`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Extremes · flood · heat** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Short-term climate risks for early warning_
...
| Screen slug | `extremes-flood-heat` |
| Chronology | `S-050` |
| Slice | `VS-22` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/22-climate-feeds/extremes-flood-heat` |
| Parent route | `/22-climate-feeds/extremes-flood-heat` |
...
| Localization prefix | `22_climate_feeds.extremes_flood_heat` |
...
Folder: `app-ui/22-climate-feeds/extremes-flood-heat/`
...
| Page | `frontend/lib/features/climate_feeds/presentation/pages/extremes_flood_heat_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-climate-feeds/extremes-flood-heat`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-050`:

### Phase 0 — Claim the screen

1. Confirm `S-050` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/22-climate-feeds/extremes-flood-heat` with the access guard for roles `climate-data-operator`.
...
11. Add ARB keys under `22_climate_feeds.extremes_flood_heat` only — no hard-coded user strings.
...
23. Mark `S-050` `status: done`, `backend: paired` (or `none` with static evidence only).
