# Implementation prompt — `22-climate-feeds/feed-config-audit`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Feed config & audit** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Which climate feeds may enter the pipeline_
...
| Module | `22-climate-feeds` |
| Screen slug | `feed-config-audit` |
| Chronology | `S-051` |
| Slice | `VS-22` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/22-climate-feeds/feed-config-audit` |
| Parent route | `/22-climate-feeds/feed-config-audit` |
...
| Localization prefix | `22_climate_feeds.feed_config_audit` |
...
Folder: `app-ui/22-climate-feeds/feed-config-audit/`
...
- `app-ui/22-climate-feeds/README.md`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/climate_feeds/` |
| Page | `frontend/lib/features/climate_feeds/presentation/pages/feed_config_audit_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/climate_feeds/presentation/widgets/` |
| Presentation state | `frontend/lib/features/climate_feeds/presentation/state/` |
| Domain | `frontend/lib/features/climate_feeds/domain/` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-climate-feeds/feed-config-audit`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-051`:

### Phase 0 — Claim the screen

1. Confirm `S-051` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   - Tabs: Home → `/22-climate-feeds/climate-home`; Rain → `/22-climate-feeds/rainfall-temperature`; Extremes → `/22-climate-feeds/extremes-flood-heat`; Config → `/22-climate-feeds/feed-config-audit`
   - Primary action: Save feed config → `/22-climate-feeds/feed-config-audit?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-climate-feeds/feed-config-audit` with the access guard for roles `climate-data-operator`.
...
11. Add ARB keys under `22_climate_feeds.feed_config_audit` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/climate_feeds/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/climate_feeds/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-051` `status: done`, `backend: paired` (or `none` with static evidence only).
