# Implementation prompt — `22-climate-feeds/rainfall-temperature`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Rainfall & temperature** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Weather signals fused with health_
...
| Screen slug | `rainfall-temperature` |
| Chronology | `S-049` |
| Slice | `VS-22` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/22-climate-feeds/rainfall-temperature` |
| Parent route | `/22-climate-feeds/rainfall-temperature` |
...
| Localization prefix | `22_climate_feeds.rainfall_temperature` |
...
Folder: `app-ui/22-climate-feeds/rainfall-temperature/`
...
| Page | `frontend/lib/features/climate_feeds/presentation/pages/rainfall_temperature_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-climate-feeds/rainfall-temperature`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-049`:

### Phase 0 — Claim the screen

1. Confirm `S-049` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
8. Register route `/22-climate-feeds/rainfall-temperature` with the access guard for roles `climate-data-operator`.
...
11. Add ARB keys under `22_climate_feeds.rainfall_temperature` only — no hard-coded user strings.
...
23. Mark `S-049` `status: done`, `backend: paired` (or `none` with static evidence only).
