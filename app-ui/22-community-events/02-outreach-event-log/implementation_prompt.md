# Implementation prompt — `22-community-events/02-outreach-event-log`
...
| Screen slug | `02-outreach-event-log` |
| Chronology | `S-119` |
| Slice | `VS-22` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/22-community-events/02-outreach-event-log` |
| Parent route | `/22-community-events/02-outreach-event-log` |
...
| Localization prefix | `22_community_events.02_outreach_event_log` |
...
Folder: `app-ui/22-community-events/02-outreach-event-log/`
...
| Page | `frontend/lib/features/community_events/presentation/pages/02_outreach_event_log_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-community-events/02-outreach-event-log`) |
...
7. Map navigation:
   ...
   - Primary action: Save event → `/22-community-events/02-outreach-event-log?state=success`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-community-events/02-outreach-event-log` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `22_community_events.02_outreach_event_log` only — no hard-coded user strings.
