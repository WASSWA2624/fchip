# Implementation prompt — `22-community-events/community-dialogue`
...
| Route | `/22-community-events/community-dialogue` |
| Parent route | `/22-community-events/community-dialogue` |
| Layout | `form-capture` |
...
| Localization prefix | `22_community_events.community_dialogue` |
...
Folder: `app-ui/22-community-events/community-dialogue/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-community-events/community-dialogue`) |
...
7. Map navigation:
   ...
   - Primary action: Submit dialogue notes → `/22-community-events/community-dialogue?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-community-events/community-dialogue` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `22_community_events.community_dialogue` only — no hard-coded user strings.
