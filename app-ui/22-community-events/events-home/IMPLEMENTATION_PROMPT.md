# Implementation prompt — `22-community-events/events-home`
...
| Route | `/22-community-events/events-home` |
| Parent route | `/22-community-events/events-home` |
| Layout | `feeder-home` |
...
| Localization prefix | `22_community_events.events_home` |
...
Folder: `app-ui/22-community-events/events-home/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-community-events/events-home`) |
...
7. Map navigation:
   ...
   - Primary action: Log event → `/22-community-events/events-home`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-community-events/events-home` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `22_community_events.events_home` only — no hard-coded user strings.
