# Implementation prompt — `22-community-events/outreach-event-log`
...
| Route | `/22-community-events/outreach-event-log` |
| Parent route | `/22-community-events/outreach-event-log` |
| Layout | `form-capture` |
...
| Localization prefix | `22_community_events.outreach_event_log` |
...
Folder: `app-ui/22-community-events/outreach-event-log/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-community-events/outreach-event-log`) |
...
7. Map navigation:
   ...
   - Primary action: Save event → `/22-community-events/outreach-event-log?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-community-events/outreach-event-log` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `22_community_events.outreach_event_log` only — no hard-coded user strings.
