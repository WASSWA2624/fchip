# Implementation prompt — `22-community-events/participation-register`
...
| Route | `/22-community-events/participation-register` |
| Parent route | `/22-community-events/participation-register` |
...
| Localization prefix | `22_community_events.participation_register` |
...
Folder: `app-ui/22-community-events/participation-register/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/22-community-events/participation-register`) |
...
7. Map navigation:
   - Tabs: Home → `/22-community-events/events-home`; Events → `/22-community-events/outreach-event-log`; Dialogue → `/22-community-events/community-dialogue`; Register → `/22-community-events/participation-register`
   - Primary action: Export register → `/22-community-events/participation-register`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/22-community-events/participation-register` with the access guard for roles `community-programme-worker`.
...
11. Add ARB keys under `22_community_events.participation_register` only — no hard-coded user strings.
