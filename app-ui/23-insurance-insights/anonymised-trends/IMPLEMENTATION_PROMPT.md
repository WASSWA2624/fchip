# Implementation prompt — `23-insurance-insights/anonymised-trends`
...
| Route | `/23-insurance-insights/anonymised-trends` |
| Parent route | `/23-insurance-insights/anonymised-trends` |
...
| Localization prefix | `23_insurance_insights.anonymised_trends` |
...
Folder: `app-ui/23-insurance-insights/anonymised-trends/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-insurance-insights/anonymised-trends`) |
...
7. Map navigation:
   ...
   - Primary action: Schedule partner report → `/23-insurance-insights/anonymised-trends`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-insurance-insights/anonymised-trends` with the access guard for roles `insurance-analyst`.
...
11. Add ARB keys under `23_insurance_insights.anonymised_trends` only — no hard-coded user strings.
