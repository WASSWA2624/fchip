# Implementation prompt — `23-insurance-insights/prevention-overview`
...
| Module | `23-insurance-insights` |
| Screen slug | `prevention-overview` |
| Chronology | `S-075` |
| Slice | `VS-23` |
| Phase | `mvp` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/23-insurance-insights/prevention-overview` |
| Parent route | `/23-insurance-insights/prevention-overview` |
| Layout | `dashboard-metrics` |
| Body layouts | `mobile` → `dashboard-metrics`, `tablet` → `dashboard-metrics`, `desktop` → `dual-pane-desktop` |
...
| Localization prefix | `23_insurance_insights.prevention_overview` |
...
Folder: `app-ui/23-insurance-insights/prevention-overview/`
...
- `app-ui/23-insurance-insights/README.md`
- `app-ui/00-shared/tokens.json`
- Shared layout specimens for `dashboard-metrics` under `app-ui/00-shared/layouts/dashboard-metrics/`
...
| Page | `frontend/lib/features/insurance_insights/presentation/pages/prevention_overview_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-insurance-insights/prevention-overview`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `dashboard-metrics`) |
...
6. Resolve `dashboard-metrics` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   - Tabs: Prevent → `/23-insurance-insights/prevention-overview`; Cohorts → `/23-insurance-insights/risk-cohort-insights`; Trends → `/23-insurance-insights/anonymised-trends`; More → `/00-shared/notifications-center`
   - Primary action: Open risk cohorts → `/23-insurance-insights/prevention-overview`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-insurance-insights/prevention-overview` with the access guard for roles `insurance-analyst`.
9. Compose the page from shared shells + `dashboard-metrics` + catalog components. Prefer:
...
11. Add ARB keys under `23_insurance_insights.prevention_overview` only — no hard-coded user strings.
...
- [ ] Layout `dashboard-metrics` is a shared layout implementation, not a one-off Scaffold tree
