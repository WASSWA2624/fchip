# Implementation prompt — `23-insurance-insights/risk-cohort-insights`
...
| Route | `/23-insurance-insights/risk-cohort-insights` |
| Parent route | `/23-insurance-insights/risk-cohort-insights` |
...
| Localization prefix | `23_insurance_insights.risk_cohort_insights` |
...
Folder: `app-ui/23-insurance-insights/risk-cohort-insights/`
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-insurance-insights/risk-cohort-insights`) |
...
7. Map navigation:
   ...
   - Primary action: Export anonymised brief → `/23-insurance-insights/risk-cohort-insights`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-insurance-insights/risk-cohort-insights` with the access guard for roles `insurance-analyst`.
...
11. Add ARB keys under `23_insurance_insights.risk_cohort_insights` only — no hard-coded user strings.
