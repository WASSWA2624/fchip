# Implementation prompt — `23-research-exports/02-export-request`
...
| Screen slug | `02-export-request` |
| Chronology | `S-125` |
| Slice | `VS-23` |
| Phase | `phase-4` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/23-research-exports/02-export-request` |
...
| Localization prefix | `23_research_exports.02_export_request` |
...
Folder: `app-ui/23-research-exports/02-export-request/`
...
| Page | `frontend/lib/features/research_exports/presentation/pages/02_export_request_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-research-exports/02-export-request`) |
...
7. Map navigation:
   ...
   - Primary action: Submit request → `/23-research-exports/04-export-pending`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-research-exports/02-export-request` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `23_research_exports.02_export_request` only — no hard-coded user strings.
