# Implementation prompt — `23-research-exports/04-export-pending`
...
| Screen slug | `04-export-pending` |
| Chronology | `S-127` |
| Slice | `VS-23` |
| Phase | `phase-4` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/23-research-exports/04-export-pending` |
| Parent route | `/23-research-exports/01-evidence-catalog` |
...
| Localization prefix | `23_research_exports.04_export_pending` |
...
Folder: `app-ui/23-research-exports/04-export-pending/`
...
| Page | `frontend/lib/features/research_exports/presentation/pages/04_export_pending_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-research-exports/04-export-pending`) |
...
7. Map navigation:
   ...
   - Primary action: Return to evidence catalog → `/23-research-exports/01-evidence-catalog`
   - Access denied → `/00-shared/10-access-denied`
   - Not found → `/00-shared/11-not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-research-exports/04-export-pending` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `23_research_exports.04_export_pending` only — no hard-coded user strings.
