# Implementation prompt — `23-research-exports/export-pending`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Export under review** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Approval and privacy checks are in progress_
...
| --- | --- |
| Kind | Screen (`detail`) |
| Module | `23-research-exports` |
| Screen slug | `export-pending` |
| Chronology | `S-127` |
| Slice | `VS-23` |
| Phase | `phase-4` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/23-research-exports/export-pending` |
| Parent route | `/23-research-exports/evidence-catalog` |
| Layout | `detail-action` |
| Body layouts | `mobile` → `detail-action`, `tablet` → `detail-action`, `desktop` → `detail-action` |
...
| Localization prefix | `23_research_exports.export_pending` |
...
Folder: `app-ui/23-research-exports/export-pending/`
...
- `app-ui/23-research-exports/README.md`
- `app-ui/00-shared/tokens.json`
- Shared layout specimens for `detail-action` under `app-ui/00-shared/layouts/detail-action/`
...
| Layer | Path |
| --- | --- |
| Feature root | `frontend/lib/features/research_exports/` |
| Page | `frontend/lib/features/research_exports/presentation/pages/export_pending_page.dart` |
| Widgets (feature-only) | `frontend/lib/features/research_exports/presentation/widgets/` |
| Presentation state | `frontend/lib/features/research_exports/presentation/state/` |
| Domain | `frontend/lib/features/research_exports/domain/` |
| Data / repository impl | `frontend/lib/features/research_exports/data/` |
| Repository contract | `frontend/lib/features/research_exports/domain/repositories/research_exports_repository.dart` |
| Router entry | `frontend/lib/app/router/` (route must equal `/23-research-exports/export-pending`) |
| Shared layout | `frontend/lib/shared/layout/` (implement/reuse `detail-action`) |
| Shared components | `frontend/lib/shared/components/` |
| Tests | `frontend/test/features/research_exports/` |
| Backend module | `backend/src/modules/research_exports/` |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-127`:

### Phase 0 — Claim the screen

1. Confirm `S-127` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
6. Resolve `detail-action` and every composed component against `app-ui/00-shared/`.
7. Map navigation:
   - Tabs: Monitor → `/14-ngo-partner/programme-monitoring`; Evidence → `/14-ngo-partner/impact-evidence`; Training → `/14-ngo-partner/training-skills-analytics`; More → `/23-research-exports/evidence-catalog`
   - Primary action: Return to evidence catalog → `/23-research-exports/evidence-catalog`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-research-exports/export-pending` with the access guard for roles `ngo-partner`, `researcher`.
9. Compose the page from shared shells + `detail-action` + catalog components. Prefer:
...
11. Add ARB keys under `23_research_exports.export_pending` only — no hard-coded user strings.
...
14. Place feature-only widgets under `frontend/lib/features/research_exports/presentation/widgets/`; anything reusable across modules must move to `frontend/lib/shared/`.
...
18. Implement under `backend/src/modules/research_exports/` per `backend/dev-plan/P016_slice_pairing.md` and backend rules above: Prisma + migration, Zod, repository, service, controller, routes, permissions, consent, audit, events, seeds, tests.
...
23. Mark `S-127` `status: done`, `backend: paired` (or `none` with static evidence only).
...
- [ ] Layout `detail-action` is a shared layout implementation, not a one-off Scaffold tree
