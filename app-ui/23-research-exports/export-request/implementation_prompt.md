# Implementation prompt — `23-research-exports/export-request`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Export request** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Approved research access_
...
| Screen slug | `export-request` |
| Chronology | `S-125` |
| Slice | `VS-23` |
| Phase | `phase-4` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/23-research-exports/export-request` |
...
| Localization prefix | `23_research_exports.export_request` |
...
Folder: `app-ui/23-research-exports/export-request/`
...
| Page | `frontend/lib/features/research_exports/presentation/pages/export_request_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-research-exports/export-request`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-125`:

### Phase 0 — Claim the screen

1. Confirm `S-125` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Submit request → `/23-research-exports/export-pending`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-research-exports/export-request` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `23_research_exports.export_request` only — no hard-coded user strings.
...
23. Mark `S-125` `status: done`, `backend: paired` (or `none` with static evidence only).
