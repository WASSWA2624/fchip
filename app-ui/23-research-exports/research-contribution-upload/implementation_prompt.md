# Implementation prompt — `23-research-exports/research-contribution-upload`

> Paste this whole file into an agent session when implementing this UI. Do not invent a different screen.

## Mission

Implement **Research contribution upload** as a production Flutter screen in `frontend/lib`, with maximum reuse of the shared kit, correct responsive behavior from the six mockups, typed domain/data layers, and a **matching backend** in the same atomic chronology unit before starting the next screen.

_Subtitle: Academic partners feed approved anonymised field sets_
...
| Screen slug | `research-contribution-upload` |
| Chronology | `S-126` |
| Slice | `VS-23` |
| Phase | `phase-4` |
| Deferred | `false` |
| Chronology status | `not-started` |
| Route | `/23-research-exports/research-contribution-upload` |
...
| Localization prefix | `23_research_exports.research_contribution_upload` |
...
Folder: `app-ui/23-research-exports/research-contribution-upload/`
...
| Page | `frontend/lib/features/research_exports/presentation/pages/research_contribution_upload_page.dart` |
...
| Router entry | `frontend/lib/app/router/` (route must equal `/23-research-exports/research-contribution-upload`) |
...
Follow `frontend/dev-plan/25-slice-execution-playbook.md` for `S-126`:

### Phase 0 — Claim the screen

1. Confirm `S-126` is the next unfinished non-deferred row in `chronology.yaml` (or stop if another screen is already in progress).
...
7. Map navigation:
   ...
   - Primary action: Contribute dataset → `/23-research-exports/research-contribution-upload?state=success`
   - Access denied → `/00-shared/access-denied`
   - Not found → `/00-shared/not-found`

### Phase 2 — Flutter UI on fixtures (maximum reuse)

8. Register route `/23-research-exports/research-contribution-upload` with the access guard for roles `ngo-partner`, `researcher`.
...
11. Add ARB keys under `23_research_exports.research_contribution_upload` only — no hard-coded user strings.
...
23. Mark `S-126` `status: done`, `backend: paired` (or `none` with static evidence only).
