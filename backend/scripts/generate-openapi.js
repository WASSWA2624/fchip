...
const buildComponents = () => ({
  securitySchemes: {
    BearerAuth: {
      ...
      scheme: 'bearer',
      bearerFormat: 'JWT',
      description: 'JWT bearer token issued by the FCHIP auth module.',
    },
    ApiKeyAuth: {
      type: 'apiKey',
      in: 'header',
      name: 'x-api-key',
      description: 'Tenant-scoped API key for integration-facing FCHIP routes.',
    },
  },
  headers: {
    ETag: {
...
const buildSpecification = () => {
  ...
  return {
    ...
    servers: [
      {
        url: '/',
        description: 'Relative server root for the deployed FCHIP backend.',
      },
    ],
    tags,
    paths,
