...
const seedMortuaryPack = async (ctx, orgPack, accessPack, clinicalPack) => {
  ...
  await ctx.upsert(
    ...
    `${scenario.key}:mortuary:event:intake-ready`,
    {
      ...
      event_type: 'RECEIVED',
      event_at: ctx.date(-3),
      actor_name: mortuaryStaff?.email || 'mortuary.staff@fchip.com',
  ...
  await ctx.upsert(
    ...
    `${scenario.key}:mortuary:pm-request:pending`,
    {
      ...
      requested_by_name: 'doctor@fchip.com',
  ...
  await ctx.upsert(
    ...
    `${scenario.key}:mortuary:release-auth:ready`,
    {
      ...
      status: 'APPROVED',
      approved_by_name: mortuaryManager?.email || 'mortuary.manager@fchip.com',
