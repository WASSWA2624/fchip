...
const finalizeSuccess = async (delivery, result) => {
  ...
  recordBackgroundJob('notification_delivery.completed', {
    'fchip.notification_delivery.id': delivery.human_friendly_id || delivery.id,
    'fchip.notification_delivery.channel': delivery.channel,
  });
};
...
const finalizeFailure = async (delivery, error) => {
  ...
  markSpanError(error, {
    'fchip.background.event': 'notification_delivery.failed',
  });
};
