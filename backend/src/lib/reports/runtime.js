...
const failRun = async (run, error) => {
  ...
  recordBackgroundJob('report_run.failed', {
    'fchip.report_run.id': run.human_friendly_id || run.id,
    'fchip.report_definition.id': run.report_definition?.human_friendly_id || run.report_definition_id,
  });
  markSpanError(error, {
    'fchip.background.event': 'report_run.failed',
  });
};
...
const processRunById = async (runId) => {
  ...
  try {
    ...
    recordBackgroundJob('report_run.processing', {
      'fchip.report_run.id': run.human_friendly_id || run.id,
      'fchip.report_definition.id': run.report_definition?.human_friendly_id || run.report_definition_id,
      'fchip.report_run.trigger_type': run.trigger_type,
    });
    ...
    recordBackgroundJob('report_run.completed', {
      'fchip.report_run.id': run.human_friendly_id || run.id,
      'fchip.report_definition.id': run.report_definition?.human_friendly_id || run.report_definition_id,
      'fchip.report_run.format': run.format,
    });
...
const enqueueReportRun = async ({
  ...
  recordBackgroundJob('report_run.queued', {
    'fchip.report_run.id': run.human_friendly_id || run.id,
    'fchip.report_definition.id': definition.human_friendly_id || definition.id,
    'fchip.report_run.trigger_type': trigger_type,
    'fchip.report_run.format': run.format,
  });
    ...
    try {
      ...
      if (!existingRun) {
        ...
        recordWorkflowEvent('report_schedule.enqueued', {
          'fchip.report_schedule.id': schedule.human_friendly_id || schedule.id,
          'fchip.report_definition.id': schedule.report_definition_id,
        });
      }
