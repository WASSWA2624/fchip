...
const meter = metrics.getMeter('fchip-backend');
const tracer = trace.getTracer('fchip-backend');

const httpRequestCounter = meter.createCounter('fchip.http.requests.total', {
  description: 'Total HTTP requests handled by the backend.',
});

const httpRequestDuration = meter.createHistogram('fchip.http.request.duration.ms', {
  description: 'HTTP request duration in milliseconds.',
  unit: 'ms',
});

const securityEventCounter = meter.createCounter('fchip.security.events.total', {
  description: 'Security-sensitive events such as auth failures and access denials.',
});

const workflowEventCounter = meter.createCounter('fchip.workflow.events.total', {
  description: 'Operational workflow events for Last Office, break-glass, and related flows.',
});

const businessEventCounter = meter.createCounter('fchip.business.events.total', {
  description: 'Key business workflow counters derived from successful operations.',
});

const backgroundJobCounter = meter.createCounter('fchip.background.jobs.total', {
...
const recordHttpRequest = ({
  ...
  const attributes = normalizeAttributes({
    ...
    'fchip.offline.policy': cache_policy,
    'fchip.tenant.present': tenant_id ? 'true' : 'false',
  });
