      ...
      setImmediate(() => {
        ...
        if (businessEvent) {
          recordBusinessEvent(businessEvent, {
            'fchip.route.family': String(req.baseUrl || '').replace(/^\/api\/v1\//, ''),
            'fchip.status_code': statusCode,
          });
        }
      });

      setSpanAttributes({
        'fchip.request.id': req.requestContext?.request_id || req.request_id || null,
        'fchip.tenant.id': req.user?.tenant_id || req.user?.tenantId || null,
        'fchip.offline.policy': cachePolicy,
      });
    });
