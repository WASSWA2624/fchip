...
const createCloseoutPack = async (data = {}, context = {}) => {
  ...
  recordBackgroundJob('closeout_pack.started', {
    'fchip.closeout_pack.id': publicId,
    'fchip.office_context.id': officeContext.human_friendly_id || officeContext.id,
  });
  ...
  try {
    ...
    recordWorkflowEvent('last_office.closeout_pack_ready', {
      'fchip.closeout_pack.id': serialized.id,
      'fchip.office_context.id': serialized.office_context_id,
    });
    recordBackgroundJob('closeout_pack.completed', {
      'fchip.closeout_pack.id': serialized.id,
      'fchip.closeout_pack.format': serialized.format,
    });
