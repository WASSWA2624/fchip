...
const deliverWebhookReplay = async (webhookSubscription, replayPayload) => {
  ...
  const headers = {
    ...
    'user-agent': 'fchip-backend/webhooks',
    'x-fchip-webhook-event': replayPayload.event,
    'x-fchip-webhook-delivery-id': replayPayload.delivery_id,
    'x-fchip-webhook-timestamp': timestamp};

  if (signingSecret) {
    headers['x-fchip-webhook-signature'] = createWebhookSignature(
      signingSecret,
      timestamp,
      requestBody
    );
