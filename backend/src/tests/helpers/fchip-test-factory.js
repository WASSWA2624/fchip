...
const createTestTenant = (overrides = {}) => ({
  ...
  tenant_name: 'FCHIP Test Tenant',
  tenant_code: 'FCHIP-TEST',
  status: 'ACTIVE',
  created_at: nowIso(),
  updated_at: nowIso(),
  ...overrides});
...
const createTestFacility = (overrides = {}) => ({
  ...
  facility_name: 'FCHIP Test Facility',
...
const createTestActor = (overrides = {}) => {
  ...
  return {
    ...
    tenant_id: DEFAULT_TEST_IDS.tenant_id,
    facility_id: DEFAULT_TEST_IDS.facility_id,
    email: 'fchip.test.user@example.test',
    status: 'ACTIVE',
    roles: roleAssignments,
    permissions: ['tenant:read', 'facility:read'],
    entitlements: ['core.fchip'],
    created_at: nowIso(),
    updated_at: nowIso(),
    ...overrides,
...
const createTestRequestContext = (overrides = {}) => ({
  request_id: 'req_fchip_test_0001',
  ...
  entitlements: ['core.fchip', ...(overrides.entitlements || [])]});
