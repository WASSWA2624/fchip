...
const buildEnv = (overrides = {}) => ({
  APP_DISPLAY_NAME: 'FairBanks Community Health Intelligence Platform',
  SMTP_HOST: null,
  SMTP_PORT: null,
  SMTP_USER: null,
  SMTP_PASS: null,
...
describe('sendEmail helper', () => {
  ...
  it('sends email through SMTP when configuration and dependency are available', async () => {
    ...
    const { sendEmail, logger } = loadSendEmail({
      envOverrides: {
        APP_DISPLAY_NAME: 'FCHIP',
        SMTP_HOST: 'smtp.example.com',
        SMTP_PORT: 587,
        SMTP_USER: 'mailer@example.com',
        SMTP_PASS: 'secret',
        SMTP_FROM_NAME: 'FCHIP Mailer',
        SMTP_FROM: '"FCHIP Team" <team@example.com>',
    ...
    expect(sendMail).toHaveBeenCalledWith(
      expect.objectContaining({
        from: '"FCHIP Mailer" <no-reply@example.com>',
