...
3. Within every product slice, work starts from the Flutter screen: reference → fixture-backed UI → typed contract → matching backend → real wiring → cross-stack proof.
4. Backend domain implementation must not run as a separate later programme. It is part of the active frontend slice.
...
7. Record created, modified, and skipped files plus the screen, backend routes/events, models, migration, seeds, and proof in the slice tracker.
