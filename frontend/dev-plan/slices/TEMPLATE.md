...
Copy this block into [`tracker.md`](./tracker.md) for each screen. Do not close a slice with unfilled sections; write `n/a` with a reason instead of leaving a blank.
...
3. Fill **Backend pairing** in the same slice, before the status can pass `contract-defined`.
4. Status reaches `done` only after **Cross-stack proof** names a passing test or command.
5. If a reference conflicts with another, record the decision under **Notes / deviations**.
