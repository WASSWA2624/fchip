import 'dart:async';

import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:fchip/core/errors/app_failure.dart';
import 'package:fchip/core/errors/result.dart';
import 'package:fchip/features/emergency/data/dtos/emergency_dtos.dart';
import 'package:fchip/features/emergency/data/repositories/emergency_repository_impl.dart';
import 'package:fchip/features/emergency/domain/entities/emergency_entities.dart';
import 'package:fchip/features/emergency/domain/repositories/emergency_repository.dart';
import 'package:fchip/features/emergency/presentation/controllers/emergency_workspace_controller.dart';
import 'package:fchip/features/emergency/presentation/widgets/emergency_dialogs.dart';
import 'package:fchip/shared/components/components.dart';
import 'package:fchip/shared/data/data.dart';
import 'package:mocktail/mocktail.dart';

import '../../helpers/test_harness.dart';

class _MockEmergencyRepository extends Mock implements EmergencyRepository {}

void main() {
  setUpAll(() {
    registerFallbackValue(const EmergencyBoardQuery());
    registerFallbackValue(_originalDetail());
    registerFallbackValue(_originalDetail().summary);
  });

  test('dispatch DTOs prefer public identifiers over internal IDs', () {
    final EmergencyAmbulance ambulance =
        const EmergencyAmbulanceDto(<String, Object?>{
          'id': '123e4567-e89b-12d3-a456-426614174000',
          'human_friendly_id': 'AMB000001',
          'identifier': 'Ambulance 1',
          'status': 'AVAILABLE',
        }).toEntity();
    final EmergencyAmbulanceDispatch dispatch =
        const EmergencyAmbulanceDispatchDto(<String, Object?>{
          'id': '123e4567-e89b-12d3-a456-426614174001',
          'human_friendly_id': 'ADS000001',
          'ambulance_id': 'AMB000001',
          'emergency_case_id': 'EME000001',
          'status': 'DISPATCHED',
        }).toEntity();

    expect(ambulance.id, 'AMB000001');
    expect(dispatch.id, 'ADS000001');
    expect(dispatch.ambulanceId, 'AMB000001');
    expect(dispatch.emergencyCaseId, 'EME000001');
  });

  group('EmergencyWorkspaceController dispatch', () {
    test(
      'patches detail, queue row, and ambulance count after persistence',
      () async {
        final _MockEmergencyRepository repository = _MockEmergencyRepository();
        _stubInitialLoad(repository);
        var persisted = false;
        when(() => repository.loadEmergencyDetail(any())).thenAnswer(
          (_) async => Result<EmergencyCaseDetail>.success(
            persisted ? _dispatchedDetail() : _originalDetail(),
          ),
        );
        when(
          () => repository.dispatchAmbulance(
            detail: any(named: 'detail'),
            ambulanceId: 'AMB000001',
            status: 'DISPATCHED',
          ),
        ).thenAnswer((_) async {
          persisted = true;
          return Result<EmergencyCaseDetail>.success(_dispatchedDetail());
        });
        final ProviderContainer container = _container(repository);
        await container.read(emergencyWorkspaceControllerProvider.future);
        await container
            .read(emergencyWorkspaceControllerProvider.notifier)
            .selectCase(_originalDetail().summary);

        final AppFailure? failure = await container
            .read(emergencyWorkspaceControllerProvider.notifier)
            .dispatchAmbulance(ambulanceId: 'AMB000001');

        final EmergencyWorkspaceState state = _state(container);
        expect(failure, isNull);
        expect(state.selectedDetail?.latestDispatch?.id, 'ADS000001');
        expect(state.board.items.single.latestDispatch?.status, 'DISPATCHED');
        expect(state.ambulanceCount, 1);
        expect(state.isSaving, isFalse);
      },
    );

    test('keeps detail and queue unchanged when persistence fails', () async {
      final _MockEmergencyRepository repository = _MockEmergencyRepository();
      _stubInitialLoad(repository);
      const AppFailure expectedFailure = AppFailure.network();
      when(
        () => repository.dispatchAmbulance(
          detail: any(named: 'detail'),
          ambulanceId: 'AMB000001',
          status: 'DISPATCHED',
        ),
      ).thenAnswer(
        (_) async => const Result<EmergencyCaseDetail>.failure(expectedFailure),
      );
      final ProviderContainer container = _container(repository);
      await container.read(emergencyWorkspaceControllerProvider.future);
      await container
          .read(emergencyWorkspaceControllerProvider.notifier)
          .selectCase(_originalDetail().summary);

      final AppFailure? failure = await container
          .read(emergencyWorkspaceControllerProvider.notifier)
          .dispatchAmbulance(ambulanceId: 'AMB000001');

      final EmergencyWorkspaceState state = _state(container);
      expect(failure, expectedFailure);
      expect(state.selectedDetail?.latestDispatch, isNull);
      expect(state.board.items.single.latestDispatch, isNull);
      expect(state.ambulanceCount, 0);
      expect(state.lastFailure, expectedFailure);
      expect(state.isSaving, isFalse);
    });
  });

  testWidgets(
    'dispatch dialog blocks dismissal and preserves input after failure',
    (WidgetTester tester) async {
      final Completer<AppFailure?> completer = Completer<AppFailure?>();
      DispatchInput? submitted;

      await pumpLocalizedWidget(
        tester,
        DispatchDialog(
          referenceData: const EmergencyReferenceData(
            ambulances: <EmergencyAmbulance>[
              EmergencyAmbulance(
                id: 'AMB000001',
                displayId: 'AMB000001',
                identifier: 'Ambulance 1',
                status: 'AVAILABLE',
              ),
            ],
          ),
          onSubmit: (DispatchInput input) {
            submitted = input;
            return completer.future;
          },
        ),
        size: const Size(390, 700),
        padding: EdgeInsets.zero,
      );

      await tester.tap(find.text('Dispatch'));
      await tester.pump();

      expect(submitted?.ambulanceId, 'AMB000001');
      expect(submitted?.status, 'DISPATCHED');
      expect(_button(tester, 'Cancel').enabled, isFalse);
      expect(_button(tester, 'Dispatch').isLoading, isTrue);

      completer.complete(const AppFailure.network());
      await tester.pumpAndSettle();

      expect(find.byType(DispatchDialog), findsOneWidget);
      expect(find.text('Ambulance 1 | AMB000001'), findsOneWidget);
      expect(_button(tester, 'Cancel').enabled, isTrue);
    },
  );

  testWidgets('status edit reuses DispatchDialog with Edit action', (
    WidgetTester tester,
  ) async {
    DispatchInput? submitted;
    await pumpLocalizedWidget(
      tester,
      DispatchDialog(
        referenceData: const EmergencyReferenceData(),
        purpose: DispatchDialogPurpose.editStatus,
        initialAmbulanceId: 'AMB000001',
        initialStatus: 'EN_ROUTE',
        onSubmit: (DispatchInput input) async {
          submitted = input;
          return null;
        },
      ),
      size: const Size(800, 500),
      padding: EdgeInsets.zero,
    );

    expect(find.text('EDIT DISPATCH STATUS'), findsOneWidget);
    expect(find.text('Ambulance ID'), findsNothing);
    expect(find.text('Edit'), findsOneWidget);

    await tester.tap(find.text('Edit'));
    await tester.pumpAndSettle();

    expect(submitted?.ambulanceId, 'AMB000001');
    expect(submitted?.status, 'EN_ROUTE');
  });
}

AppButton _button(WidgetTester tester, String label) {
  return tester.widget<AppButton>(
    find.byWidgetPredicate(
      (Widget widget) => widget is AppButton && widget.label == label,
    ),
  );
}

ProviderContainer _container(_MockEmergencyRepository repository) {
  final ProviderContainer container = ProviderContainer(
    overrides: <Object?>[
      emergencyRepositoryProvider.overrideWithValue(repository),
    ].cast(),
  );
  addTearDown(container.dispose);
  return container;
}

void _stubInitialLoad(_MockEmergencyRepository repository) {
  when(() => repository.listEmergencyBoard(any())).thenAnswer((invocation) {
    final EmergencyBoardQuery query =
        invocation.positionalArguments.single as EmergencyBoardQuery;
    return Future<Result<AppPage<EmergencyCaseSummary>>>.value(
      Result<AppPage<EmergencyCaseSummary>>.success(
        AppPage<EmergencyCaseSummary>(
          items: <EmergencyCaseSummary>[_originalDetail().summary],
          request: query.pageRequest,
          totalItemCount: 1,
        ),
      ),
    );
  });
  when(() => repository.loadReferenceData()).thenAnswer(
    (_) async =>
        const Result<EmergencyReferenceData>.success(EmergencyReferenceData()),
  );
  when(() => repository.loadEmergencyDetail(any())).thenAnswer(
    (_) async => Result<EmergencyCaseDetail>.success(_originalDetail()),
  );
}

EmergencyWorkspaceState _state(ProviderContainer container) {
  return container
      .read(emergencyWorkspaceControllerProvider)
      .requireValue
      .when(
        success: (EmergencyWorkspaceState state) => state,
        failure: (AppFailure failure) => throw StateError(failure.code),
      );
}

EmergencyCaseDetail _originalDetail() {
  return const EmergencyCaseDetail(
    summary: EmergencyCaseSummary(
      id: 'EME000001',
      displayId: 'EME000001',
      patientId: 'PAT000001',
      patientDisplayId: 'PAT000001',
      patientDisplayName: 'Jane Doe',
      severity: 'HIGH',
      status: 'OPEN',
    ),
    triageAssessments: <EmergencyTriageAssessment>[
      EmergencyTriageAssessment(
        id: 'TRA000001',
        emergencyCaseId: 'EME000001',
        triageLevel: 'LEVEL_2',
      ),
    ],
    responses: <EmergencyResponseRecord>[
      EmergencyResponseRecord(
        id: 'ERS000001',
        emergencyCaseId: 'EME000001',
        notes: 'Stabilized',
      ),
    ],
  );
}

EmergencyCaseDetail _dispatchedDetail() {
  const EmergencyAmbulanceDispatch dispatch = EmergencyAmbulanceDispatch(
    id: 'ADS000001',
    displayId: 'ADS000001',
    emergencyCaseId: 'EME000001',
    ambulanceId: 'AMB000001',
    ambulanceDisplayId: 'AMB000001',
    ambulanceLabel: 'Ambulance 1',
    status: 'DISPATCHED',
  );
  final EmergencyCaseDetail original = _originalDetail();
  return original.copyWith(
    summary: original.summary.copyWith(latestDispatch: dispatch),
    dispatches: const <EmergencyAmbulanceDispatch>[dispatch],
  );
}
